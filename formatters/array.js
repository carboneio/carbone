...
// It stores loop IDs and their current iteration
var loopIds = {};

/**
 * Get row number of loop, will be called several times (each loop row)
 * Loops are unique thanks to loopId (given in preprocessMarkers)
 *
 * @param   {String}  d       Array passed by carbone
 * @param   {Integer} loopId  Loop ID generated by carbone
 * @param   {String}  start   Number to start with (default: 0)
 * @return  {Integer}         Correct row number
 */
function rowNumber (d, loopId, start) {
  if (start === undefined) {
    start = 0;
  }
  if (loopIds[loopId] === undefined) {
    loopIds[loopId] = {
      current : parseInt(start, 10)
    }
  }
  return loopIds[loopId].current++;
}
