...
var analyzer = {
  ...
  decomposeTags : function(tags){
    ...
    for (var i = 0; i < tags.length; i++) {
      ...
      var _prevTagPart = _tagParts[0].replace(/^ */g, '').replace(/ *$/g, '');
      _depth = 0;
      for (var j = 1; j < _tagParts.length; j++) {
        // We split the tagName to extract formatters that may be applied to it
        var _formatters = _tagParts[j].split(':');
        // The tag is always in first position of the split
        // We replace spaces at the begin and at the end of the tag to clean it
        /*var _tagPart = _formatters[0];*/
        var _tagPart = _formatters[0].replace(/^ */g, '').replace(/ *$/g, '');
        // Formatters  are the rest of the split
        // date:parse(YYYYMMDD):format(DD/MM/YYYY)
        // Will be splitted : [ 'parse(YYYYMMDD)', 'format(DD/MM/YYYY)' ]
        _formatters = _formatters.slice(1).map(function(formatter){
