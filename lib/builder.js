  ...
  getBuilderFunction : function (descriptor, existingFormatters) {
    ...
    for (var _objIndex = 0; _objIndex < _hierarchy.length; _objIndex++) {
      ...
      if (_type === 'object') {
        ...
        _code.add('prev', 'main', that.getFilterString(_getSafeVar, _getSafeValue, _iteratorsUsedToGroupByInAggrerators, _currentParentsVariable, _conditionToFindObject, _foundCode , false, _objNameTemp));
      ...
      for (var i = 0; i < _xmlParts.length; i++) {
        ...
        if (_dataAttr) {
          ...
          // if a loop is used with a filter, the Carbone tag which is added in preprocessing to generate unique id must be neutral regarding row filtering
          // otherwise, an entire row can stay visible because one Carbone tag is still visible!
          if (_xmlPart.parsedFormatter?.[0]?.str === 'generateImageDocxGlobalId') {
