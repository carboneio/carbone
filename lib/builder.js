...
var builder = {
      ...
      parser.findVariables(xml, options.existingVariables, function (err, xmlWithoutVariable, variables) {
        ...
        parser.findMarkers(xmlWithoutVariable, function (err, xmlWithoutMarkers, markers) {
          ...
          parser.preprocessMarkers(markers, variables, function (err, preprocessedMarkers) {
            options.aggDatabase = new Map(); // reset aggregators
            var _xmlParts = [];
            var _builder;
            try {
            ...
            postprocessor.update(data, options, function (err) {
              ...
              var _xmlResult = '';
              try {
                const concatTime = options.metrics?.time('concatTime');
  ...
  getFormatterString : function (getSafeValue, varName, xmlPos, contextName, currentIterators, currentParents, parsedFormatters = [], existingFormatters, isCalledInReduceLoop = false) {
    ...
    for (let i = 0; i < parsedFormatters.length; i++) {
      ...
      else if (_originalFormatterFn === false) {
        throw Error('Formatter "'+_originalFormatterStr+'" is disabled in the Community Edition.');
      }
    ...
    for (var _objIndex = 0; _objIndex < _hierarchy.length; _objIndex++) {
      ...
      if (_type === 'object') {
        ...
        if ( _isArrayOfArrayAccess===true) {
          ...
          _loopCode += 'if (!Array.isArray('+_arrayOfObjectNameG+') && '+_arrayOfObjectNameG+' !== null && typeof '+_arrayOfObjectNameG+' === "object") {\n';
          _loopCode += _arrayOfObjectNameG+' = Object.entries('+_arrayOfObjectNameG+').map((a) => { return {att : a[0], val : a[1] } });\n';
          _loopCode += '}\n';
        }
        ...
        for (var it = 0; it < _iterators.length; it++) {
          ...
          if (_iterator.attr === 'i') {
            ...
            if ( _repeater ) {
              ...
              _code.add('prev', 'main', ' if ( '+_repeaterName+' > '+parseInt(params.maxRepetitionFactor,10)+' ) { throw Error("The repeater cannot be above '+parseInt(params.maxRepetitionFactor,10)+'"); }\n');
              // TODO
      ...
      for (var i = 0; i < _xmlParts.length; i++) {
        ...
        if (_dataAttr !== undefined && _dataAttr !== null && (_dataAttr !== '' || _dynamicData[_dataObj].hasSubAtt !== true /* #patch20230227 */)) {
          let _currentParentsVariable = [_getSafeVar(_dataObj), ..._objParentNames.map(_getSafeVar)];
          // handle conditions
