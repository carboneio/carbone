...
var builder = {
  ...
  buildXML : function(xml, data, options, callback){
    ...
    var _langFilePath =  path.resolve(params.templatePath+'/lang/', params.lang+'.json');
    fs.readFile(_langFilePath, 'utf8', function (err,langData) {
      if (err) {
        if(err.code == 'ENOENT')
        {
      ...
      var objLang = JSON.parse(langData);
      //find translation markers {t()}
      parser.translate(xml,objLang,function(err, xmlWithoutTranslation){
