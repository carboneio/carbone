        ...
        parser.findMarkers(xmlWithoutVariable, function (err, xmlWithoutMarkers, markers) {
          ...
          parser.preprocessMarkers(markers, variables, function (err, preprocessedMarkers) {
            ...
            try {
              ...
              _xmlParts = _builder(_obj, options, _builder.builderDictionary);
  ...
  getFormatterString : function (getSafeValue, varName, contextName, formatters, existingFormatters, onlyFormatterWhichInjectXML) {
    ...
    for (var i = 0; i < formatters.length; i++) {
      ...
      if (_indexFirstParenthesis !== -1) {
        ...
        for (var j = 0; j < _arguments.length; j++) {
          if (_arguments[j] !== '') {
            ...
            _argumentStr.push(_arguments[j].replace(/^ *'?/, '').replace(/'? *$/, ''));
          }
        }
      }
      else {
      ...
      if ( (existingFormatters[_functionStr].canInjectXML === true && onlyFormatterWhichInjectXML === true)
        ...
        _lineOfCodes.push(
          getSafeValue(_argumentStr) +'[0] = ' + varName +';\n'
          + varName +' = formatters.' + _functionStr + '.apply(' + contextName + ', ' + getSafeValue(_argumentStr) + ');\n'
        );
      }
    }
  ...
  assembleXmlParts : function (arrayOfStringPart, sortDepth, builderDictionary) {
    ...
    for (var i = 0; i < arrayOfStringPart.length; i++) {
      ...
      if (_part.bef !== undefined) {
        _str = builderDictionary[_part.bef];
      }
      _str += _part.str;
      if (_part.aft !== undefined) {
        _str += builderDictionary[_part.aft];
      }
      _res += _str;
  ...
  getBuilderFunction : function (descriptor, existingFormatters) {
    ...
    for (var _objIndex = 0; _objIndex < _hierarchy.length; _objIndex++) {
      ...
      // Array type
        ...
        // declare any nested object
        if (_objName!=='_root') {
          _code.add('prev', 'main', 'var '+_arrayNameG+'='+ _getSafeVar(_objParentName)+'['+_realObjName+'];\n');
        }
        else {
        ...
        _code.add('prev','main', 'var '+_arrayNameG+'_length = ('+_arrayNameG+' instanceof Array)?'+_arrayNameG+'.length : 0;\n');
      ...
      for (var i = 0; i < _xmlParts.length; i++) {
        ...
        if (_dataAttr) {
          ...
          _code.add('main', 'if('+ _getSafeVar(_dataObj) +' !== undefined){\n');
          _code.add('main', 'var _str = ' + _getSafeVar(_dataObj) + '[' + _dataAttrG + ']' + ';\n');
    ...
    var _fn;
    try {
      _fn = new Function('data', 'options', _dictionaryName, _codeAssembled);
