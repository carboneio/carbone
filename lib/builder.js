...
var builder = {
        ...
        parser.findMarkers(xmlWithoutVariable, function (err, xmlWithoutMarkers, markers) {
          ...
          parser.preprocessMarkers(markers, variables, function (err, preprocessedMarkers) {
            ...
            postprocessor.update(data, options, function (err) {
              ...
              if (options.extension === 'docx' && _descriptor.isContainingHorizontalLoop === true) {
                ...
                // Should we remove it everytime for all docx in preprocessing? It would be faster. To be safe, we do it only if the docx contain at least
                // one horizontal loops.
                // MS spec says: https://learn.microsoft.com/en-us/dotnet/api/documentformat.openxml.wordprocessing.tablegrid?view=openxml-2.8.1
                ...
                _xmlResult = _xmlResult.replaceAll(/<w:tblGrid>[\s\S]+?<\/w:tblGrid>/g, '');
              }
  ...
  getFormatterString : function (getSafeValue, varName, xmlPos, contextName, currentIterators, currentParents, parsedFormatters = [], existingFormatters, isCalledInReduceLoop = false) {
    ...
    function getInjectedArgument (argument) {
      ...
      const _argumentWithoutStartDot = argument.replace(helper.HIDDEN_CHAR_DISABLE_VARIABLE, '').slice(_nbStartDot);
