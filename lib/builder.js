    ...
    for (var c = 0; c < conditions.length; c++) {
      ...
      if (_attr === 'i') {
        ...
        if (parseInt(_condition.right, 10) < 0) {
          _rightOperator = _objName + '_array_length ' +  _condition.right;
        }
        _str += _objName + '_i '+_condition.operator + _rightOperator;
      }
      else {
        if (_alreadyDeclaredFilter[_objName] === undefined) {
          _str += _objName + ' && ';
  ...
  getBuilderFunction : function (descriptor, existingFormatters) {
    ...
    for (var _objIndex = 0; _objIndex < _hierarchy.length; _objIndex++) {
      ...
      if (_type === 'object') {
        ...
        _code.add('prev','main', _arrayOfObjectName+'='+'('+_objParentName+' !== undefined)? '+_objParentName+'[\''+_realObjName+'\']:[];\n');
        for (var i = 0; i < _xmlParts.length; i++) {
          var _xmlPart = _xmlParts[i];
          var _conditions = _xmlPart.conditions || [];
          var _condition = _conditions[0];
          
          _code.add('prev', 'main', _objName+' = {};\n');
          ...
          _code.add('prev', 'main', that.getFilterString(_conditions, _objName+'='+_arrayOfObjectName+'['+_arrayOfObjectIndexName+'];\n break;\n', _objName, false));
          _code.add('prev', 'main', '}\n');

          //_code.add('prev','main', _objName+'='+'('+_arrayOfObjectName+' instanceof Array)? '+_arrayOfObjectName+'['+_condition.right+']:{};\n');
        }
      }
      // Array type
      else if (_type === 'array') {
      ...
      for (var i = 0; i < _xmlParts.length; i++) {
        ...
        if (_dataAttr) {
          ...
          if(_xmlPart.isConditionUsedWithoutIterator === false) {
          ...
          _code.add('main', '};\n');
          //if(_xmlPart.isConditionUsedWithoutIterator === true) {
          //  
          //}
