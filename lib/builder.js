...
var builder = {
        ...
        parser.findMarkers(xmlWithoutVariable, function (err, xmlWithoutMarkers, markers) {
          ...
          parser.preprocessMarkers(markers, variables, function (err, preprocessedMarkers) {
            ...
            try {
              ...
              // console.log(JSON.stringify(_descriptor));
            ...
            postprocessor.update(data, options, function (err) {
              ...
              var _xmlResult = builder.assembleXmlParts(_xmlParts, 20, options); // TODO -> adapt the depth of the sort according to the maximum depth in _xmlParts
   ...
   */
  assembleXmlParts : function (arrayOfStringPart, sortDepth, options) {
    var _res = '';
    ...
    for (var i = 0; i < arrayOfStringPart.length; i++) {
      ...
      // Execute postProcess formatters
      if (_part.postProcess instanceof Object) {
        var _previousText = _part.str;
        _part.str =   _previousText.slice(0, _part.postIndex);
        _part.str +=  _part.postProcess.fn.apply(options, _part.postProcess.args);
        _part.str +=  _previousText.slice(_part.postIndex);
      }
      _res += _part.str;
  ...
  getBuilderFunction : function (descriptor, existingFormatters) {
    ...
    for (var _objIndex = 0; _objIndex < _hierarchy.length; _objIndex++) {
      ...
      for (var i = 0; i < _xmlParts.length; i++) {
        ...
        if (_dataAttr) {
          ...
          _code.add('main', 'if('+ _dataObj +' !== undefined && '+ _dataObj +' !== null){\n');
          ...
          _code.add('main', 'context.parentsData = ['+_dataObj+', '+_objParentNames.join(',')+'];\n');
          _code.add('main', that.getFormatterString('_str', 'context', _formatters, existingFormatters, false));
          // if a formatter return an object, the formatter will be called at the end of the process
          _code.add('main', 'if(_str instanceof Object) {\n');
          _code.add('main', '  _strPart.postProcess = _str;\n');
          // keep the index where to inject the string during post processing (between "before" and "after" xml string)
          _code.add('main', '  _strPart.postIndex = _strPart.str.length;\n');
          ...
          _code.add('main', '  console.warn("Carbone WARNING: The marker \\"' + _dataAttr + '\\" is undefined.");\n');
          _code.add('main', "  _str = '';\n");
          _code.add('main', '};\n');
          ...
          _code.add('main', that.getFormatterString('_str', 'context', _formatters, existingFormatters, true));
    ...
    var _fn;
    try {
      _fn = new Function('data', 'context', 'helper', _codeAssembled);
