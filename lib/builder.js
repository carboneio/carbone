...
var builder = {
        ...
        parser.findMarkers(xmlWithoutVariable, function (err, xmlWithoutMarkers, markers) {
          ...
          parser.preprocessMarkers(markers, variables, function (err, preprocessedMarkers) {
            ...
            try {
              ...
              var _descriptor = extracter.splitXml(xmlWithoutMarkers, _dynamicDescriptor, markers);
  ...
  getFormatterString : function (getSafeValue, varName, xmlPos, contextName, currentIterators, currentParents, parsedFormatters = [], existingFormatters, isCalledInReduceLoop = false) {
    ...
    for (let i = 0; i < parsedFormatters.length; i++) {
      ...
      for (let k = 0; k < _formatters.length; k++) {
        ...
        for (let j = 0; j < _formatterArgs.length; j++) {
          ...
          const _nbStartDot = (_argument.match(/[^.]/))?.index ?? 0;
          const _argumentWithoutStartDot = _argument.slice(_nbStartDot);
          ...
          _injectedArguments.push(_injectedArgument);
        }
        const _allArgs = [contextName, varName, ..._injectedArguments];
        let _sourceCode = '';
        if (_isPropagationUpdatedByPreviousFormatter === true) {
          _isPropagationUpdatedByPreviousFormatter = false;
          _sourceCode += 'if('+contextName+'.stopPropagation === false) {\n';
          _lineOfCodes._nbConditionToClose++;
        }
        _sourceCode += varName +' = formatters.' + _formatterStr + '.call(' + _allArgs.join(', ') + ');\n';
      ...
      _isPropagationUpdatedByPreviousFormatter = _originalFormatterFn.isUpdatingPropagation === true;
    ...
    // Close opened condition with "stopPropagation"
    _lineOfFormattersSourceCode[_lineOfFormattersSourceCode.length-1]                                   += '}\n'.repeat(_lineOfFormattersSourceCode._nbConditionToClose);
    _lineOfFormattersSourceCodePostponeExecution[_lineOfFormattersSourceCodePostponeExecution.length-1] += '}\n'.repeat(_lineOfFormattersSourceCodePostponeExecution._nbConditionToClose);
   ...
   */
  getFilterStringOld : function (getSafeVar, getSafeValue, currentIterators, currentParents, conditions, codeIfTrue, prefix, inverseCondition, forceObjectTested) {
    prefix = prefix || '';
  ...
  getFilterString : function (getSafeVar, getSafeValue, currentIterators, currentParents, conditions, codeIfTrue, prefix, inverseCondition, forceObjectTested) {
    if (!codeIfTrue || !(conditions instanceof Array) || conditions.length ===0) {
      return '';
    }
    ...
    for (var c = 0; c < conditions.length; c++) {
      ...
      currentParents[0] = _condition.left.parent;
      if (forceObjectTested) {
        currentParents[0] = forceObjectTested;
      }
      if (_leftOperand === 'i') {
        ...
        // for boolean, convert data to string to accept both cases ('true' == true) and (true == true) for backward compatibility with Carbone v1 & v2
        const _leftOperandCodeCast = (_condition.right === 'true' || _condition.right === 'false') ? _leftOperandCode+"+''" : _leftOperandCode;
  ...
  getBuilderFunction : function (descriptor, existingFormatters) {
    ...
    for (var _objIndex = 0; _objIndex < _hierarchy.length; _objIndex++) {
      ...
      if (_type === 'object') {
        ...
        var _firstObjectFound = _objName+'_firstObjectFound';

        _code.add('prev', 'main', _getSafeVar(_objName) + '={};\n');
        _code.add('prev', 'main', 'var '+_getSafeVar(_firstObjectFound) + '= null;\n'); // TODO test with undefined
        ...
        let _foundCode = _getSafeVar(_objName)+'='+_getSafeVar(_objNameTemp)+';\n break;\n';
        if (_aggCode !== '') {
          _foundCode = 'if ('+_getSafeVar(_firstObjectFound)+' === null) { ';
          _foundCode +=   _getSafeVar(_firstObjectFound)+'='+_getSafeVar(_objNameTemp);
          _foundCode += '}';
          _foundCode += _getSafeVar(_objName)+'='+_getSafeVar(_objNameTemp)+';\n';
          _foundCode += _aggCode;
        }
        let _currentParentsVariable = [_getSafeVar(_objNameTemp), ..._objParentNames.map(_getSafeVar)];
        _code.add('prev', 'main', that.getFilterString(_getSafeVar, _getSafeValue, _iteratorsUsedToGroupByInAggrerators, _currentParentsVariable, _conditionToFindObject, _foundCode , _objNameTemp, false, _objNameTemp));
        if (_conditionToFindObject.length === 0) {
          _code.add('prev', 'main',  _foundCode);
        }
        _code.add('prev', 'main', '}\n');
        if (_aggCode !== '') {
          _code.add('prev', 'main', _getSafeVar(_objName)+'='+_getSafeVar(_firstObjectFound)+' ?? {};\n'); // TODO test with undefined
        }
      }
      // Array type
      else if (_type === 'array') {
      ...
      for (var i = 0; i < _xmlParts.length; i++) {
        ...
        if (_dataAttr) {
          ...
          _code.add('main', that.getFilterString(_getSafeVar, _getSafeValue, _iteratorsUsedToGroupByInAggrerators, _currentParentsVariable, _conditions, '_strPart.rowShow = false', _getSafeVar(_objName), true));
          _code.add('main', '_str = "";\n');
          ...
          _code.add('main', '  if (!_str.args || !_str.args[0]){ _str = null; } else { _str = ""; }\n');
          _code.add('main', '};\n');
