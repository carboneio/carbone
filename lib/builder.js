...
var builder = {
   ...
   */
  getFilterString : function(objName, conditions, codeIfTrue, prefix, inverseCondition){
    prefix = prefix || '';
    if(!codeIfTrue || (conditions && conditions.length ===0) || !objName){
      return '';
    }
    var _str = '';
    ...
    for (var c = 0; c < conditions.length; c++) {
      ...
      var _attr = _condition.left;
      var _objName = objName;
      //if the left part is an object
      var _objectSeparatorIndex = _condition.left.indexOf('.');
      if(_objectSeparatorIndex !== -1){
        var _obj = _condition.left.slice(0, _objectSeparatorIndex);
        _attr = _condition.left.slice(_objectSeparatorIndex+1);
        _objName = '_'+prefix+_obj+'_filter';
        if(_alreadyDeclaredFilter[_objName] === undefined){
          _declareStr += 'var '+_objName+'='+objName+'["'+_obj+'"];\n';
        }
      }
      if(_alreadyDeclaredFilter[_objName] === undefined){
        _str += _objName + ' && ';
  ...
  getBuilderFunction : function(descriptor){
    ...
    for (var _objIndex = 0; _objIndex < _hierarchy.length; _objIndex++) {
      ...
      if(_nestedArray.length>0){
        var _firstParentOfTypeArray = _objParentName;
        while(_firstParentOfTypeArray !== '' && _dynamicData[_firstParentOfTypeArray].type !== 'array'){
          _firstParentOfTypeArray = _dynamicData[_firstParentOfTypeArray].parent;
        };
        var _lastArrayName = _nestedArray[_nestedArray.length-1];
        while(_firstParentOfTypeArray !== _lastArrayName && _nestedArray.length>0){
          _lastArrayName = _nestedArray[_nestedArray.length-1];
          _str += "}\n";
          if(_dynamicData[_lastArrayName].iterator !== undefined){
            _nbCustomIterator--;
          }
          _nestedArray.pop();
        }
      ...
      for (var i = 0; i < _xmlParts.length; i++) {
        ...
        if(_dataAttr){
          ...
          if (_partDepth > 0){
            ...
            _str += that.getFilterString(_dataObj, _conditions, '_strPart.rowShow = false', _objName, true);
          }
