...
var builder = {
        ...
        parser.findMarkers(xmlWithoutVariable, function (err, xmlWithoutMarkers, markers) {
          ...
          parser.preprocessMarkers(markers, variables, function (err, preprocessedMarkers) {
            ...
            try {
              ...
              var _builder = builder.getBuilderFunction(_descriptor, options.formatters, options.extension);
              var _obj = {
                d : data,
   ...
   * @return {function} f
   */
  getBuilderFunction : function (descriptor, existingFormatters, extension) {
    var that = this;
    ...
    for (var _objIndex = 0; _objIndex < _hierarchy.length; _objIndex++) {
      ...
      for (var i = 0; i < _xmlParts.length; i++) {
        ...
        if (_dataAttr) {
          ...
          const lineBreak = LINEBREAK[extension];
          
          if (lineBreak) {
            _code.add('main', `  _str = _str.replace(/\\r?\\n/g, '${lineBreak}');`);
          }
