    ...
    parser.translate(xml, options, function (err, xmlWithoutTranslation) {
      ...
      parser.findVariables(xmlWithoutTranslation, options.existingVariables, function (err, xmlWithoutVariable, variables) {
        ...
        parser.findMarkers(xmlWithoutVariable, function (err, xmlWithoutMarkers, markers) {
          ...
          // Find dynamic pictures markers. There are created programmatically by analyzing xml tags and markers
          parser.findPictures(xmlWithoutMarkers, markers, function (err, xmlWithoutMarkers, markers) {
    ...
    for (var _objIndex = 0; _objIndex < _hierarchy.length; _objIndex++) {
      ...
      for (var i = 0; i < _xmlParts.length; i++) {
        ...
        if (_dataAttr) {
          ...
          // If the marker has the 'pic' type, we insert an inline draw:image with correct link
          _code.add('main', "if('"+ _dataType +"' === 'pic') {\n");
          _code.add('main', 'var _strValue = ' + _dataObj + "['" + _dataAttr + "']" + ';\n');
          _code.add('main', 'var _str = \'<draw:image xlink:href="\' + _strValue +\'" xlink:type="simple" xlink:show="embed" xlink:actuate="onLoad"/>\';\n');
          _code.add('main', "} else {\n");
