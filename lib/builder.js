...
var builder = {
   ...
   *                                  'existingVariables' : existing variables (object returned by parser.findVariables())
   *                                }
  ...
  assembleXmlParts : function(arrayOfStringPart, sortDepth){
    ...
    for (var i = 0; i < arrayOfStringPart.length; i++) {
      ...
      if(JSON.stringify(_prevPos)!==JSON.stringify(_part.pos)){
        _prevPos = _part.pos;
      }
      else{
        continue;
