...
var builder = {
  ...
  forEachArrayExit: function (currentlyVisitedArrays, objDependencyDescriptor, nextAttrName, execute) {
    ...
    if (currentlyVisitedArrays.length > 0) {
      ...
      if (!objDependencyDescriptor[nextAttrName]) debugger;
      if (!objDependencyDescriptor[_firstParentOfTypeArray]) debugger;
    ...
    for (var _objIndex = 0; _objIndex < _hierarchy.length; _objIndex++) {
      ...
      for (var i = 0; i < _xmlParts.length; i++) {
        ...
        if (_dataAttr) {
          ...
          _code.add('main', 'if(_str === null || _str === undefined || /^undefined\:.+/.test(_str)) {\n');
          _code.add('main', "  _str = '';\n");
          _code.add('main', '};\n');
          _code.add('main', 'if (_formatterOptions.isHidden !== null){\n');
          _code.add('main', '  _strPart.hide = _formatterOptions.isHidden;\n');
          _code.add('main', '}\n');
        ...
        _code.add('main', `var posNumber = posNumber || 0;\n`);
        if (_xmlPart.after) {
          _code.add('main', "_strPart.str += '" + _xmlPart.after + "';\n");
          _code.add('main', `posNumber = +(_strPart.str.match(/r=".*?(\\d*)"/) || [])[1] || 0;\n`);
          if (_arrayIndexName) {
            _code.add('main', `_strPart.str = _strPart.str.replace(/(r=".*?)(\\d+)"/g, \`$1\${posNumber+${_arrayIndexName || 0}\}"\`);\n`);
          }
        }
        ...
        if (_noIteratorTemplate === false) {
          if (_arrayIndexName) {
            _code.add('main', `var formulas =  _strPart.str.match(/<f.*?>.*?<\\/f>/g); \n`);
            _code.add('main', `if(formulas && formulas.length) { 
              formulas.forEach(formula => {
              var newFormula = formula;
              var dataCells = formula.match(/((\\w+\\d+:\\w+\\d+)|(\\w+\\d+))/g);
              dataCells.forEach(item => {
                if(item.indexOf(':')!=-1) {
                  var cells = item.split(':').map(cell => {
                    var [group, col, row] = cell.match(/(\\w+)(\\d+)/); 
                    return {value: cell, row , col }
                  });
                  if(cells[0].row == cells[1].row) {
                    newFormula = newFormula.replace(item, item.replace(new RegExp(posNumber, 'g'), posNumber+${_arrayIndexName || 0}));
                  } 
                } else {
                  newFormula = newFormula.replace(item, item.replace(posNumber, posNumber+${_arrayIndexName || 0}));
                }
              })
              _strPart.str = _strPart.str.replace(formula, newFormula);
            })
             }\n`);
            _code.add('main', `_strPart.str = _strPart.str.replace(/(r=".*?)(\\d+)"/g, \`$1\${posNumber+${_arrayIndexName || 0}\}"\`);\n`);
          }
