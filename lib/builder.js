...
var builder = {
  ...
  /**
   * Replace __ROW_NUMBER_*_*__ by the right value (part.str)
  ...
  getLoopIteration : function (loopIds, part) {
    var _rowColumnRegex = /__ROW_NUMBER_(\d*?)_(\d*?)__/;
    var _rowColumnMatch = _rowColumnRegex.exec(part.str);

    // If we match a ROW_NUMBER marker
    if (_rowColumnMatch !== null && part.rowShow) {
      ...
      part.str = part.str.replace(_rowColumnMatch[0], loopIds[_loopId].current++);
    }
  },
