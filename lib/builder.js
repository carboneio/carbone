...
var builder = {
      ...
      parser.findVariables(xmlWithoutTranslation, options.existingVariables, function (err, xmlWithoutVariable, variables) {
        ...
        parser.findMarkers(xmlWithoutVariable, function (err, xmlWithoutMarkers, markers) {
          ...
          parser.preprocessMarkers(markers, variables, function (err, preprocessedMarkers) {
            ...
            var _xmlResult = builder.assembleXmlParts(_xmlParts, 20, _builder.xmlDictionary); // TODO -> adapt the depth of the sort according to the maximum depth in _xmlParts
   ...
   * @param {array} xmlDictionary : xml part coming from template
   * @return {string} final result
   */
  assembleXmlParts : function (arrayOfStringPart, sortDepth, xmlDictionary) {
    var _res = '';
    ...
    for (var i = 0; i < arrayOfStringPart.length; i++) {
      ...
      if (_part.bef !== undefined) {
        _res += xmlDictionary[_part.bef];
      }
      _res += _part.str;
      if (_part.aft !== undefined) {
        _res += xmlDictionary[_part.aft];
      }
  ...
  /**
   * Generate a function which updates xmlDictionary each time a new XML part must be injected
   *
   * It generate a function for usage conveniance
   *
   * @param  {Array} xmlDictionary  Array of XMl string
   * @return {Integer}              number which represent the index where the corresponding XML string is.
   */
  injectXMLSecurely : function (xmlDictionary) {
    return (xmlString) => {
      return xmlDictionary.push(xmlString) - 1;
    };
  },
  ...
  getBuilderFunction : function (descriptor, existingFormatters) {
    ...
    var _xmlDictionay = [];
    var _addXML = that.injectXMLSecurely(_xmlDictionay);
    ...
    if (_staticData.before !== '') {
      ...
      _code.add('main', "  'str' : '',\n");
      _code.add('main', "  'bef' : "+_addXML(_staticData.before) + '\n');
      _code.add('main', '};\n');
    ...
    for (var _objIndex = 0; _objIndex < _hierarchy.length; _objIndex++) {
      ...
      // Array type
        ...
        if (_beforeStr !== undefined && _beforeStr !== '') {
          ...
          _code.add('main', "  'str' : '',\n");
          _code.add('main', "  'bef' : "+_addXML(_beforeStr) + '\n');
          _code.add('main', '};\n');
          _code.add('main', '_strParts.push(_strPart);\n');
        }
        // store array name
      ...
      for (var i = 0; i < _xmlParts.length; i++) {
        ...
        _code.add('main', '};\n');
        if (_xmlPart.before) {
          _code.add('main', '_strPart.bef = '+_addXML(_xmlPart.before) + ';\n');
        }
        ...
        if (_xmlPart.after) {
          _code.add('main', '_strPart.aft = ' + _addXML(_xmlPart.after) + ';\n');
        }
    ...
    if (_staticData.after !== '') {
      ...
      _code.add('main', "  'str' : '',\n");
      _code.add('main', "  'aft' : "+ _addXML(_staticData.after) + '\n');
      _code.add('main', '};\n');
      _code.add('main', '_strParts.push(_strPart);\n');
    }
    ...
    try {
      ...
      _fn.xmlDictionary = _xmlDictionay;
    }
    catch (err) {
