...
var conditionFormatter = require('../formatters/condition.js');
  ...
  getFormatterString : function (getSafeValue, varName, contextName, formatters, existingFormatters, onlyFormatterWhichInjectXML) {
    ...
    for (var i = 0; i < formatters.length; i++) {
      ...
      if ( (existingFormatters[_functionStr].canInjectXML === true && onlyFormatterWhichInjectXML === true)
        ...
        const _formatterCall = varName +' = formatters.' + _functionStr + '.call(' + contextName + ', ' + varName + _argumentStr + ');\n';
        if (conditionFormatter[_functionStr] !== undefined) {
          _lineOfCodes.push('if('+contextName+'.stopPropagation === false){\n');
          _lineOfCodes.push(_formatterCall);
          _lineOfCodes.push('}');
        }
        else {
          _lineOfCodes.push(_formatterCall);
