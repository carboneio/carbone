  ...
  getBuilderFunction : function (descriptor, existingFormatters) {
    ...
    for (var _objIndex = 0; _objIndex < _hierarchy.length; _objIndex++) {
      ...
      // Array type
        ...
        var _duplicator = _dynamicData[_objName].duplicator;
        var _duplicatorGlobalCounter = _getSafeVar(_objName)+'_repeatCount';
        ...
        if ( _duplicator ) {
          _code.add('prev','main', 'var '+_duplicatorGlobalCounter+' = 0;\n');
          _nestedArray.push(_objName);
        }
        ...
        for (var it = 0; it < _iterators.length; it++) {
          ...
          if (_iterator.attr === 'i') {
            ...
            if ( _duplicator ) {
              var _duplicatorAttr = _getSafeValue(_duplicator);
              var _duplicatorName = _getSafeVar(_objName)+'_repeat';
              _code.add('prev', 'main', ' let '+_duplicatorName+' = '+_getSafeVar(_objName)+'['+_duplicatorAttr+'];\n');
              _code.add('main', 'while( '+_duplicatorName+'-- > 0) {\n');
              _code.add('main', '  _xmlPos['+(_nbCustomIterator+_arrayDepth*2-1)+'] = '+_duplicatorGlobalCounter+'++;\n');
            }
          }
          else {
