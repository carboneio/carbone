  ...
  getFilterString : function (getSafeVar, getSafeValue,  conditions, codeIfTrue, prefix, inverseCondition, forceObjectTested) {
    ...
    for (var c = 0; c < conditions.length; c++) {
      ...
      if (_attr === 'i') {
        ...
        let _rightOperator = '';
        if (helper.removeQuote(_condition.right) === 'true' || helper.removeQuote(_condition.right) === 'false') {
          _rightOperator = (helper.removeQuote(_condition.right) === 'true') ? true : false;
        }
        else {
          _rightOperator = getSafeValue(helper.removeQuote(_condition.right));
        }
        _str += getSafeVar(_objName)+'['+getSafeValue(_attr)+']'+_condition.operator+_rightOperator;
      }
