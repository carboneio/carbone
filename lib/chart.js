...
const stateMachineChartContentLO = {
  ...
  inDrawObject : {
    ' xlink:href="' : (ctx, posEndOfLastTag, pos, posEnd) => {
      ...
      if (!_chartObj) {
        return 'main'; // TODO test it
      }
      const _endOfDrawObjectXML  = ctx.xmlIn.indexOf('/>', posEnd); // TODO + 2 ?????
...
const stateMachineContentDocx = {
  main : {
    '<w:drawing' : () => 'inDrawing'
  },
  inDrawing : {
    '<wp:docPr' : () => 'inDocPr',
    '<c:chart'  : (ctx, posEndOfLastTag, pos, posEnd) => {
      ctx.xmlOut += ctx.xmlIn.slice(ctx.lastPos, pos);
      ctx.lastPos = pos;
      return 'inChart';
    },
...
const chart = {
  ...
  generateDocxRowCountFormatter : function (markers) {
    ...
    let _firstMarkerAgg = _secondMarker;
    for (var i = 0; i < _iterators.length; i++) {
      let _iterator = _iterators[i];
      _firstMarkerAgg = _firstMarkerAgg.replace(_iterator[0], '');
    }
    return {
  ...
  preProcessLo : function (template, options) {
    ...
    for (var i = 0; i < template.files.length; i++) {
      ...
      // TODO improve filtering only in chart dorectory
  ...
  /**
   * Finishes a docx chart.
   *
   * Update counter and indexes of the chart
   *
   * @param   {string}  chartXML  The chart xml
   * @return  {string}  XML updated, witht right counter and index in XML
   */
  finishDocxChart : function (chartXML) {
    let _index = -1;
    let _rowCounter = 0;
    let _newXML = '';
    let _prevXMLPos = 0;
   ...
   *
   * A chart is linked to manby other files in a DOCX
   *
   * @param      {Object}  template                                 The template
  ...
  getDocxRelatedFiles : function (template, oldChartPath = 'word/charts/chart1.xml') {
    ...
    // chart
    _relatedFiles.push({ nameFn : (newChartName) => `${_chartDir}/${newChartName}.xml`, dataFn : (newXML) => chart.finishDocxChart(newXML)  });
  ...
  /**
   * Removes markers in embedded spreasheet, which arelinked to a chart
   *
   * We update only the chart, and MS Word will generate the XLSX file himself from the chart
  ...
  preProcessDocx : function (template, options) {
    ...
    for (var i = 0; i < template.files.length; i++) {
      ...
      let _ctx = {
        ...
        xlsxCells           : template.embeddedXlsxCellValues[_related.xlsx], // cell content TODO TODO TODO
    ...
    const _ctxContent = chart._executeStateMachine(_mainDocumentFile, {}, stateMachineContentDocx, options);
    if (_ctxContent.xmlOut !== '' && _ctxContent.isFullyExecuted === true) {
