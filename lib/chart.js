...
const debug = require('debug')('carbone:chart');
const path = require('path');

const chartRegexODT = /<chart:chart[^<>]*?>[^]*?<\/chart:chart>/g;
    ...
    for (let i = 0, j = template.files.length; i < j; i++) {
      ...
      if (_file.name === 'content.xml') {
        ...
        while ( (_index = _xml.indexOf('<CARBONE_FILE', ++_index)) !== -1) {
          ...
          const _newChartName = _chartId + _globalChartId++;
          for (let j = 0; j < _chart.otherFiles.length; j++) {
            let _newFile = _chart.otherFiles[j];
            if (_newFile.nameFn !== undefined) {
              console.log('\n\n\ngenerate:', JSON.stringify({ name : _newFile.nameFn(_newChartName), data : _newFile.dataFn(_chartXML)}, null, 2));
              template.files.push({
                name   : _newFile.nameFn(_newChartName),
                data   : _newFile.dataFn(_chartXML),
                parent : ''
              });
            }
...
function removeMarkers (xml) {
  return xml.replace(/\{([^{]+?)\}/g, function (m, marker, offset) {
