...
const color = {
  ...
  postProcessODT : function (template, data, options) {
    ...
    while (_item.done === false) {
      ...
      const _styleFamily = _originalStyleTag.styleFamily; // the familly of the tag ['paragraph', 'text', 'shape']
      ...
      for (let i = 0, j = _item.value[1].colors.length; i < j; i++) {
        ...
        const _colorsProperties = color.getColorTagPropertiesOdt(_colors.oldColor, _originalStyleTag.colors);
        ...
        console.log(_newColor, _colorsProperties.element, _colorsProperties.colorType);
  ...
  getColorTagPropertiesOdt : function (oldColor, originalColorsProperties) {
    console.log('getColorTagPropertiesOdt', oldColor, originalColorsProperties);
  ...
  preProcessDocx : function (template) {
    ...
    if (_file === null) {
      ...
      // throw new Error('the "word/document.xml" file does not exist.');
    }
    color.getBindColorMarkers(_file.data, function (
      err,
