...
const color = {
    ...
    while (_item.done === false) {
      ...
      for (let i = 0, j = _item.value[1].colors.length; i < j; i++) {
        ...
        if (_colors.colorType) {
          _newColor = color.colorFormatConverter[_colors.colorType](_newColor, 'odt');
        }
  ...
  preProcessDOCX : function (template) {
    ...
    // _file = file.getTemplateFile(template, 'document.xml');
    if (_file === null) {
    ...
    color.getBindColorMarkers(_file.data, function (
      ...
      if (!!err === true) {
        console.error('Carbone bindColor error: the bindColor markers are incorrect: ' + err);
        return;
      }
      // stop immediately if no bindColor marker has been found
      if (bindColorList.length === 0) {
          ...
          xml = xml.replace(_regexColor, function (xmlTag, tagType) {
            ...
            if (tagType === 'highlight') {
              ...
              if (bindColorList[i].colorType !== 'color') {
                console.warn(`Carbone bindColor warning: the background color on DOCX documents can only be changed with the color name format, use the color format "color" instead of "${bindColorList[i].colorType}".`);
              }
            }
            console.log(`<w:${tagType} w:val="{${bindColorList[i].marker}:getAndConvertColorDocx(${bindColorList[i].colorType}, ${_elementType})}"/>`);
    ...
    color : (colorString, reportFormat, elementType) => {
      ...
      if (reportFormat && reportFormat === 'docx' &&
        ...
        if (_colorList[colorString]) {
          ...
          console.warn(`Carbone bindColor warning: the color '${colorString}' does not exist for background color on DOCX report. Do you mean "${_alternativeColorName}" ?`);
          return _colorList.transparent;
        }
      }
