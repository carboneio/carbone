  ...
  preProcessDocxColor : function (template) {
    ...
    for (let f = 0, len = template.files.length; f < len; f++) {
      ...
      _file.data = _file.data.replace(_patternColor, function (tag, colorFormatterOnly, index) {
        ...
        _offset += tag.length;
        const _tagWithCustomFormatter = tag.replace(/}$/g, _docxFormatter);
        ...
        if (_scopeObj === undefined) {
          throw new Error(`The color scope "${_scope}" is unknown in "${tag}". The formatter accepts only ${Object.keys(_scopeMap).join(', ')} for the first argument.`);
        }
      ...
      for (let i = 0; i < _colorTags.length; i++) {
        ...
        if (_colorTag.scopeStart === -1 || _colorTag.scopeEnd < _colorTag.pos) {
          throw new Error(`The Carbone tag "${_colorTag.src.replace(_docxFormatter, '}')}" is outside of its defined scope "(${_colorTag.scope})"`);
        }
        // apply the color
