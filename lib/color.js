  ...
  preProcessDocx : function (template) {
    ...
    for (var k = 0, l = template.files.length; k < l; k++) {
      ...
      if (_templateName.indexOf('.xml') !== -1 &&
        ...
        // Find colors in cells properties to generate color marker
        console.log(_bindColorList);
        _file.data = _file.data.replace(/<w:tcPr>[^]*?<\/w:tcPr>/g, function (xml) {
          console.log(xml);
          for (let i = 0, j = _bindColorList.length; i < j; i++) {
            const _refColor = _bindColorList[i].referenceColor;
            const _regexColor = new RegExp(`w:(fill|color)="(${_refColor}|${_refColor.toUpperCase()}|${_refColor.toLowerCase()})"`, 'g');
            xml = xml.replace(_regexColor, function (xmlAttribute, tagType) {
