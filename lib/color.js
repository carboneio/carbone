...
const applyBackgroundColorInCellsOffice = {
  main : {
    '<w:tcPr' : (ctx) => {
      ctx.isColorApplied = false; // reset for each cell
      return 'inCellStyle';
    }
  },
  inCellStyle : {
...
const color = {
  ...
  preProcessDocxColor : function (template) {
    ...
    for (let f = 0, len = template.files.length; f < len; f++) {
      ...
      _file.data = _file.data.replace(_patternColor, function (tag, colorFormatterOnly, index) {
        if (parser.isCarboneMarker(tag) === false) {
          return tag;
        }
        _offset += tag.length;
        const _tagWithCustomFormatter = tag.replace(/}$/g, ':colorToDocx}');
        const _parsedFormatter = extracter.parseFormatter(colorFormatterOnly);
        const _scope    = _parsedFormatter.args[0] ?? 'p';
        const _scopeObj = _scopeMap[_scope];
        const _type     = _parsedFormatter.args[1] ?? 'text';
        if (_scopeObj === undefined) {
          throw new Error(`Cannot find where the color is applied in "${tag}". The formatter accepts only ${Object.keys(_scopeMap).join(', ')} for the first argument.`);
        }
        if (_typeMap[_type] === undefined) {
          throw new Error(`The color type "${_type}" is unknown in "${tag}". The formatter accepts only ${Object.keys(_typeMap).join(', ')} for the second argument.`);
        }
        if (_scopeObj.forbiddenTypes.indexOf(_type) !== -1) {
          throw new Error(`The color type "${_type}" is not allowed for the scope "${_scope}" in "${tag}".`);
        }
      ...
      for (let i = 0; i < _colorTags.length; i++) {
        ...
        _colorTag.scopeStart = Math.max(_file.data.lastIndexOf( _colorTag.scopeObj.start+' ', _colorTag.pos), _file.data.lastIndexOf( _colorTag.scopeObj.start+'>', _colorTag.pos));
        _colorTag.scopeEnd = _file.data.indexOf( _colorTag.scopeObj.end, _colorTag.scopeStart); // reach </p>
        if (_colorTag.scopeStart === -1 || _colorTag.scopeEnd < _colorTag.pos) {
          throw new Error(`The Carbone tag "${_colorTag.src}" is outside of its defined scope "(${_colorTag.scope})"`);
        }
        // apply the color
        executeColorStateMachine(_file, _typeMap[_colorTag.type], _colorTag, _xmlPatches);
      }
      ...
      let _newXML = [];
      let _prevPatchFrom = 0;
      let _currentXMLPos = 0;
      for (let i = 0; i < _xmlPatches.length; i++) {
        const _xmlPatch = _xmlPatches[i];
        if (_xmlPatch.from === _prevPatchFrom) {
          continue;
        }
        _newXML.push(_file.data.slice(_currentXMLPos, _xmlPatch.from));
        _newXML.push(_xmlPatch.add);
        _currentXMLPos = _xmlPatch.to;
        _prevPatchFrom = _xmlPatch.from;
      }
      _file.data = _newXML.join('') + _file.data.slice(_currentXMLPos);
 ...
 *
 * Do nothing if the color is alrady applied
 *
 ...
 */
function replaceXMLAttributeAndUpdateXMLOutput (ctx, posEndOfLastTag, pos, posEnd, newAttribute, priority) {
  if (ctx.isColorApplied === true) {
    return;
  }
  ctx.isColorApplied = true;
  const _endOfAttributeValue = ctx.xmlIn.indexOf('"', posEnd);
  ctx.xmlPatches.push({ from : posEnd, to : _endOfAttributeValue, add : newAttribute, priority });
}
 ...
 */
function addXMLBeforeAndUpdateXMLOutput (ctx, posEndOfLastTag, pos, posEnd, injectedXMLBeforeTheTag, priority) {
  if (ctx.isColorApplied === true) {
    return;
  }
  ctx.isColorApplied = true;
  ctx.xmlPatches.push({ from : pos, to : pos, add : injectedXMLBeforeTheTag, priority });
}
