...
const applyTextColorOffice = {
  ...
  inRunStyle : {
    '</w:rPr' : (...args) => {
      ...
      addXMLBeforeAndUpdateXMLOutput(...args, `<w:color w:val="${_tag.src}"/>`,  _tag.scopeObj.priority); // (low priority ) add color on sub paragraph if it does not exist
      return 'main';
    },
  ...
  inColorValue : {
    ' w:val="' : (...args) => {
      const _tag = args[0].colorTag;
      replaceXMLAttributeAndUpdateXMLOutput(...args, _tag.src,  _tag.scopeObj.priority); // (highest priority) patch existing color
      return 'inRunStyle';
    }
  }
};
...
const applyBackgroundColorInCellsOffice = {
  ...
  inCellStyle : {
    '</w:tcPr' : (...args) => {
      const _tag = args[0].colorTag;
      addXMLBeforeAndUpdateXMLOutput(...args, `<w:shd w:val="clear" w:fill="${_tag.src}"/>`,  _tag.scopeObj.priority); // (low priority ) add color on the cell if it does not exist
      return 'main';
    },
  ...
  inBackroundColor : {
    ...
    ' w:themeFill="' : (...args) => {
      const _tag = args[0].colorTag;
      removeXMLAttribute(...args, _tag.scopeObj.priority);
      return 'inBackroundColor';
    }
  }
};
...
const applyTextColorHTML = (type = 'text', scope = 'row') => {
  ...
  return {
    ...
    inHTMLRowOrCellOrP : {
      ' style="' : (...args) => {
        const _tag = args[0].colorTag;
        // #PRIORITY_HTML For HTML, the priority is used to accept multiple css properties in the same style attribute (append = true && priority !== previousOfPreviousPatch)
        // The length of the css property is used to make a difference. If more properties are managed, we should use another method
        replaceHtmlStyleAttributePatch(...args, _tag.src, _cssProperty.length, _cssProperty, true);
        return 'inHTMLRowOrCellOrP';
      },
      '>' : (ctx, posEndOfLastTag, pos, posEnd) => {
        ...
        ctx.xmlPatches.push({ from : pos, to : pos, add : 'style="', priority : Number.MAX_SAFE_INTEGER });
        // #PRIORITY_HTML same doc
        addXMLBeforeAndUpdateXMLOutput(ctx, posEndOfLastTag, pos, posEnd, `${_cssProperty}:${_tag.src};`, _cssProperty.length, true);
        ctx.xmlPatches.push({ from : pos, to : pos, add : '"', priority : Number.MIN_SAFE_INTEGER, append : true });
        return 'main';
      }
    },
  };
...
const color = {
  ...
  preProcessDocxColor : function (template) {
    ...
    const _scopeMap = {
      row  : { start : '<w:tr', end : '</w:tr>', forbiddenTypes : []             , priority : 1            },
      cell : { start : '<w:tc', end : '</w:tc>', forbiddenTypes : []             , priority : 2            },
      p    : { start : '<w:p' , end : '</w:p>' , forbiddenTypes : ['background'] , priority : 3 /* high */ }
    };
    const _typeMap = {
  ...
  preProcessHtmlColor : function (template) {
    const _docxFormatter = 'colorToHtml';
    const _scopeMap = {
      row  : { start : '<tr'  , end : '</tr>' , forbiddenTypes : [] , priority : 1  }, /* this priority are not used because we rely on CSS for propagation */
      cell : { start : '<td'  , end : '</td>' , forbiddenTypes : [] , priority : 2  }, /* we use the priority system to solve another problem, search for #PRIORITY_HTML */
      p    : { start : '<p'   , end : '</p>'  , forbiddenTypes : [] , priority : 3  }
    };
    const _typeMap = {
 ...
 * @param      {Object}  scopeMap                 The scope map with priority to resolve conflict (two colors at the same place). Ex:
 *                                                {
 *                                                  row  : { start : '<w:tr', end : '</w:tr>', forbiddenTypes : [], priority : 1 },
 *                                                  cell : { start : '<w:tc', end : '</w:tc>', forbiddenTypes : [], priority : 2 }
 *                                                }
 ...
 */
function preProcessColor (file, scopeMap, typeMap, colorConverterFormatter) {
  const _colorConverterFormatter = ':' + colorConverterFormatter + '}';
  ...
  for (let i = 0; i < _colorTags.length; i++) {
    ...
    _colorTag.scopeEnd = file.data.indexOf( _colorTag.scopeObj.end, _colorTag.scopeStart); // reach </p>
    ...
    executeColorStateMachine(file, _patchGenerator, _colorTag, _xmlPatches);
  }
  ...
  _xmlPatches.sort((a, b) => (a.from === b.from) ? b.priority - a.priority : a.from - b.from);
  let _newXML = [];
  let _prevPatch = {};
  let _currentXMLPos = 0;
  for (let i = 0; i < _xmlPatches.length; i++) {
    const _xmlPatch = _xmlPatches[i];
    if (_xmlPatch.from === _prevPatch.from && (_xmlPatch.append !== true || _xmlPatch.priority === _prevPatch.priority )) {
      continue;
    }
    _newXML.push(file.data.slice(_currentXMLPos, _xmlPatch.from));
    _newXML.push(_xmlPatch.add);
    _currentXMLPos = _xmlPatch.to;
    _prevPatch = _xmlPatch;
  }
  file.data = _newXML.join('') + file.data.slice(_currentXMLPos);
}
 ...
 * @return  {Object}
 */
function executeColorStateMachine  (file, stateMachine, colorTag, xmlPatches) {
  let _ctx = {
    xmlIn          : file.data,
 ...
 * @param      {Integer}  priority         the priority of the patch when it is inserted at the same position in XML
 * @param      {Boolean}  append           Accept to append the patch with the previous one, if it is inserted at the same position in XML, and the priority is different
 * @return     {String}   The value read in XML attribute and concatenate.
 */
function replaceXMLAttributeAndUpdateXMLOutput (ctx, posEndOfLastTag, pos, posEnd, newAttribute, priority, append = false) {
  if (ctx.isColorApplied === true) {
    return;
  }
  ctx.isColorApplied = true;
  const _endOfAttributeValue = ctx.xmlIn.indexOf('"', posEnd);
  ctx.xmlPatches.push({ from : posEnd, to : _endOfAttributeValue, add : newAttribute, priority, append });
}
 ...
 * @return     {String}   The value read in XML attribute and concatenate.
 */
function replaceHtmlStyleAttributePatch (ctx, posEndOfLastTag, pos, posEnd, newCssValue, priority, cssPropertyToPatch, append = false) {
  if (ctx.isColorApplied === true) {
    return;
  }
  const _patch = html.patchStyleAttribute(ctx.xmlIn, posEnd, cssPropertyToPatch, newCssValue);
  if (_patch) {
    _patch.priority = priority;
    _patch.append = append;
    ctx.isColorApplied = true;
 ...
 */
function addXMLBeforeAndUpdateXMLOutput (ctx, posEndOfLastTag, pos, posEnd, injectedXMLBeforeTheTag, priority, append = false) {
  if (ctx.isColorApplied === true) {
    return;
  }
  ctx.isColorApplied = true;
  ctx.xmlPatches.push({ from : pos, to : pos, add : injectedXMLBeforeTheTag, priority, append });
}


function removeXMLAttribute (ctx, posEndOfLastTag, pos, posEnd, priority) {
  const _endOfAttributeValue = ctx.xmlIn.indexOf('"', posEnd) + 1;
  ctx.xmlPatches.push({ from : pos, to : _endOfAttributeValue, add : '', priority });
}
