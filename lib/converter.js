    ...
    for (var i in conversionFactory) {
      ...
      if (_factory && (_factory.pythonThread !== null || _factory.officeThread !== null)) {
        ...
        if (_factory.pythonThread !== null) {
          _factory.pythonThread.kill(); // LibreOffice process will die automatically 
        }
        else if (_factory.officeThread !== null) {
          _factory.officeThread.kill(); // Thread process will die automatically 
        }
      }
    }
...
function generateOnExitCallback (factoryID, isPythonProcess, factoryUniqueName) {
  return function (error) {
    ...
    console.log('Carbone: Process '+_processName+' of factory '+factoryID+' died ' + error);
      
    // if both processes Python and Office are off...
    if (_factory.pythonThread === null && _factory.officeThread === null) {
      console.log('Carbone: Factory '+factoryID+' is completely off');
      ...
      if (_job) {
        ...
        if (_job.returnLink===true || _job.error !== null) {
          if (_job.error && _job.nbAttempt < params.attempts) {
            console.log(_job.error + ': attempt '+_job.nbAttempt, _job);
            _job.error = null;
            jobQueue.push(_job);
          }
          else {
          ...
          fs.readFile(_job.outputFilePath, function (err, content) {
            ...
            try {
              ...
              console.log('Cannot remove file' + _job.outputFilePath);
            }
            executeQueue();
          });
        }
...
function detectLibreOffice () {
  ...
  if (process.platform === 'darwin') {
    ...
    if (isLibreOfficeFound === false) {
      console.log(_redColor+'-- CarboneJS: cannot find LibreOffice. Document conversion cannot be used.'+_resetColor);
    }
  }
  else {
