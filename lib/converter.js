...
/*  store the timer object ID, to clear at the end of the process. */
var factoriesTimeoutIds = [];
...
var converter = {
  ...
  exit : function (callback) {
    ...
    // Clear all factory timeout
    clearAllFactoriesTimeOut();
...
function generateOnDataCallback (factoryID) {
  return function (data) {
    ...
    if (data !== '204') { // Document converted
      ...
      _factory.isConverting = false;
      clearSingleFactoryTimeOut(_factory.timeoutId);
      _factory.timeoutId = 0;
      if (_job) {
        ...
        if (_job.returnLink===true || _job.error !== null) {
          ...
          fs.readFile(_job.outputFilePath, function (err, content) {
            ...
            if (params.factoryMemoryCheck === true) {
              ...
              _factory.nbrReports++;
              const _percentageFactoryMemoryLoaded = _factory.nbrReports * params.factoryMemoryFileSize * 100 / totatMemoryAvailableMB;
              if (_percentageFactoryMemoryLoaded > params.factoryMemoryThreshold && _factory.officeThread !== null) {
                _factory.officeThread.kill();
                _factory.nbrReports = 0;
              }
            }
            executeQueue();
          });
...
function sendToFactory (factory, job) {
  ...
  if (params.converterFactoryTimeout > 0) {
    factory.timeoutId = setTimeout(function () {
      factory.currentJob.callback('Carbone Converter timeout error: the process is taking too long.', null);
      if (factory.officeThread !== null) {
        factory.officeThread.kill();
      }
    }, params.converterFactoryTimeout);
    factoriesTimeoutIds.push(factory.timeoutId);
  }
}

function clearSingleFactoryTimeOut (timeoutId) {
  if (params.converterFactoryTimeout > 0 && timeoutId) {
    clearTimeout(timeoutId);
  }
}

function clearAllFactoriesTimeOut () {
  if (params.converterFactoryTimeout > 0) {
    for (let i = 0, n = factoriesTimeoutIds.length; i < n; i++) {
      clearTimeout(factoriesTimeoutIds[i]);
    }
    factoriesTimeoutIds = [];
  }
}
