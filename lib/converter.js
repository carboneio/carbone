...
function sendToOffice(listener, job, callback){
  ...
  _python.on('exit', function(error){
    if(error){
      console.log('\n\n\n Warning, error during conversion on document'+job.inputFilePath+' \n\n');
    }
 ...
 * TODO: try many path in a loop until we find LibreOffice and Python
 */
function detectLibreOffice(){
  ...
  if(process.platform === 'darwin'){
    ...
    converterOptions.pythonExecPath = 'python';
    converterOptions.sofficeExecPath = 'soffice';
  }
  else{
