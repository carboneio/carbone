...
var converterOptions = {
  ...
  /*'portStart' : 2000,*/
  /* Number of LibreOffice listeners to start by default */
  'nbListeners' : 1,
...
function startListener(listenerID, callback){
  ...
  setTimeout(function(){
    //test if the listener object still exists (protection in the case of converter.exit method is called while a listener is restarting)
...
function detectLibreOffice(){
  ...
  exec(converterOptions.sofficeExecPath + ' --version', function(error, stdout, stderr){
    if(error){
      console.log(_redColor+'-- CarboneJS: LibreOffice executable not found or LibreOffice cannot be executed: '+error+_resetColor);
    }
  });
}
