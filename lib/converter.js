...
var converter = {
   ...
   */
  convertFile : function (inputFile, outputType, formatOptions, callback, returnLink=false, outputFile=false, exportFilters={}) {
...
function generateOnExitCallback (factoryID, isPythonProcess, factoryUniqueName) {
  return function (error) {
    ...
    if (_factory.pythonThread === null && _factory.officeThread === null) {
      ...
      _otherThreadToKill.kill('SIGTERM');
    }
  };
}
...
function onCurrentJobEnd (factory) {
  ...
  if (_job && _job.callback instanceof Function) {
    ...
    if (_job.error) {
      ...
      // temporay removed. Do we need to retry?
    ...
    else if (_job.returnLink===true ) {
      _job.callback(null, _job.outputFilePath);
    }
    else {
      fs.readFile(_job.outputFilePath, function (err, content) {
        _job.callback(null, content);
