...
function generateOnExitCallback (factoryID, isPythonProcess, factoryUniqueName) {
  return function (error) {
    ...
    if (_factory.pythonThread === null && _factory.officeThread === null) {
      ...
      _otherThreadToKill.kill(); // send SIGTERM by default
      // Should we send a SIGKILL after a certain amount of time?
      // Be careful, LibreOffice has two threads oosplash (parent) -> soffice (child).
      //
      // On Linux, if SIGKILL is sent, only oosplash is killed.
      // The child is still alive and stdin, stdout and stderr are not closed automatically.
      // The event "spawn.close" is received only if stdin, stdout and stderr are closed.
      // So with SIGKILL, we must close  stdin, stdout and stderr ourself and kill the child thread soffice ourself.
      //
      // Can we avoid the thread oosplash?. Some say it is possible but is does not work our side.
      // https://bbs.archlinux.org/viewtopic.php?id=120405
      // https://support.icewarp.com/hc/en-us/articles/115002557287-soffice-bin-and-oosplash-processes-converting-documents-to-pdf-for-preview-problem-WebClient-hangs-with-500      // 
    }
  };
}
