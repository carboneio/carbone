...
var dynpics = {
  ...
  manageOdt : function (template, callback) {
    ...
    this.getOdtPictures(contentFile.data, function (pictures) {
      that.replaceOdtPicture(template, pictures, function () {
        return callback(null, template);
      });
    });
	},
	...
	_sizeOfInEmu : function (image) {
		try {
			var _dimensionsPx = sizeOf(image);
		} catch (e) {
			return null;
		}
		var _dimensionsEmu = {
  ...
  _updateDocxPictureDimensionsInXml : function (xml, relation, picture) {
    ...
    // console.log(_currentDimensions)
    // console.log(_pictureDimensions)
    // console.log(_resizedDimensions)
