...
var extracter = {
  ...
  splitMarkers : function (markers) {
    ...
    for (var i = 0; i < markers.length; i++) {
      ...
      var _isConditionUsedWithoutIterator = false;
      // Loop on each char.
      ...
      for (var c = 0; c < _marker.length; c++) {
        ...
        if (_char===']' && _isArray===true) {
          ...
          if (_iterators.length === 0) {
            var _testivo = false;
            if (_word === '' && _conditions.length > 0) {
              _testivo = true;
              _isConditionUsedWithoutIterator = true;
            }
            ...
            if(_isIteratorUsedInCurrentArray === false) {
              ...
              for (var k = 0; k < _conditions.length; k++) {
                var _condition = _conditions[k];
                var _filterSuffix = helper.cleanJavascriptVariable(_condition.left.attr + '__' + _condition.right);
                // correction of previously detected conditions
                if (_condition.left.parent === _uniqueMarkerName) {
                  _condition.left.parent += _filterSuffix;
                }
                _strCon += _filterSuffix; 
              }
              _uniqueMarkerName += _strCon;
            }
      ...
      if (_iterators.length === 0) {
        ...
        if (_conditions.length > 0) {
          ...
          _xmlPart.isConditionUsedWithoutIterator = !_isIteratorUsedInCurrentArray;
        }
...
function findAndSetExactPositionOfArrays (xml, descriptor) {
  ...
  for (var _objName in descriptor) {
    ...
    if (_obj.type === 'array' && _obj.iterators.length>0) {
      ...
      console.log('\n\n\n');
      console.log(_objName, _roughPosStart, _roughPosEnd);
      console.log('\n\n\n');
