...
var extracter = {
            ...
            if(_res[_uniqueMarkerName].position.end === undefined){
              ...
              for (var it = 0; it < _iterators.length; it++) {
                ...
                if(/\./.test(_iterators)){
                  ...
                  _res[_uniqueMarkerName].iterators.push({'obj':_splitIterator[0],'attr' : _splitIterator[1]});
                }
                else{
                  _res[_uniqueMarkerName].iterators.push({'attr' : _iterator});
                }
              };
            }
          }
  ...
  buildSortedHierarchy : function(descriptor){
    ...
    _sortedObj.sort(function(a,b){ 
      //the root "_root" is always the first
      if(a.parent === ''){
        return -1;
      }
      if(b.parent === ''){
      ...
      if(_levelA < _levelB && a.type === 'object'){
        return -1;
      }
      if(_levelA > _levelB && b.type === 'object'){
        return 1;
      }
