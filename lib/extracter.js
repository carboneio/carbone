...
var extracter = {
  ...
  splitXml : function (xml, descriptor, markers, options) {
    ...
    findAndSetExactPositionOfArrays_v5(xml, descriptor, markers);
...
function findAndSetValidPositionOfConditionalBlocks (xml, descriptor, markers) {
  ...
  var _xmlFlattened = parser.flattenXML_v5(xml);
  for (var _objName in descriptor) {
    ...
    for (var i = 0; i < _xmlParts.length; i++) {
      ...
      if (_lastFormatter.indexOf('showBegin') !== -1 || _lastFormatter.indexOf('hideBegin') !== -1) { // TODO return error if not last
        ...
        var _safeXMLBlocks = parser.findSafeConditionalBlockPosition_v5(_xmlFlattened, _beginPart.pos, _part.pos);
 ...
 * ]
 */
function findAndSetExactPositionOfArrays_v5 (xml, descriptor, markers) {
  var _oddZones = [];
  var _xmlFlattened = parser.flattenXML_v5(xml);
  for (var _objName in descriptor) {
    ...
    if (_obj.type === 'array' && _obj.iterators.length>0) {
      ...
      //var _realArrayPosition = xml.slice(Math.trunc(_roughPosStart), Math.trunc(_roughPosEnd));
      // var _pivot = parser.findPivot(_subString);
      var _realArrayPosition = parser.findPivot_v5(_xmlFlattened, Math.trunc(_roughPosStart), Math.trunc(_roughPosEnd));
      // TODO  // TODO // TODO
      //if (!_pivot) {
