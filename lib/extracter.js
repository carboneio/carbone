...
var extracter = {
  ...
  removeLastTableGridDocx : function (str) {
    ...
    let _tableGridLastIndex = str?.lastIndexOf('<w:tblGrid>') ?? -1;
    ...
    if (_tableGridLastIndex !== -1) {
      ...
      const _tblGridChangeStart = str.lastIndexOf('<w:tblGridChange');
      const _tblGridChangeEnd = str.lastIndexOf('</w:tblGridChange>');
      if (_tblGridChangeStart > -1 && _tableGridLastIndex > _tblGridChangeStart && _tableGridLastIndex < _tblGridChangeEnd && _tableGridLastIndexEnd > _tblGridChangeEnd) {
        // manage nested '<w:tblGrid>' when the file is coming from Google docs (old Office file format)
        _tableGridLastIndex = str.lastIndexOf('<w:tblGrid>', _tblGridChangeStart);
      }
  ...
  parseFormatter : function (formatterString = '') {
    ...
    for (let i = 0; i < _arguments.length; i++) {
      ...
      if (_argument[0] === "'") {
        // remove it and remove single quote escape '' (-> same sytem as PostgreSQL. A lot simpler to manager than \')
        _argument = _argument.slice(1).replace(/''/g, "'");
