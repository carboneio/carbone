...
var extracter = {
  ...
  splitXml : function (xml, descriptor, markers, options) {
    ...
    if (_horizontalRepetitionPart.length > 0) {
      ...
      for (let k = _horizontalRepetitionPart.pop(); k >= 0; k--) {
        const _part = _allSortedParts[k];
        if (k <= _horizontalRepetitionPart[_horizontalRepetitionPart.length - 1]) {
          _horizontalRepetitionPart.pop();
        }
        ...
        if (_removedTableGridText.isFound === true) {
          k = _horizontalRepetitionPart.pop() ?? -1;
          continue;
        }
