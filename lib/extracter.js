...
var extracter = {
        ...
        if (_char===']' && _isArray===true) {
          ...
          // If it was the second part of the array ("i+1")
            ...
            if (_res[_uniqueMarkerName] === undefined) {
              throw Error(`The marker ${helper.printMarker(markers, i)} has no corresponding "[i]" marker before. Please add markers to describe the i-th section of the repetition.`);
            }
   ...
   */
  splitXml : function (xml, descriptor, markers) {
      ...
      for (var i = 0; i < _xmlParts.length; i++) {
        ...
        if (_part.moveTo !== undefined) {
          ...
          if (descriptor.dynamicData[_objName].type !== 'objectInArray') {
            // if (descriptor.dynamicData[_objName].xmlPartForAgreggator === undefined) {
            //   descriptor.dynamicData[_objName].xmlPartForAgreggator = [];
            // }
            // descriptor.dynamicData[_objName].xmlPartForAgreggator.push(_part);
...
function findAndSetExactPositionOfArrays (xml, descriptor, markers) {
  ...
  for (var _objName in descriptor) {
    ...
    if (_obj.type === 'array' && _obj.iterators.length>0) {
      ...
      // TODO test avec un bind color qui rajoute un [i] dans la section répété d'un i+1. Cas pourri
      if (!_pivot) {
