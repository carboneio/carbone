...
var extracter = {
  ...
  splitXml : function (xml, descriptor, markers, options) {
    ...
    let _isContainingHorizontalLoopDetected = false;
    ...
    for (var i = 0; i < _allSortedParts.length; i++) {
      ...
      if (_prevPart.array === 'start') {
        ...
        // detect horizontal loop in docx to remove `<w:tblGrid> </w:tblGrid>` later (maybe we should remove it for all templates?)
        if (options?.extension === 'docx' && _isContainingHorizontalLoopDetected === false && /^[\s]*<w:tc>/.test(_prevPart.after) === true && xml.indexOf('<w:document ') !== -1) {
          _res.isContainingHorizontalLoop = true;
        }
