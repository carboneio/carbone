...
const file = {
  ...
  fetchFileDatabase : function (options, callback) {
    ...
    function executeItem (item, next) {
      ...
      const _fetchFileTime = options.metrics.time('fetchFileTime');
      egress.downloadExternalFile(_url, ACCEPTED_MIMETYPE_EXTERNAL_FILES, ACCEPTED_EXTENSION_FILES, options, (err, file) => {
        _fetchFileTime.timeEnd();
        options.metrics.add('fetchFileBytes', file?.data?.length ?? 0);
        _obj.data = file?.data;
  ...
  detectType : function (template) {
    ...
    if (this._checkWordInFilename(template, 'visio/')) {
      return 'vsdx';
    }
