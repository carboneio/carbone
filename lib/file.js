...
var file = {
  ...
  isZipped: function (filePath, callback) {
    ...
    if (Buffer.isBuffer(filePath)) {
      const buffer = Buffer.from(filePath, 0, 10).toString();
  ...
  openTemplate : function (templateId, callback) {
    ...
    var _templateFile;
    if (!Buffer.isBuffer(templateId)) _templateFile = path.resolve(params.templatePath, templateId); else _templateFile = templateId;
