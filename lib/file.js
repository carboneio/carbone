...
const unzipEmbeddedFileTypes = ['.xlsx', '.ods'];
const yazl = require('yazl');
const debug = require('debug')('carbone');
...
let pdfLib;
(async () => {
  // Is it the best way to laod ESM module in a CJS file?
...
const file = {
   ...
   *
   * It download images and updates options.fileDatabase
   *
  ...
  fetchFileDatabase : function (options, callback) {
    ...
    if ((options.fileDatabase?.size ?? 0) === 0) {
    ...
    const _items = [...options.fileDatabase];
