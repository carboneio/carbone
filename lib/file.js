...
let pdfLib;
(async () => {
  // Is it the best way to load ESM module in a CJS file?
...
const file = {
  ...
  fetchFileDatabase : function (options, callback) {
    ...
    if (options?.fileDatabase === undefined || options.fileDatabase.previousSize === options.fileDatabase.size || options.fileDatabase.size === 0) {
      // TODO test previousSize !== size (it happens because buildXML can be called for headers/footers and it should not )
