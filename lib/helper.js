...
var helper = {
    ...
    return {
      ...
      processNextItem : function (err) {
        ...
        handlerItem.call(
          this,
          this.currentItem,
          this.processNextItem.bind(this)
        );
      },
   ...
   * 3 - replace each directory in the current string with "."
   * 4 - add that (with a path-separator) in front of the target string
   * 5 - return that combined string
   *
   * ✔️  d.list[i].color / d.color2                 = ..color2
   * ✔️  d.list[i].color / d.list[i].color2         = .color2
   * ✔️  d.list[i].list[i].color / d.list[i].color2 = ..color2
   * X  d.list[i].color / d.list[i].list[i].color2 = .list[i].color2
   *
   *
   * @param {String} from d.list[i].color
   * @param {Srting} to d.list[i].color2
   * @return {String} .color2
   */
  findMarkerRelativePath : function (from, to) {
    let _similarPath = [];
    ...
    let _diff = false;
    _splitTo.forEach((node, $index) => {
      if (_diff === false && $index < _splitFrom.length && node === _splitFrom[$index]) {
        _similarPath.push(node);
      }
      else {
        _diff = true;
        _diffMarkerTo.push(node);
      }
    });

    _diff = false;
    _splitFrom.forEach((node, $index) => {
      if (_diff === false && $index < _splitTo.length && node === _splitTo[$index]) {
        // _similarPath.push(node);
      }
      else {
        _diff = true;
        _diffMarkerFrom.push(node);
      }
    });

    let _finalMaker = '';
    // console.log(_splitFrom, _splitFrom.length, _splitTo, _splitTo.length);
    // console.log(_similarPath, _similarPath.length, _markerObjects, _markerObjects.length);
    // console.log(_diffMarkerFrom, _diffMarkerTo)

    for (let i = 0, j = _diffMarkerFrom.length - 1; i < j; i++) {
      _finalMaker += '.';
    }

    _diffMarkerTo.forEach(x => {
      _finalMaker += '.' + x;
    });
    return _finalMaker;
  },
