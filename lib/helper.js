...
var helper = {
  ...
  */
  ...
  printVersion : function (license) {
    ...
    _text.push('- Proprietary software made by Ideoys SAS - France - 521 295 162 00023 RCS');
   ...
   */
  findCheckLicense : function (licenseDir, publicKey, callback, publicationDate = package.carbonePublicationDate) {
    fs.readdir(licenseDir, { withFileTypes : true }, (err, files) => {
      if (err) {
        return callback(false, 'Cannot find *.carbone-license files in '+licenseDir);
      }
      let _mostRecentBirthtime = 0;
      ...
      for (var i = 0; i < files.length; i++) {
        ...
        if (_file.isFile() === true && /\.carbone-license$/.test(_file.name) === true) {
          let _filename = path.join(licenseDir, _file.name);
      ...
      if (_mostRecentLicenseFile === '') {
        return callback(false, 'No *.carbone-license files found in '+licenseDir);
      }
      ...
      fs.readFile(_mostRecentLicenseFile, 'utf8', (err, license) => {
        ...
        kittenJwt.verify(license, publicKey, (err, payload) => {
          if (payload && payload.aud !== 'carbone-ee-on-premise') {
            return callback(false, 'Invalid license (not for carbone-ee-on-premise) ' + _licenseFile);
          }
          if (err) {
            ...
            if (payload && payload.data && payload.data.plan === 'eternity') {
              ...
              return kittenJwt.verify(license, publicKey, (err, payload) => {
                if (err) {
                  return callback(false, 'Invalid license ' + _licenseFile, payload);
                }
                return callback(true, 'WARNING: Your license ' + _licenseFile + ' has expired. Carbone-ee sill works but without support & updates', payload);
              }, _publicationTimestamp);
            }
            return callback(false, 'Invalid license '+ _licenseFile);
          }
          if ((_publicationTimestamp / 1000) > payload.exp) {
            return callback(false, 'Invalid license. Check system clock. ' + _licenseFile, payload);
          }
          if (payload.data.plan === 'trial') {
            return callback(true, 'Trial license '+ _licenseFile, payload);
          }
          return callback(true, 'Valid license '+ _licenseFile, payload);
        });
      });
    });
