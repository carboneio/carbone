...
var helper = {
  ...
  */
   ...
   * @param {String} content content used to compare with the text at the position 'pos'.
   * @param {String} text text to search inside the content beginning at the pos position
   * @param {String} pos position to compare the text
   * @returns {Boolean} returns true if the text exist, otherwise false
   */
  compareStringFromPosition : function (content, text, pos) {
    for (let i = 0, j = content.length; i < j; i++) {
      if (pos + i < text.length && content[i] !== text[pos + i]) {
        return false;
      }
    }
    return true;
