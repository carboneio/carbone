...
let pdfLib; 
(async () => {const { PDFDocument } = await import('pdf-lib'); pdfLib = PDFDocument; })();
...
var helper = {
  ...
  /**
   * This character is injected when a formatter contains a variable between quotes
   * to disable the variable system.
   */
  HIDDEN_CHAR_DISABLE_VARIABLE : '\u0001',

  // accept only 64-characters-long hexadecimal id for idTemplate
  // By default Carbone sanitizes file name (no path traversal,...) but it accepts any filename
  SECURITY_API_ID_TEMPLATE_64_HEX_ONLY : 0b00000001, // 1
  // Disable demo template.
  SECURITY_API_DISABLE_DEMO_TEMPLATE   : 0b00000010, // 2
  // disable crash report, automatic font/extension installation (zero impact on reports)
  SECURITY_LO_DISABLE_USELESS_FEATURE  : 0b00000100, // 4
  // Disable all script and macro feature of documents.
  // By default, Macro cannot be executed but this parameter deactivate all script execution
  SECURITY_LO_DISABLE_SCRIPT_EXECUTION : 0b00001000, // 8
  // disable automatic download of external images.
  SECURITY_LO_DISABLE_EXTERNAL_LINK    : 0b00010000, // 16
  // Disallow conversion of exotic file formats to reduce attack surface
  SECURITY_LO_DISABLE_EXOTIC_FILE      : 0b00100000, // 32
  // Removes author, revision author and commments
  SECURITY_LO_REMOVE_PERSONAL_INFO     : 0b01000000, // 64
  // Removes macros and scripts from ODT and ODS templates
  SECURITY_API_REMOVE_MACROS           : 0b10000000, // 128
  /**
  ...
  assignObjectIfAttributesExist : function (target, source) {
    ...
    return 'Additional plugin parameters detected: ' + _unknownParams.join(', ');
  },
   ...
   * @return {Array}     currentIterators iterators of currently visited array in the builder
   */
  getSafePathCode : function (getSafeValue, rootObj, path, currentIterators) {
    ...
    // replace all dynamic array access [..i] [.i] by [2i] or [1i] to simplify processing
    const _pathWithoutDirectAccess = path.replace(/\[(\.+)i\]/g, (o, dots) => {
      return '[' + dots.length + 'i]';
    });
    var _attributes = _pathWithoutDirectAccess.replace(/\s/g,'').split('.');
    for (var i = 0; i < _attributes.length; i++) {
      ...
      for (var j = 0; j < _arrayAccess.length; j++) {
        const _arrayIndexString = _arrayAccess[j][1];
        var _arrayIndex = parseInt(_arrayIndexString, 10);
        if (isNaN(_arrayIndex) === true || _arrayIndex < 0) {
          throw Error(_arrayIndexError);
        }
        // if it is an iterator, replace the static index by the dynamic one
        if (_arrayIndexString.endsWith('i') === true) {
          if (_arrayIndex > currentIterators.length) {
            throw Error('Forbidden array access in "'+path+'". No array iterators matching with '+('.'.repeat(_arrayIndex)+'i'));
          }
          _arrayIndex = currentIterators[currentIterators.length - _arrayIndex];
        }
  ...
  downloadExternalFile : function (url, acceptedMimeTypes, acceptedExtensions, options, callback) {
    const rock = require('rock-req'); // TODO
    ...
    rock({method : 'GET', url, timeout : 6000, tenantId : options.tenantId, requestId : options.requestId }, function (err, res, body) {
      ...
      let _extension = helper.getFileExtensionFromUrl(url, res.headers?.['content-disposition']);
      ...
      let _mimeType = helper.cleanContentType(res.headers['content-type']);
      // If the mime type image retreived from the Content-Type is not valid, the file extension is used to get the correct mime type
   ...
   *
   * By default, without runningError() callback, the queue is stopped as soon as handlerItem() returns an error.
   * The error is returned by finalCallback(err).
   *
   * If runningError(err) is defined, the queue is not stopped when there are errors, but runningError(err) is called for each error.
   * The callback finalCallback() is still called at the end with err = null.
   *
   * @param {Array}    items         Array of items               The array is not modified
   * @param {Function} handlerItem   handler(item, callback(err)) function called for each item of items array
   * @param {Function} callback      finalCallback(err)           function called when the queue is finished
   * @param {Number}   nbParallel    [optional] nb Parallel job = 1 by default
   * @param {Function} runningError  [optional] runningError(err) if defined, it returns error with this function instead of finalCallback
   *                                                              and the process does not stop on the first error
   */
  genericQueue : function (items = [], handlerItem, finalCallback, nbParallel = 1, runningError) {

    return {
      items        : items,
      nbItemsDone  : items.length,
      nbItemsToDo  : items.length,
      nbItemsTotal : items.length,
      isRunning    : false, // queue is ON
      isFinished   : false, // queue is finished
      ...
      processNextItem : function () {
        if (this.nbItemsDone === 0 && this.isFinished === false) {
          // We must use this "isFinished" boolean to guarantee the callback is called only once
          // because processNextItem can be called multiple times with this.nbItemsDone === 0 (race condition)
          this.isRunning = false;
          this.isFinished = true;
          return finalCallback(null);
        }
        if (this.nbItemsToDo === 0) {
          // no more item to process, still executing some other items. Now, we are waiting for nbItemsDone=0
          return;
        }

        const _currentItem = this.items[this.nbItemsTotal - this.nbItemsToDo];
        this.nbItemsToDo--;

        handlerItem.call(this, _currentItem, (err) => {
          this.nbItemsDone--;
          if (err) {
            if (runningError) {
              runningError(err);
          ...
          // start a new item
          setImmediate(this.processNextItem.bind(this));
        });
      },
      ...
      start : function () {
        if (this.isRunning === false) {
          this.isRunning = true;
          const _parallelExecution = Math.min(items.length, nbParallel);
          if (_parallelExecution === 0) {
            // nothing todo
            this.isRunning = false;
            return finalCallback(null);
          }
          for (let i = 0; i < _parallelExecution; i++) {
            setImmediate(this.processNextItem.bind(this));
          }
        }
      }
  ...
  */
   ...
   *
   * @param      {Object}    license               The license
   * @param      {String}    licenseErrorMessage   The license error message
   * @param      {Boolean}   [isEnterprise=false]  Indicates if enterprise mode
   * @return     {String}                          Message to print
   */
  printVersion : function (license, licenseErrorMessage, isEnterprise = false) {
    let _text = [];
    if (isEnterprise === false) {
      _text.push('WARNING:\n--------');
      _text.push(licenseErrorMessage);
      _text.push('⚠️  Only features of the Community Edition are enabled');
      _text.push('👉 Upgrade to the Enterprise Edition to access all features and support.\n   Contact us at contact@carbone.io or https://carbone.io\n');
    }
    _text.push('Carbone On-Premise: v' + package.version);
    ...
    if (params.securityLevel > 0) {
      let _textToPrint = '- Security level: '+params.securityLevel;
      _textToPrint += params.securityLevel & helper.SECURITY_API_ID_TEMPLATE_64_HEX_ONLY  ? '\n  - [API] Accept only 64-chars hex for idTemplate ✓'                 : '';
      _textToPrint += params.securityLevel & helper.SECURITY_API_REMOVE_MACROS            ? '\n  - [API] Remove embbed Macros and Scripts into ODT/ODS templates ✓' : '';
      _textToPrint += params.securityLevel & helper.SECURITY_LO_DISABLE_USELESS_FEATURE   ? '\n  - [LO] disable useless feature (remote crash reports...) ✓'        : '';
      _textToPrint += params.securityLevel & helper.SECURITY_LO_DISABLE_SCRIPT_EXECUTION  ? '\n  - [LO] script execution disabled ✓'                                : '';
      _textToPrint += params.securityLevel & helper.SECURITY_LO_REMOVE_PERSONAL_INFO      ? '\n  - [LO] remove personal info (author, comments) ✓'                  : '';
      _textToPrint += params.securityLevel & helper.SECURITY_LO_DISABLE_EXTERNAL_LINK     ? '\n  - [LO] external link disabled ✓'                                   : '';
      _textToPrint += params.securityLevel & helper.SECURITY_LO_DISABLE_EXOTIC_FILE       ? '\n  - [LO] exotic file disabled ✓'                                     : '';
      _text.push(_textToPrint);
    }
    return _text.join('\n');
  },
  ...
  collectStatistics : function (tenantId) {
    if (params.collectStatistics === true  && process.pkg /* not in development */) {
      const rock = require('rock-req'); // I will improve this code in v5 (setEgressConfig must be called before)
   ...
   *                                          position  : 'begin' | 'end' <- position of the file
   *                                       }
   * @param   {String}    outputFilePath   The output file path. Can the same as mainPdfFilePath to overwrite the initial file
...
/**
 * We use async because pdflib was not easy to use with callbacks
 */
...
async function mergePDFPromise (mainPdfFilePath, fileDatabase, outputFilePath) {
  ...
  for (let [key, value] of fileDatabase) {
    ...
    _pdfToAdd.source = null; // used for error management
  }
