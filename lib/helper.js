  ...
  */
  ...
  setEgressConfig : function (param) {
    ...
    if (param.egressProxyPrimary) {
      const _filter = param.egressIssuerFilter || [];
      ...
      const _pathFormatter = new Function('protocol', 'hostname', 'port', 'path', 'issuer', 'ruid', 'return `' + param.egressProxyPathFormat + '`;');
      rock.defaults.beforeRequest = (target) => {
        if (target.remainingRetry < target.maxRetry) {
          console.log('WARNING: egress traffic retry ' + target.hostname + target.path);
        }
        if (_filter.indexOf(parseInt(target.issuer, 10)) === -1) {
          return target;
        }
        const _port = target.port ?? (target.protocol === 'https:' ? 443 : 80);
        target.path = _pathFormatter(target.protocol.slice(0,-1), target.hostname, _port, target.path, (target.issuer ?? 0), (target.ruid ?? 0));
        let _proxy = _primary;
