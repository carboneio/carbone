   ...
   */
   getTemplateDefaultStyles : function (paragraphStyles, options, xmlContent, filename) {
    ...
    if (options.extension === 'docx') {
      ...
      const _regStyle = paragraphStyles.match(/text:style-name="((T|P|MP)[^"]*)"/);
      if (_regStyle !== null && _regStyle.length > 0 && _regStyle[0] !== '' && _regStyle[1] !== '') {
        const _resParagraphStyle = xmlContent.match(new RegExp(`<style:style style:name="${_regStyle[1]}"[^]*?<style:paragraph-properties ([^<>]*?)>`))
        if (_resParagraphStyle !== null && _resParagraphStyle.length > 0 && _resParagraphStyle[0] !== '' && _resParagraphStyle[1] !== '') {
          _defaultTemplateStylesId += `-${_regStyle[1]}`;
          _resParagraphStyle[1].replace(/(fo:text-align|style:writing-mode|fo:line-height|fo:margin-top|fo:margin-bottom|style:justify-single-word)="[^"]*?"/g, function (xmlStylePar, attribute) {
            _defaultTemplateStyles.paragraph += ` ${xmlStylePar}`;
          });
        }
         ...
         */
        const _reqTextStyle = xmlContent.match(new RegExp(`<style:style style:name="${_regStyle[1]}"[^]*?style:text-properties ([^<>]*?)>`))
        if (_reqTextStyle !== null && _reqTextStyle.length > 0 && _reqTextStyle[0] !== '' && _reqTextStyle[1] !== '') {
          _defaultTemplateStylesId += `-${_regStyle[1]}`;
          _reqTextStyle[1].replace(/(style:font-name|style:font-name-complex|fo:font-size|fo:background-color|fo:color|fo:font-style)="[^]*?"/g, function (xmlStyleProperties, attribute) {
