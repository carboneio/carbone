...
const html = {
  ...
  buildXMLContentOdt : function (styleID, descriptor) {
    ...
    for (let i = 0, j = descriptor.length; i < j; i++) {
      if (descriptor[i].content === html.BREAK_LINE || descriptor[i].content === '#PE#') {
        _content += html.xmlStyles.odt.br;
      }
      else if (descriptor[i].content !== '#PB#' && descriptor[i].content !== '#AB#' && descriptor[i].content !== '#AE#') {
  ...
  buildContentDOCX : function (descriptor, options) {
    ...
    for (let i = 0, j = descriptor.length; i < j; i++) {
      if (descriptor[i].content === html.BREAK_LINE) {
      ...
      } else if (descriptor[i].content === "#PB#") {
        if (_isHTMLP === 0) {
          _content += '<w:p>';
        }
        _isHTMLP++;
      } else if (descriptor[i].content === "#PE#") {
        if (_isHTMLP === 1) {
          _content += '</w:p><w:p/>'; // empty paragraph is more stable than a break line
        }
        _isHTMLP--;
      } else if (descriptor[i].content === "#AB#") {
        if (_isXMLA === 0) {
          /** =========== TO CLEAN UP ============ */
          let link = descriptor[i].href;
          ...
          let _rId = 0;
          if (!options.hyperlinkDatabase.has(link)) {
            _rId = options.hyperlinkDatabase.size;
            // If the image doesn't exist, it create a new ID
            options.hyperlinkDatabase.set(link, {
              id : _rId
            });
          } else {
            const _hpProp = options.hyperlinkDatabase.get(link);
            _rId = _hpProps.id;
          }
          /** ---------             ------------ */
          /** --------- TO CLEAN UP ------------ */
          if (_isHTMLP === 0 && _isXMLP === false) {
            _isHTMLP++;
            _isXMLP = true;
            _content += `<w:p>`
          }
          /** ---------             ------------ */
          _content +=  `<w:hyperlink r:id="${hyperlinks.getHyperlinkReferenceId(_rId)}">`
        }
        _isXMLA++;
      } else if (descriptor[i].content === "#AE#") {
        if (_isXMLA === 1) {
        ...
        if (_isHTMLP > 0 && ((i + 1 === descriptor.length ) ||
                             (_isXMLP === true && descriptor[i + 1].content === "#PB#") ||
  ...
  convertHTMLEntities : function (htmlContent = '') {
    return htmlContent.replace(/(&[\w\d#]*;)|(\n|\r|\t)/g, function (entity) {
