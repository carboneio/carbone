...
const html = {
  ...
  compareStringFromPosition : function (reference, text, pos) {
    ...
    for (let i = 0, j = reference.length; i < j; i++) {
      if (pos + i < text.length && reference[i] !== text[pos + i]) {
        return false;
      }
      // tmp +=  text[pos + i];
    }
    ...
    for (let i = 0, j = htmlContent.length; i < j; i++) {
      ...
      if (htmlContent[i] === '<') {
        ...
        if (Object.prototype.hasOwnProperty.call(actualTag, 'type') && actualTag.type === 'end' && actualTag.name !== 'br') {
          ...
          console.log('Style to delete:', actualTag.name, activeStyles, tagsToDelete);
        }
      }
      else {
      ...
      if ((i + 1 >= htmlContent.length) || // if at the end of the string
          ...
          (charToSave.length > 0 && previousTagType !== 'begin' && actualTag.type === 'begin')) {// if actual element is a starting tag and the previous element is different than a starting tag
        descriptor.push({
          content : charToSave,
          styles  : activeStyles
        });
        charToSave = '';
        console.log('BREAK + CLEAN:', activeStyles, tagsToDelete, descriptor);
        // CLEAN active styles
        activeStyles = activeStyles.filter((x) => {
        ...
        console.log('After CLEAN:', activeStyles, tagsToDelete);
      }
      ...
      if (Object.prototype.hasOwnProperty.call(actualTag, 'type') && actualTag.type === 'begin' && actualTag.name !== 'br') {
        activeStyles.push(actualTag.name);
        console.log('Add Style:', actualTag.name, activeStyles, tagsToDelete);
      }

      previousTagType = Object.prototype.hasOwnProperty.call(actualTag, 'type') ? actualTag.type : null;
    }
    // All tags should be deleted at the end of the loop.
    if (tagsToDelete.length > 0 || activeStyles.length > 0) {
      activeStyles.push(...tagsToDelete);
      let tagListError = activeStyles.reduce((acc, cu, id) => acc += '<' + cu + '>' + (id + 1 !== activeStyles.length ? ' ' : '') , '');
