...
const html = {
   ...
   * @returns {String}
   */
  reorderXML : function (content, fileType) {
    ...
    let marker = null;
    let _tagsToSearchs = fileType === 'docx' ? ['<w:p', '</w:p>'] : ['<text:p', '</text:p>']; // default ODT
    // eslint-disable-next-line no-cond-assign
    while ((marker = _patternHTML.exec(content)) !== null) {
      const _paragraphStartPos = this.findStartingParagraph(_tagsToSearchs, content, marker.index);
      const _paragraphEndPos = content.indexOf(_tagsToSearchs[1], marker.index + marker[0].length) + _tagsToSearchs[1].length;

      if (_paragraphList.includes(_paragraphStartPos) === true) {
        continue;
      }
      _paragraphList.push(_paragraphStartPos);

      const _paragraph = content.substring(_paragraphStartPos, _paragraphEndPos);
      console.log(_paragraph.substring(0, marker.index - _paragraphStartPos));
      /** Substring to get only the paragraph properties <w:rPr></w:rPr> */
      const _styles = html.getFontStyle(_paragraph.substring(0, marker.index - _paragraphStartPos), fileType);
      console.log('STYLES:', _styles);

      _markerList.push({
        newContent    : this.createParagraphContent(_paragraph, fileType, _styles),
        beginTagFirst : _paragraphStartPos,
  ...
  findStartingParagraph : function (tagsToSearch, content, initialPos) {
    ...
    while (_posToSearch > 0) {
      const _paragraphStartPos = content.lastIndexOf(tagsToSearch[0], _posToSearch);
      const _tmpContent = content.slice(_paragraphStartPos, _paragraphStartPos + tagsToSearch[0].length + 1);
      ...
      if (_lastCharacter === ' ' || _lastCharacter === '>') {
        return _paragraphStartPos;
      }
      _posToSearch = _paragraphStartPos - 1;
    }
    throw new Error('The template is not valid, a paragraph is missing.');
  },
  ...
  createParagraphContent : function (content, fileType, styles) {
    let _xmlTagRegExp = /<([^>]+?)>/g;
    let _prevIndex = -1;
    ...
    console.log('Xml without content:', _xmlTemplate);
    console.log('Descriptor', _descriptor);
  ...
  preprocessODT : function (template) {
    ...
    _file.data = html.reorderXML(_file.data, 'odt');
    // let patternHTML = /{([^{}]*?):html[^{}]*?}/g;
   ...
   */
  buildXMLContentOdt : function (styleID, descriptor = [], options, fontStyles) {
    let _content = '';
    let _style = '';
    let _styleLists = '';
    /** ------ */
    ...
    for (let i = 0, j = descriptor.length; i < j; i++) {
      ...
      if (_isHTMLP === 0 && _isXMLP === false && _isList === 0 && (descriptor[i].type === '' || descriptor[i].type === '#AB#') && descriptor[i].tags.includes('li') === false) {
        _isHTMLP++;
        _isXMLP = true;
        _content += `<text:p${fontStyles ? ' ' + fontStyles : ''}>`;
      }
  ...
  /** ============================= DOCX ONLY ============================= */
  getFontStyle : function (paragraphStyles, fileType) {
    console.log(paragraphStyles);
    let _htmlFormatterArgs = '';
    ...
    if (fileType === 'docx') {
      ...
      if (_regFontSizeResult !== null && _regFontSizeResult.length > 0 && _regFontSizeResult[0] !== '' && _regFontSizeResult[1] !== '') {
        _fontSize = _regFontSizeResult[1];
      }
      if (_fontFamily || _fontSize) {
        _htmlFormatterArgs += `(${_fontFamily ? '\'' + _fontFamily + '\'' : 'null' }, ${_fontSize ? '\'' + _fontSize + '\'' : 'null' })`;
      }
    }
    else if (fileType === 'odt') {
      const _regFontStyleResult = paragraphStyles.match(/text:style-name="[^"]*"/);
      if (_regFontStyleResult !== null && _regFontStyleResult.length > 0 && _regFontStyleResult[0] !== '') {
        _htmlFormatterArgs += `(${_regFontStyleResult[0]})`;
      }
    }
    return _htmlFormatterArgs;
  },
  /**
  ...
  preProcessDocx : function (template) {
    ...
    for (let i = 0, j = template.files.length; i < j; i++) {
      ...
      if (_file.name.indexOf('.xml') !== -1 && _file.name.indexOf('.rels') === -1 &&
        ...
        _file.data = html.reorderXML(_file.data, 'docx');
        // let marker;
        // let patternHTML = /{([^{}]*?):html[^{}]*?}/g;
        // /** 2 - Find HTML markers and get the position */
        // // eslint-disable-next-line no-cond-assign
