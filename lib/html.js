...
const html = {
  ...
  buildXMLContentOdt : function (styleID, descriptor, options, defaultStyleId) {
    ...
    for (let i = 0, j = descriptor.length; i < j; i++) {
      ...
      if (_isHTMLP === 0 && _isXMLP === false && _isList === 0 && (descriptor[i].type === '' || descriptor[i].type === html.types.ANCHOR_BEGIN || descriptor[i].type === html.types.IMAGE)) {
        _isHTMLP++;
        _isXMLP = true;
      ...
      if (descriptor[i].type === html.types.BREAK_LINE) {
        ...
        _content.add('<text:list-item>');
        // _content += '<text:list-item>';
        if (descriptor[i + 1].type === '' || descriptor[i + 1].type === html.types.PARAGRAPH_BEGIN || descriptor[i + 1].type === html.types.IMAGE || descriptor[i + 1].type === html.types.ANCHOR_BEGIN) {
          // _content += `<text:p${_defaultStylesObject.paragraph ? ' ' + _defaultStylesObject.paragraph : ''}>`;
          _content.add(`<text:p${_defaultStylesObject.paragraph ? ' ' + _defaultStylesObject.paragraph : ''}>`);
          _isXMLP = true;
        }
  ...
  buildXmlContentDOCX : function (descriptor, options, styleId = '') {
    ...
    for (let i = 0, j = descriptor.length; i < j; i++) {
      ...
      else if (descriptor[i].type === html.types.IMAGE) {
        _content.add(`<w:drawing>
          <wp:inline distT="0" distB="0" distL="0" distR="0">
            <wp:extent cx="`)
        _content.add(imageFormatters.scaleImage.call(options, descriptor[i].src, 'width', (descriptor[i].width >= 0 ? descriptor[i].width * 914400 / 96 : descriptor[i].width), 'emu', 'fill'));
        _content.add(`" cy="`);
        _content.add(imageFormatters.scaleImage.call(options, descriptor[i].src, 'height', (descriptor[i].height >= 0 ? descriptor[i].height * 914400 / 96 : descriptor[i].height), 'emu', 'fill'));
        _content.add(`"/><wp:effectExtent l="0" t="0" r="0" b="0"/><wp:docPr id="`);
        _content.add(imageFormatters.generateImageDocxId.call(options, descriptor[i].src));
              ...
              <a:graphicData uri="http://schemas.openxmlformats.org/drawingml/2006/picture">
                <pic:pic xmlns:pic="http://schemas.openxmlformats.org/drawingml/2006/picture">
                  <pic:nvPicPr>
                    <pic:cNvPr id="`);
        _content.add(imageFormatters.generateImageDocxId.call(options, descriptor[i].src));
        _content.add(`" name="" descr=""></pic:cNvPr>
                    <pic:cNvPicPr>
                  ...
                  <pic:blipFill>
                    <a:blip r:embed="`);
        _content.add(imageFormatters.generateImageDocxReference.call(options, descriptor[i].src, 'document.xml'));
        _content.add(`"></a:blip>
                    <a:stretch>
                      <a:fillRect/>
                    </a:stretch>
