...
const html = {
  ...
  reorderXML : function (content, fileType) {
    ...
    while ((marker = _patternHTML.exec(content)) !== null) {
      const _paragraphStartPos = this.findStartingParagraph(_tagsToSearchs[0], content, marker.index);
      ...
      /** Substring to get only the paragraph properties <w:rPr></w:rPr> (DOCX) or <text:p text:style-name="P1"> (ODT) */
      const _styles = html.getFontStyle(_paragraph.substring(0, marker.index - _paragraphStartPos), fileType);

      _markerList.push({
        newContent    : this.createNewXMLContent(_paragraph, fileType, _styles),
  ...
  findStartingParagraph : function (tagsToSearch, content, initialPos) {
    ...
    while (_posToSearch > 0) {
      const _paragraphStartPos = content.lastIndexOf(tagsToSearch, _posToSearch);
      const _tmpContent = content.slice(_paragraphStartPos, _paragraphStartPos + tagsToSearch.length + 1);
  ...
  /**
   * Retreive the font styles and properties from a paragraph.
   * Only ODT and DOCX are supported
   *
   * @param {*} paragraphStyles
   * @param {*} fileType
   * @returns
   */
