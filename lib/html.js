...
const html = {
    ...
    for (var i = 0; i < template.files.length; i++) {
      ...
      while (_item.done === false) {
        ...
        if (_item.value[1]?.filename === _file.name) {
          // console.log(_item.value[0], _item.value[1]);
          _newXmlRelations += _item.value[1].style;
          // console.log(_newXmlRelations);
  ...
  buildXMLContentOdt : function (styleID, descriptor, options, defaultStyleId) {
    ...
    console.log("✅ STYLE ID: ",_defaultStylesObject.paragraph , _styleParagraph);
   ...
   */
   getTemplateDefaultStyles : function (paragraphStyles, options, xmlContent, filename) {
    ...
    if (options.extension === 'docx') {
      ...
      const _regParagraphStyle = paragraphStyles.match(/text:style-name="((P|MP)[^"]*)"/);
      console.log(_regParagraphStyle, `<style:style style:name="${_regParagraphStyle[1]}"[^]*?<style:paragraph-properties ([^<>]*?)>`);
      if (_regParagraphStyle !== null && _regParagraphStyle.length > 0 && _regParagraphStyle[0] !== '' && _regParagraphStyle[1] !== '') {
        const _resParagraphStyle = xmlContent.match(new RegExp(`<style:style style:name="${_regParagraphStyle[1]}"[^]*?<style:paragraph-properties ([^<>]*?)>`))
        if (_resParagraphStyle !== null && _resParagraphStyle.length > 0 && _resParagraphStyle[0] !== '' && _resParagraphStyle[1] !== '') {
          ...
          _resParagraphStyle[1].replace(/(fo:text-align|style:writing-mode|fo:line-height|fo:margin-top|fo:margin-bottom)="[^"]*?"/g, function (xmlStylePar) {
            _defaultTemplateStyles.paragraph += ` ${xmlStylePar}`;
          });
        }
      }
      // Get the text style ID
      const _regTextStyleId = paragraphStyles.match(/text:style-name="((T|MP)[^"]*)"/);
      if (_regTextStyleId !== null && _regTextStyleId.length > 0 && _regTextStyleId[0] !== '' && _regTextStyleId[1] !== '') {
         ...
         */
        const _reqTextStyle = xmlContent.match(new RegExp(`<style:style style:name="${_regTextStyleId[1]}"[^]*?style:text-properties ([^<>]*?)>`))
