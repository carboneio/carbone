  ...
  buildXMLContentOdt : function (styleID, descriptor, options, defaultStyleId) {
    ...
    for (let i = 0, j = descriptor.length; i < j; i++) {
      ...
      if (descriptor[i].type === html.types.BREAK_LINE) {
        ...
        _content.add(imageFormatters.scaleImage.call(options, descriptor[i].src, 'width', -1, 'cm', 'contain'));
        _content.add(`cm" svg:height="`);
        _content.add(imageFormatters.scaleImage.call(options, descriptor[i].src, 'height', -1, 'cm', 'contain'));
        ...
        // _content += `
        // <draw:frame draw:name="Image1" text:anchor-type="char" svg:width="1.2126in" svg:height="1.8189in" draw:z-index="0">
        //   <draw:image xlink:href="Pictures/1000000000000E80000015C071855B5F4574C847.jpg" draw:mime-type="image/jpeg" xlink:type="simple" xlink:show="embed" xlink:actuate="onLoad"/>
        // </draw:frame>
        // `
      }
  ...
  parseHTML : function (xml) {
    ...
    while (_tag !== null) {
      ...
      if (_tagOnly === 'br' || _tagOnly === 'br/') {
        ...
        const _regResp = _tagStr.match(/src="([^<">]*?)"/);
        if (_regResp?.[1]) {
          const _urlOrBase64 = _regResp?.[1];
          _res.push({
            content : '',
          ...
          // console.log(options.imageDatabase.get(_urlOrBase64));
        }
      }
      else if ( _tagOnly === 'p' ) {
        _res.push({
