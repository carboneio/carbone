...
const html = {
   ...
   */
   reorderXML : function (content, fileType) {
    ...
    while ((marker = _patternHTML.exec(content)) !== null) {
      ...
      _markerList.push({
        newContent: this.createParagraphContent(_paragraph, fileType),
  ...
  seperateHTMLMarker : function(content, pos, fileType) {
    let patternHTML = /{([^{}]*?):html[^{}]*?}/g;
    let _descriptor = [];
    let _lastIndex = -1;
    const _postProcessFormatter = fileType === 'docx' ? ':getHTMLContentDocx' : ':getHTMLContentOdt';
    if (content.indexOf(":html") === -1) {
      _descriptor.push({
        pos: pos,
        data: content,
  ...
  createParagraphContent : function (content, fileType) {
    let _xmlTagRegExp = /<([^>]+?)>/g;
    let _prevIndex = -1;
    ...
    while ((tag = _xmlTagRegExp.exec(content)) !== null) {
      if (_prevIndex !== -1 && _prevIndex !== tag.index) {
        ...
        _descriptor.push(...this.seperateHTMLMarker(_content, _prevIndex - _totalContentLength, fileType));
