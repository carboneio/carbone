...
const html = {
  ...
  buildXmlContentDOCX : function (descriptor, options, styleId = '') {
    ...
    for (let i = 0, j = descriptor.length; i < j; i++) {
      if (_isHTMLP === 0 && _isXMLP === false && _listLevel === -1 && (descriptor[i].type === '' || descriptor[i].type === html.types.ANCHOR_BEGIN || descriptor[i].type === html.types.IMAGE) && descriptor[i].tags.includes('li') === false) {
        _isHTMLP++;
        _isXMLP = true;
        _content.add('<w:p>');
        // _content += '<w:p>';
