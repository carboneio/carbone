...
const html = {
      ...
      if (_file.name.indexOf('.xml') !== -1 && _file.name.indexOf('.rels') === -1 &&
        ...
        while ((marker = patternHTML.exec(_file.data)) !== null) {
          ...
          if (propertiesPosBegin !== -1) {
            const _regFontResult = _file.data.substring(propertiesPosBegin, k).match(/<w:rFonts[^]*?(w:ascii|w:hAnsi|w:cs|w:eastAsia)="([^]*?)"[^]*?\/>/);
            if (_regFontResult.length > 0 && _regFontResult[0] !== '' && _regFontResult[2] !== '') {
          ...
          /** 4 - Insert HTML markers if positions are found */
          if (subContentPos !== -1) {
            _file.data = helper.insertAt(_file.data, subContentPos, `{${marker[1]}:getHTMLContentDocx${defaultFont ? '(\'' + defaultFont + '\')' : '' }}`);
            _file.data = _file.data.replace(marker[0], '');
          }
        }
      }
   ...
   */
  buildContentDOCX : function (descriptor, options, defaultFont) {
