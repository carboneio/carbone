...
const html = {
  ...
  buildXMLContentOdt : function (styleID, descriptor = []) {
    ...
    for (let i = 0, j = descriptor.length; i < j; i++) {
      ...
      if (descriptor[i].type === html.types.BREAK_LINE) {
      ...
      else if (descriptor[i].type === html.types.UNORDERED_LIST_BEGIN) {
        // Create a style to manage bullets
        _content += '<text:list>'
        _isList++;
      }
      else if (descriptor[i].type === html.types.UNORDERED_LIST_END) {
        // Create a style to manage bullets
        _content += '</text:list>'
        if (_isList === 1) {
          _content += `<text:p text:style-name="Standard"/>`;
        ...
        // Create a style to manage bullets
        _content += '<text:list-item><text:p>'
      }
      ...
      if (_isList > 0 && (( i + 1 !== descriptor.length && descriptor[i + 1].type === html.types.LIST_ITEM_END && descriptor[i].type !== html.types.ORDERED_LIST_END && descriptor[i].type !== html.types.UNORDERED_LIST_END) ||
                          (i + 1 !== descriptor.length && descriptor[i + 1].type === html.types.ORDERED_LIST_BEGIN) ||
                          ( i + 1 !== descriptor.length && descriptor[i + 1].type === html.types.UNORDERED_LIST_BEGIN))) {
        _content += '</text:p>';
      }
      ...
      if (_isHTMLP > 0 && ((i + 1 === descriptor.length ) ||
                          ...
                          (_isXMLP === true && (descriptor[i + 1].type === html.types.ORDERED_LIST_BEGIN || descriptor[i + 1].type === html.types.UNORDERED_LIST_END)))) {
        _content += '</text:p>';
        _isXMLP = false;
        _isHTMLP--;
      }
    ...
    return { content : _content, style : _style};
      ...
      // if (descriptor[i].content === html.types.BREAK_LINE || descriptor[i].content === html.types.PARAGRAPH_END) {
      //   _content += html.xmlStyles.odt.br;
      // }
      // else if (descriptor[i].content !== '') {
      //   const _uniqueStyleID = styleID + i;
      //   _content += `<text:p text:style-name="${_uniqueStyleID}">${descriptor[i].content}</text:p>`;
      //   if (descriptor[i].tags && descriptor[i].tags.length > 0) {
  ...
  buildContentDOCX : function (descriptor, options) {
    ...
    for (let i = 0, j = descriptor.length; i < j; i++) {
      ...
      if (_isHTMLP === 0 && _isXMLP === false && (descriptor[i].type === '' || descriptor[i].type === '#AB#') && descriptor[i].tags.includes('li') === false) {
        _isHTMLP++;
        _isXMLP = true;
        _content += '<w:p>';
      }
      ...
      if (descriptor[i].type === html.types.BREAK_LINE) {
        if ( _isHTMLP > 0) { //_isXMLP === true || FIX NEW LINES
        ...
        _content += `<w:p><w:pPr><w:numPr><w:ilvl w:val="${_listLevel}"/><w:numId w:val="${_listId}"/></w:numPr>${_listStyle}</w:pPr><w:r>${_listStyle}<w:t>${descriptor[i].content}</w:t></w:r></w:p>`;
      }
      else {
        /** CONTENT + STYLE */
        ...
        _content += `<w:r><w:rPr>${_contentProperties}</w:rPr><w:t xml:space="preserve">${descriptor[i].content}</w:t></w:r>`;
      }

      if (_isHTMLP > 0 && ((i + 1 === descriptor.length ) ||
                          (_isXMLP === true && descriptor[i + 1].type === html.types.PARAGRAPH_BEGIN) ||
                          (_isXMLP === true && descriptor[i + 1].tags.includes('li') === true))) {
        _content += '</w:p>';
        _isXMLP = false;
        _isHTMLP--;
      }
  ...
  types : {
    ...
    UNORDERED_LIST_BEGIN : '#OLB#',
    UNORDERED_LIST_END   : '#OLE#',
  }
};
