...
/**
 * TODO
 * - [ ] DOCX only: what happen if a style is included twice into a paragraph? crash?
 * - [ ] Créer les fichiers XML pour les lists et images (footer/header) quand ils n'existent pas
 * - [ ] Supporter les Styles headings <text:h text:style-name="Heading_20_4"/> puis chercher le style par defaul dans "styles.xml"
 */
...
const html = {
   ...
   * @returns {String}
   */
  reorderXML : function (content, options, filename) {
    ...
    while ((marker = _patternHTML.exec(content)) !== null) {
      ...
      const _styles = html.getTemplateDefaultStyles(_paragraph.substring(0, marker.index - paragraphStartPos), options, content, filename);
  ...
  preProcessODT : function (template, options) {
    ...
    for (var i = 0; i < template.files.length; i++) {
      const _file = template.files[i];
      if (['content.xml', 'styles.xml'].indexOf(_file.name) === -1) {
        continue;
      }
      _file.data = html.reorderXML(_file.data, options, _file.name);
    }
  },
  /**
  ...
  postProcessODT : function (template, data, options) {
    ...
    for (var i = 0; i < template.files.length; i++) {
      ...
      while (_item.done === false) {
        ...
        if (_item.value[1]?.filename === _file.name) {
  ...
  buildXMLContentOdt : function (styleID, descriptor, options, defaultStyleId) {
    ...
    const _defaultStylesObject = this.getHtmlDefaultStylesDatabase(options, defaultStyleId);
    /** Add style for the default paragraph styles */
    let _styleParagraph = "";
    if (_defaultStylesObject.paragraph) {
      const _styleParagraphID = styleID + 'D';
      _styleParagraph = ` text:style-name="${ _styleParagraphID }"`;    
      _style += `<style:style style:name="${ _styleParagraphID }" style:family="paragraph"><style:paragraph-properties${_defaultStylesObject.paragraph}/></style:style>`;    
    }
    ...
    for (let i = 0, j = descriptor.length; i < j; i++) {
      ...
      if (_isXMLP === false && _isList === 0 && (descriptor[i].type === '' || descriptor[i].type === html.types.ANCHOR_BEGIN || descriptor[i].type === html.types.IMAGE)) {
        _isXMLP = true;
        _content.add(`<text:p${_styleParagraph}>`);
      }
      ...
      if (descriptor[i].type === html.types.BREAK_LINE) {
        if (_isXMLP === true || _isHTMLP > 0 || _isList > 0) {
          ...
          /** Empty paragraph */
          _content.add(`<text:p${ _styleParagraph ? _styleParagraph : ' text:style-name="Standard"'}/>`);
        }
      }
        ...
        _content.add(`</text:p><text:p${_styleParagraph ? _styleParagraph : ' text:style-name="Standard"'}/>`);
        _isXMLP = false;
        _isHTMLP--;
      }
        ...
        if (_isList > 1) {
          if (descriptor[i - 1].type === '' || descriptor[i - 1].type === html.types.BREAK_LINE || descriptor[i - 1].type === html.types.PARAGRAPH_END || descriptor[i - 1].type === html.types.ANCHOR_END) {
            _content.add('</text:p>');
            _isXMLP = false;
            if (_isHTMLP > 0) {
              _isHTMLP--;
        ...
        if ((descriptor[i].tags && descriptor[i].tags.length > 0) || _defaultStylesObject.text !== '') {
          ...
          let _styleContent = html.buildXMLStyle('odt', descriptor[i].tags, _defaultStylesObject);
  ...
  postProcessDocx : function (template, data, options) {
    ...
    /** TODO:
     * - [ ] Create the file "numbering.xml" if it does not exist
     */
    let _file = null;
    for (let i = 0, j = template.files.length; i < j; i++) {
   ...
   */
  buildXmlContentDOCX : function (descriptor, options, styleId = '', filename= '') {
    ...
    const _fileOrigin = filename !== '' ? path.basename(filename) : 'document.xml'
    ...
    for (let i = 0, j = descriptor.length; i < j; i++) {
      ...
      if (descriptor[i].type === html.types.BREAK_LINE) {
        ...
        _content.add(imageFormatters.generateImageDocxReference.call(options, descriptor[i].src, _fileOrigin, { width: descriptor[i].width, height: descriptor[i].height }));
        ...
        if (_listLevel > -1 && i - 1  >= 0 && (descriptor[i - 1].type === '' || descriptor[i - 1].type === html.types.LIST_ITEM_BEGIN || descriptor[i - 1].type === html.types.BREAK_LINE || descriptor[i - 1].type === html.types.PARAGRAPH_END || descriptor[i - 1].type === html.types.ANCHOR_END)) {
          _content.add('</w:p>');
          _isXMLPList = false;
        }
        _listLevel++;
        ...
        let _contentProperties = html.buildXMLStyle('docx', descriptor[i].tags, _defaultTemplateStyles);
        if (_isXMLA > 0) {
   ...
   */
   getTemplateDefaultStyles : function (paragraphStyles, options, xmlContent, filename) {
    ...
    if (options.extension === 'docx') {
      /** Used to pass all previous tests: later, the `buildXmlContentDOCX` is setting "document.xml" the filename is null */
      if (filename && filename.includes("document.xml") === true) {
        filename = null;
      }
      // Font Family
      ...
      if (_regFontSizeResult !== null && _regFontSizeResult.length > 0 && _regFontSizeResult[0] !== '' && _regFontSizeResult[1] !== '') {
        ...
        /** Special case: MS Word may not include `w:sz` for no reasons but it is required for the font size */
        if (_regFontSizeResult[0].includes('w:szCs') === true && 
            _defaultTemplateStyles.text.includes('w:sz ') === false) {
          _regFontSizeResult[0] += `<w:sz w:val="${_regFontSizeResult[1]}"/>`
        }
      ...
      // spacing, example: <w:spacing w:line="600" w:lineRule="auto"/>
      const _regSpacingResult = paragraphStyles.match(/<(w:spacing)[^<>]*?\/>/);
      if (_regSpacingResult !== null && _regSpacingResult.length > 0 && _regSpacingResult[0] !== '' && _regSpacingResult[1] !== '') {
        _defaultTemplateStylesId += `-${_regSpacingResult[1]}`
        _defaultTemplateStyles.paragraph += _regSpacingResult[0];
      }
      // indent, example: <w:ind w:left="720"/>
      const _regIndResult = paragraphStyles.match(/<(w:ind)[^<>]*?\/>/);
      if (_regIndResult !== null && _regIndResult.length > 0 && _regIndResult[0] !== '' && _regIndResult[1] !== '') {
        _defaultTemplateStylesId += `-${_regIndResult[1]}`
        _defaultTemplateStyles.paragraph += _regIndResult[0];
      }
      // Bold / Italic / Underline / strike / <w:smallCaps/> / <w:vanish/> / <w:dstrike/> / <w:caps w:val="true" />
      paragraphStyles.replace(/(<w:b\/>|<w:bCs\/>|<w:i\/>|<w:iCs\/>|<w:u[^<>]*?\/>|<w:strike\/>|<w:caps[^<>]*?\/>|<w:smallCaps[^<>]*?\/>|<w:dstrike[^<>]*?\/>|<w:vanish[^<>]*?\/>)/g, function (xmlStyleProperties) {
        if (_defaultTemplateStyles.text.includes(xmlStyleProperties) === false) {
          _defaultTemplateStylesId += `-${xmlStyleProperties.replace(/[<>/="\s]/g, '')}`
      ...
      /**
       * Get the paragraph style properties: Text alignment, right to left, and margin.
       */
      const _getParagraphStyles = (xmlContent, styleID) => {
        let paragraphStyles = ''
        const _resParagraphStyle = xmlContent.match(new RegExp(`<style:style style:name="${styleID}"[^>]*?>([^]*?)<\/style:style>`))
        if (_resParagraphStyle !== null && _resParagraphStyle.length > 0 && _resParagraphStyle[0] !== '' && _resParagraphStyle[1] !== '') {
          _resParagraphStyle[1].replace(/(fo:text-align|style:writing-mode|fo:line-height|fo:margin-top|fo:margin-bottom|style:justify-single-word|fo:text-indent|style:auto-text-indent)="[^"]*?"/g, function (xmlStylePar, attribute) {
            paragraphStyles += ` ${xmlStylePar}`;
          });
        }
      ...
      */
      const _getTextStyles = (xmlContent, styleID) => {
        let textStyles = '';
        const _reqTextStyle = xmlContent.match(new RegExp(`<style:style style:name="${styleID}"[^>]*?>([^]*?)<\/style:style>`))
        if (_reqTextStyle !== null && _reqTextStyle.length > 0 && _reqTextStyle[0] !== '' && _reqTextStyle[1] !== '') {
          _reqTextStyle[1].replace(/(style:font-name|style:font-name-complex|fo:font-size|fo:background-color|fo:color|fo:font-style|fo:font-weight|style:font-weight-asian|style:font-weight-complex|fo:letter-spacing)="[^]*?"/g, function (xmlStyleProperties, attribute) {
            textStyles += ` ${xmlStyleProperties}`;
          });
        }
        return textStyles;
      }

      // // Get the paragraph and text style from HEADER/Footer
      const _regStyleMP = paragraphStyles.match(/text:style-name="(MP[^"]*)"/);
      if (_regStyleMP !== null && _regStyleMP.length > 0 && _regStyleMP[0] !== '' && _regStyleMP[1] !== '' && xmlContent) {
        const _stylesP = _getParagraphStyles(xmlContent, _regStyleMP[1])
        if (_stylesP !== '') {
          _defaultTemplateStyles.paragraph += _stylesP;
          _defaultTemplateStylesId += `-${_regStyleMP[1]}`;
        }
        const _styleT = _getTextStyles(xmlContent, _regStyleMP[1]);
        if (_styleT !== '') {
          _defaultTemplateStyles.text += _styleT;
          _defaultTemplateStylesId += `-${_regStyleMP[1]}`;
        }
      }

      // Get the paragraph style ID from paragraphs or headers/footers: RTL, Text Alignement
      const _regStyleP = paragraphStyles.match(/text:style-name="(P[^"]*)"/);
      if (_regStyleP !== null && _regStyleP.length > 0 && _regStyleP[0] !== '' && _regStyleP[1] !== '' && xmlContent) {
        const _stylesP = _getParagraphStyles(xmlContent, _regStyleP[1])
        if (_stylesP !== '') {
          _defaultTemplateStyles.paragraph += _stylesP;
          _defaultTemplateStylesId += `-${_regStyleP[1]}`;
        }
        const _styleT = _getTextStyles(xmlContent, _regStyleP[1]);
        if (_styleT !== '') {
          _defaultTemplateStyles.text += _styleT;
          _defaultTemplateStylesId += `-${_regStyleP[1]}`;
        }
      ...
      /** 
       * The paragraph my contain multiple empty text, only the closest <text:span is retrieved
       * For instance: "<text:p text:style-name="P1"><text:span text:style-name="T3"></text:span><text:span text:style-name="T2">""
       */
      const _indexClosestText = paragraphStyles.lastIndexOf('<text:span');
      if (_indexClosestText !== -1) {
        const _searchedTextStyle = paragraphStyles.substring(_indexClosestText);

        // Get text style ID
        const _regStyleT = _searchedTextStyle.match(/text:style-name="(T[^"]*)"/);
        if (_regStyleT !== null && _regStyleT.length > 0 && _regStyleT[0] !== '' && _regStyleT[1] !== '' && xmlContent) {
          const _styleT = _getTextStyles(xmlContent, _regStyleT[1]);
          if (_styleT !== '') {
            _defaultTemplateStyles.text += _styleT;
            _defaultTemplateStylesId += `-${_regStyleT[1]}`;
          }
        }
      }
    }
    if (_defaultTemplateStylesId !== 'style') {
      ...
      return `('${_defaultTemplateStylesId}'${filename ? ',\''+ filename + '\'' : ''})`;
    }
  ...
  */
   ...
   */
  buildXMLStyle : function (fileType, tags, defaultStyles = null) {
    let _styles = defaultStyles?.text ?? '';
    ...
    for (let i = 0, j = tags.length; i < j; i++) {
      if (Object.prototype.hasOwnProperty.call(html.xmlStyles[fileType], tags[i])) {
        ...
        const _style = html.xmlStyles[fileType][tags[i]];

        if (fileType === 'odt') {
          const _attribute = _style?.split("=")?.[0];
          if (_attribute && _styles.includes(_attribute) === true) {
            continue;
          }
          _styles +=  ' ' + _style;
        } else if (fileType === 'docx' && _styles.includes(_style.substring(0, 5)) === false) {
          _styles += _style;
        }
      }
    }
  ...
  /**
   * Update or Insert a CSS property inside style attribute of HTML (used by :color)
   *
   * @param      {string}  html                The html
   * @param      {number}  styleValueIndex     The style value index in HTML string. Ex <tr style="blabla
   *                                                                                               ^
   * @param      {string}  cssPropertyToPatch  The css property to patch. Ex: color
   * @param      {string}  newCssValue         The new css value to update or insert
   * @return     {Object}                      A patch to apply in HTML string:
   *                                           {
   *                                             from : index in HTML string
   *                                             to   : index in HTML string
   *                                             add  : string to insert in HTML string between "from" and "to"
   *                                           }
   */
  patchStyleAttribute : function (html, styleValueIndex, cssPropertyToPatch, newCssValue) {
    const CSS_PREV_CHAR = [';', '\n', ' ', '"'];
    const CSS_NEXT_CHAR = [' ', ':'];
    const _endOfStyleAttribute = html?.indexOf('"', styleValueIndex) ?? -1;
    if (_endOfStyleAttribute === -1) {
      return null;
    }
    let _cssPropertyToPatchIndex = html.indexOf(cssPropertyToPatch, styleValueIndex);
    while (_cssPropertyToPatchIndex > -1 && _cssPropertyToPatchIndex < _endOfStyleAttribute) {
      const _prevChar = html[_cssPropertyToPatchIndex-1];
      const _nextChar = html[_cssPropertyToPatchIndex + cssPropertyToPatch.length];
      // check if it is the right CSS attribute. Ex: Avoid "background-color:" if we search "color:"
      if (CSS_PREV_CHAR.indexOf(_prevChar) !== -1 && CSS_NEXT_CHAR.indexOf(_nextChar) !== -1) {
        break;
      }
      _cssPropertyToPatchIndex = html.indexOf(cssPropertyToPatch, _cssPropertyToPatchIndex + 1);
    }
    // CSS attribute already here patch it
    if (_cssPropertyToPatchIndex !== -1 && _cssPropertyToPatchIndex < _endOfStyleAttribute) {
      let _endOfCssPropertyToPatchIndex = html.indexOf(';', _cssPropertyToPatchIndex);
      if (_endOfCssPropertyToPatchIndex === -1 || _endOfCssPropertyToPatchIndex > _endOfStyleAttribute) {
        _endOfCssPropertyToPatchIndex = _endOfStyleAttribute;
      }
      const _cssStartValueIndex = html.indexOf(':', _cssPropertyToPatchIndex) + 1;
      return { from : _cssStartValueIndex, to : _endOfCssPropertyToPatchIndex, add : newCssValue  };
    }
    return { from : styleValueIndex, to : styleValueIndex, add : cssPropertyToPatch + ':' + newCssValue + '; ' };
  },
