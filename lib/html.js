...
const html = {
   ...
   */
   reorderXML : function (content) {
    ...
    while ((marker = patternHTML.exec(content)) !== null) {
      ...
      const _beginTagFirst = content.lastIndexOf('<text', marker.index);
      ...
      const _endTagLast = content.indexOf('</text:p>', _endPosHtml) + 9;
      ...
      markerList.push({
        content: this.createParagraphDescriptor(_content, _startTag, _endTag),
        beginTagFirst: _beginTagFirst,
        endTagLast: _endTagLast
      });
      // console.log(marker, _startParagraphStartPos, _startParagraphEndPos, _endParagraphPos);
      // const _startTag = ;
      // const _endTag = content.substring(_endParagraphPos, _endParagraphPos + '</text:p>'.length);
      // const createParagraph = (el) => `${_startTag}${el}${_endTag}`
    }
  ...
  createParagraphDescriptor : function(content, startTag, endTag) {
    let patternHTML = /{([^{}]*?):html[^{}]*?}/g;
    let _newContent = "";
    let _lastIndex = -1;
    while ((marker = patternHTML.exec(content)) !== null) {
    ...
    const _lastStaticContent = content.substring(_lastIndex === -1 ? 0 : _lastIndex, content.length);
