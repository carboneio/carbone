...
const html = {
  ...
  buildXMLContentOdt : function (styleID, descriptor, options, defaultStyleId) {
    ...
    this.cleanInvalidParagraph(descriptor);
    console.log(descriptor);
    ...
    for (let i = 0, j = descriptor.length; i < j; i++) {
      ...
      console.log(descriptor[i], _isHTMLP, _isXMLP, _isList)
      ...
      if (descriptor[i].type === html.types.BREAK_LINE) {
      ...
      else if (descriptor[i].type === html.types.IMAGE && ( _isHTMLP > 0 || _isList > 0)) {
        ...
        _content.add('<text:list-item>');
        // _content += '<text:list-item>';
        console.log(descriptor[i + 1]?.type !== html.types.PARAGRAPH_BEGIN)
        if (descriptor[i + 1]?.type !== html.types.PARAGRAPH_BEGIN && (descriptor[i + 1].type === '' || descriptor[i + 1].type === html.types.IMAGE || descriptor[i + 1].type === html.types.ANCHOR_BEGIN)) {
          // _content += `<text:p${_defaultStylesObject.paragraph ? ' ' + _defaultStylesObject.paragraph : ''}>`;
          _content.add(`<text:p${_defaultStylesObject.paragraph ? ' ' + _defaultStylesObject.paragraph : ''}>`);
          _isXMLP = true;
        }
      ...
      if (_isXMLP === true && ((i + 1 === descriptor.length ) ||
        ...
        // _content += '</text:p>';
        _content.add('</text:p>');
        console.log(_isXMLP, "PASS HEREEEEE", _isList);
        _isXMLP = false;
        if (_isHTMLP > 0) {
          _content.add('<text:p text:style-name="Standard"/>');
          _isHTMLP--;
  ...
  buildXmlContentDOCX : function (descriptor, options, styleId = '') {
    ...
    for (let i = 0, j = descriptor.length; i < j; i++) {
      ...
      console.log(descriptor[i], _isHTMLP, _isXMLP, _listLevel, descriptor[i].type === '')
      ...
      if (_isHTMLP === 0 && _isXMLP === false && _listLevel === -1 && (descriptor[i].type === '' || descriptor[i].type === html.types.ANCHOR_BEGIN || descriptor[i].type === html.types.IMAGE)) {
        ...
        if (_defaultTemplateStyles.paragraph) {
          ...
          // _content += `<w:pPr>${_defaultTemplateStyles.paragraph}</w:pPr>`;
        }
      }
      ...
      if (descriptor[i].type === html.types.BREAK_LINE) {
        ...
        console.log("P END", _isHTMLP);
        /** empty paragraph is more stable than a break line */
        _content.add('</w:p><w:p/>')
        _isXMLP = false;
        _isHTMLP--;
      }
      else if (descriptor[i].type === html.types.IMAGE && (_isXMLP === true || _isXMLPList === true)) {
        _content.add(`<w:drawing><wp:inline distT="0" distB="0" distL="0" distR="0"><wp:extent cx="`)
        ...
        _content.add(`" name="" descr=""/><wp:cNvGraphicFramePr><a:graphicFrameLocks xmlns:a="http://schemas.openxmlformats.org/drawingml/2006/main" noChangeAspect="1"/></wp:cNvGraphicFramePr><a:graphic xmlns:a="http://schemas.openxmlformats.org/drawingml/2006/main"><a:graphicData uri="http://schemas.openxmlformats.org/drawingml/2006/picture"><pic:pic xmlns:pic="http://schemas.openxmlformats.org/drawingml/2006/picture"><pic:nvPicPr><pic:cNvPr id="`);
        _content.add(imageFormatters.generateImageDocxId.call(options, descriptor[i].src, { width: descriptor[i].width, height: descriptor[i].height }));
        _content.add(`" name="" descr=""></pic:cNvPr><pic:cNvPicPr><a:picLocks noChangeAspect="1" noChangeArrowheads="1"/></pic:cNvPicPr></pic:nvPicPr><pic:blipFill><a:blip r:embed="`);
        ...
        _content.add(`" cy="`);
        _content.add(imageFormatters.scaleImage.call(options, descriptor[i].src, 'height', (descriptor[i].height >= 0 ? descriptor[i].height * 914400 / 96 : descriptor[i].height), 'emu', 'fill', { width: descriptor[i].width, height: descriptor[i].height }));
        _content.add(`"/></a:xfrm><a:prstGeom prst="rect"><a:avLst/></a:prstGeom></pic:spPr></pic:pic></a:graphicData></a:graphic></wp:inline></w:drawing>`);
      }
        ...
        _content.add(`<w:hyperlink r:id="${hyperlinks.getHyperlinkReferenceId(_hpProperties.id)}">`)
        _isXMLA++;
      }
        ...
        _content.add(`<w:p><w:pPr><w:numPr><w:ilvl w:val="${_listLevel}"/><w:numId w:val="${_listId}"/></w:numPr>${_defaultTemplateStyles.paragraph ? _defaultTemplateStyles.paragraph : ''}</w:pPr>`)
      ...
      console.log(_isXMLP, _isHTMLP, descriptor[i + 1]?.type, _isXMLA)
   ...
   */
  cleanInvalidParagraph: function (descriptor) {
    ...
    for (let j = 0; j < descriptor.length; j++) {
      if (_isEmpty === true && _paragraphBeginPos >= 0 &&
        ...
        console.log(_paragraphBeginPos, j);
      ...
      console.log(descriptor[j], descriptor[j].content?.trim()?.length)
      if (descriptor[j].type === html.types.IMAGE || descriptor[j].type === html.types.ANCHOR_BEGIN || (descriptor[j].content !== '' && descriptor[j].content?.trim()?.length > 0 && descriptor[j].type !== html.types.PARAGRAPH_BEGIN)) {
        _isEmpty = false;
