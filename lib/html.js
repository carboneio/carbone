...
const html = {
  ...
  parseHTML : function (htmlContent) {
    ...
    for (let i = 0, j = htmlContent.length; i < j; i++) {
      ...
      if (htmlContent[i] === '<') {
        ...
        if (actualTag.type === 'begin') {
          console.log('Add Style:', actualTag.name);
          activeStyles.push(actualTag.name);
        }
        console.log(activeStyles);
        console.log(`Jump from ${i} to ${actualTag.pos}`);
        i = actualTag.pos; // jump at the end of the tag
      }
      else {
        console.log("Add: ", htmlContent[i]);
        charToSave += htmlContent[i];
      }

      if (i + 1 >= htmlContent.length || (actualTag !== null && actualTag.type === 'end' && htmlContent[i + 1] !== '<')) {
        descriptor.push({
          content : charToSave,
          styles  : activeStyles
        });
      ...
      if (actualTag !== null && actualTag.type === 'end') {
        console.log('Delete Style:', actualTag.name);
        activeStyles = activeStyles.filter(x => x !== actualTag.name);
      }
    }
  ...
  identifyTag : function (htmlContent, pos) {
    ...
    let i = pos;
    let type = 'begin';
    while (pos < htmlContent.length && htmlContent[i] !== '>') {
