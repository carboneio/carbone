...
const html = {
   ...
   * @param {Boolean} force html to be inline. Avoid generating a new paragraph in document. Used by formatter htmlInline.
   * @returns {String, String} new XML to inject inside ODT reports at 2 places: the content and the style.
   */
  buildXMLContentOdt : function (styleID, descriptor = [], options, isInline = false) {
    let _content = '';
    let _style = '';
    let _styleLists = '';
    /** ------ */
    ...
    for (let i = 0, j = descriptor.length; i < j; i++) {
      ...
      if (isInline === false && _isHTMLP === 0 && _isXMLP === false && _isList === 0 && (descriptor[i].type === '' || descriptor[i].type === '#AB#') && descriptor[i].tags.includes('li') === false) {
        _isHTMLP++;
        _isXMLP = true;
        _content += '<text:p>';
      }
      ...
      if (descriptor[i].type === html.types.BREAK_LINE) {
        if (isInline === true || _isXMLP === true || _isHTMLP > 0 || _isList > 0) {
      ...
      else if (isInline === false && descriptor[i].type === html.types.PARAGRAPH_BEGIN && _isList === 0 && _isXMLA === 0) {
        if (_isHTMLP === 0) {
          _content += '<text:p>';
        }
        _isHTMLP++;
      }
      else if (isInline === false && descriptor[i].type === html.types.PARAGRAPH_END && _isList === 0  && _isXMLA === 0) {
        if (_isHTMLP === 1) {
      ...
      else if (isInline === false && (descriptor[i].type === html.types.UNORDERED_LIST_BEGIN || descriptor[i].type === html.types.ORDERED_LIST_BEGIN)) {
        let _listID = '';
      ...
      else if (isInline === false && (descriptor[i].type === html.types.UNORDERED_LIST_END || descriptor[i].type === html.types.ORDERED_LIST_END)) {
        _content += '</text:list>';
      ...
      else if (isInline === false && descriptor[i].type === html.types.LIST_ITEM_BEGIN) {
        _content += '<text:list-item>';
        if (descriptor[i + 1].type === '' || descriptor[i + 1].type === html.types.PARAGRAPH_BEGIN) {
          _content += '<text:p>';
        }
      }
      else if (isInline === false && descriptor[i].type === html.types.LIST_ITEM_END) {
      ...
      if (isInline === false && _isHTMLP > 0 && ((i + 1 === descriptor.length ) ||
