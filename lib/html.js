...
const html = {
   ...
   *
   * TODO
   * 0. [x] détecter les positions (début et fin) de tous les makeurs HTML, créer un tableau des marker HTML
   * 1. [x] reprendre le tag paragraphe avec tous les attributs
   * 2. [ ] parser les markeurs html avec le reste pour faire un descripteur [ { pos, isHtml, content } ]
   *        et faire un tableau des marker HTML déjà traité pour ne pas reboucler dessus et traiter un marker de nouveau
   * 3. [ ] à partir du descripteur reconstruire dans les nouveaux paragraphes
   * 4. [ ] supprimer le paragraphe initial et réinjecter le nouveau contenu
   *
   * Seperate HTML formatters and others markers mixed in a single paragraph into new paragraphs
   * @param {String} content XML content
   * @returns {String}
   */
   reorderXML : function (content) {
    ...
    while ((marker = patternHTML.exec(content)) !== null) {
      console.log(marker);
      const _beginTagFirst = content.lastIndexOf('<text:p', marker.index);

      if (paragraphList.includes(_beginTagFirst) === true) {
        continue;
      }

      const _endPosHtml = marker.index + marker[0].length;

      const _beginTagLast = content.indexOf('>', _beginTagFirst) + 1;
      const _endTagFirst = content.indexOf('</text:p>', _endPosHtml);
      const _endTagLast = _endTagFirst + 9;

      const _content = content.substring(_beginTagLast, _endTagFirst);
      const _startTag = content.substring(_beginTagFirst, _beginTagLast);
      const _endTag = content.substring(_endTagFirst, _endTagLast);


      console.log(_content, _startTag, _endTag);

      paragraphList.push(_beginTagFirst);
      markerList.push({
        newContent: this.createParagraphContent(_content, _startTag, _endTag),
        beginTagFirst: _beginTagFirst,
        endTagLast: _endTagLast
      });
    }

    for (let i = markerList.length -  1; i >= 0; i--) {
      console.log("Injected:", content.substring(0, markerList[i].beginTagFirst), markerList[i].newContent, content.substring(markerList[i].endTagLast));
      content = content.substring(0, markerList[i].beginTagFirst) + markerList[i].newContent + content.substring(markerList[i].endTagLast);
    }
    return content;
  },
  createParagraphContent : function(content, startTag, endTag) {
    let patternHTML = /{([^{}]*?):html[^{}]*?}/g;
    let _newContent = "";
    let _lastIndex = -1;
    while ((marker = patternHTML.exec(content)) !== null) {
      const _staticContent = content.substring(_lastIndex === -1 ? 0 : _lastIndex, marker.index)
      if (_staticContent.length > 0) {
        _newContent += `${startTag}${_staticContent}${endTag}`
      }
      _newContent += marker[0];
      _lastIndex = marker.index + marker[0].length
    };
    const _lastStaticContent = content.substring(_lastIndex, content.length);
    console.log()
    if (_lastStaticContent.length > 0) {
      _newContent += `${startTag}${_lastStaticContent}${endTag}`
    }
    console.log("NEW CONTENT:", _newContent);
    return _newContent;
  },
  /**
  ...
  preprocessODT : function (template) {
    ...
    _file.data = html.reorderXML(_file.data);
    // let patternHTML = /{([^{}]*?):html[^{}]*?}/g;
   ...
   */
  buildXMLContentOdt : function (styleID, descriptor = [], options) {
    let _content = '';
    let _style = '';
    let _styleLists = '';
    /** ------ */
    ...
    for (let i = 0, j = template.files.length; i < j; i++) {
      ...
      if (_file.name.indexOf('.xml') !== -1 && _file.name.indexOf('.rels') === -1 &&
        ...
        while ((marker = patternHTML.exec(_file.data)) !== null) {
          ...
          let k = marker.index + marker[0].length;
          /** 3 - Find the position of the next text block (end of </w:p>) to insert subcontent */
          let propertiesPosBegin = _file.data.lastIndexOf('<w:rPr>', k);
          if (propertiesPosBegin !== -1) {
            const _textProperties = _file.data.substring(propertiesPosBegin, k);
            const _regFontResult = _textProperties.match(/<w:rFonts[^]*?(w:ascii|w:hAnsi|w:cs|w:eastAsia)="([^]*?)"[^]*?\/>/);
            if (_regFontResult !== null && _regFontResult.length > 0 && _regFontResult[0] !== '' && _regFontResult[2] !== '') {
              defaultFont = _regFontResult[2];
            }
            const _regFontSizeResult = _textProperties.match(/<w:sz w:val="([^]*?)"\/>/);
            if (_regFontSizeResult !== null && _regFontSizeResult.length > 0 && _regFontSizeResult[0] !== '' && _regFontSizeResult[1] !== '') {
              defaultFontSize = _regFontSizeResult[1];
            }
          }
          ...
          if (subContentPos !== -1) {
            ...
            if (defaultFont || defaultFontSize) {
              _htmlArguments += `(${defaultFont ? '\'' + defaultFont + '\'' : 'null' }, ${defaultFontSize ? '\'' + defaultFontSize + '\'' : 'null' })`;
            }
            _file.data = helper.insertAt(_file.data, subContentPos, `{${marker[1]}:getHTMLContentDocx${_htmlArguments}}`);
            _file.data = _file.data.replace(marker[0], '');
          }
        }
      }
   ...
   */
  buildContentDOCX : function (descriptor, options, defaultFont, defaultFontSize) {
    ...
    for (let i = 0, j = descriptor.length; i < j; i++) {
      ...
      if (descriptor[i].type === html.types.BREAK_LINE) {
        ...
        if (defaultFont && defaultFont !== 'null') {
          _contentProperties += `<w:rFonts w:ascii="${defaultFont}" w:hAnsi="${defaultFont}" w:cs="${defaultFont}" w:eastAsia="${defaultFont}"/>`;
        }
        if (defaultFontSize && defaultFontSize !== 'null') {
          _contentProperties += `<w:sz w:val="${defaultFontSize}"/>`;
        }
