...
const html = {
  ...
  reorderXML : function (content, options) {
    ...
    let marker = null;
    // TODO: FOR ODT REPORT, WE MUST SUPPORT HTML INSIDE <text:p> and <text:h>
  ...
  buildXMLContentOdt : function (styleID, descriptor = [], options, defaultStyleId) {
    ...
    for (let i = 0, j = descriptor.length; i < j; i++) {
      ...
      if (_isHTMLP === 0 && _isXMLP === false && _isList === 0 && (descriptor[i].type === '' || descriptor[i].type === html.types.ANCHOR_BEGIN) && descriptor[i].tags.includes('li') === false) {
        _isHTMLP++;
        _isXMLP = true;
