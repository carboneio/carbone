  ...
  parseHTML : function (xml) {
    ...
    while (_tag !== null) {
      ...
      if ( _prevLastIndex < _tag.index) {
        // Get the content
        let _contentOnly = xml.slice(_prevLastIndex, _tag.index);
        // Remove the end comment section
        const _endCommentIndex = _isComment === true ? _contentOnly.indexOf('-->') : -1;
        if (_endCommentIndex !== -1) {
          _isComment = false;
          _contentOnly = _contentOnly.slice(_endCommentIndex + 3);
        }
        if (_isComment === false) {
          _res.push({
            content : html.convertUnsupportedXMLCharactersIntoXMLEntities(_contentOnly),
            type    : '',
      ...
      if (_tagOnly === 'br' || _tagOnly === 'br/') {
        ...
        /**
         * 2 cases possible:
         * - The comment does not contain XML tags, the comment is removed automatically
         *    - Solution: do nothing
         * - The comment DOES contain XML tags, the comment is splited over multiple tags =>
         *    - Solution: find the end comment tag and remove it
         */

        // If the comment embeded XML tag, we must skip all content until the `-->` is found
        if (_tagStr?.[_tagStr.length - 1] !== '-' && _tagStr?.[_tagStr.length - 2] !== '-' ) {
          _isComment = true;
        }
      }
    ...
    if (_prevLastIndex < xml.length) {
      // Get the content
      let _contentOnly = xml.slice(_prevLastIndex);
      // Remove the end comment section
      const _endCommentIndex = _isComment === true ? _contentOnly.indexOf('-->') : -1;
      if (_endCommentIndex !== -1) {
        _isComment = false;
