...
const html = {
   ...
   */
   reorderXML : function (content, fileType) {
    ...
    while ((marker = patternHTML.exec(content)) !== null) {
      console.log(marker);
      const _paragraphStartPos = content.lastIndexOf('<text:p', marker.index);

      if (paragraphList.includes(_paragraphStartPos) === true) {
        continue;
      }

      const _endPosHtml = marker.index + marker[0].length;
      const _paragraphEndPos = content.indexOf('</text:p>', _endPosHtml) + 9;

      paragraphList.push(_paragraphStartPos);
      markerList.push({
        newContent: this.createParagraphContent(content.substring(_paragraphStartPos, _paragraphEndPos), fileType),
    ...
    for (let i = markerList.length -  1; i >= 0; i--) {
      content = content.slice(0, markerList[i].beginTagFirst) + markerList[i].newContent + content.slice(markerList[i].endTagLast);
    }
    console.log(content);
    return content;
  },
  seperateHTMLMarker : function(content, pos, fileType) {
    ...
    while ((marker = patternHTML.exec(content)) !== null) {
      ...
      _descriptor.push({
        pos: pos,
        data: `{${marker[1]}:getHTMLContentOdt}`,
        isHTML: true
      })
  ...
  createParagraphContent : function (content, fileType) {
    ...
    for (let i = _descriptor.length - 1; i >= 0; i--) {
      ...
      if (_el.isHTML === true) {
        ...
        _tmpContent = _xmlToUse.substring(0, _el.pos) + _el.data + _xmlToUse.substring(_el.pos, _xmlToUse.length);
      }
    }
    if (_tmpContent) {
      _newContent = _tmpContent + _newContent;
  ...
  hasStaticContent : function(content) {
