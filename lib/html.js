...
const html = {
  ...
  reorderXML : function (content, options) {
    ...
    let marker = null;
    let _tagsToSearchs = options.extension === 'docx' ? ['<w:p', '</w:p>'] : ['<text:p', '</text:p>']; // default ODT
    ...
    while ((marker = _patternHTML.exec(content)) !== null) {
      ...
      const _styles = html.getTemplateDefaultStyles(_paragraph.substring(0, marker.index - _paragraphStartPos), options, content);
    ...
    for (let i = 0, j = descriptor.length; i < j; i++) {
      ...
      if (descriptor[i].type === html.types.BREAK_LINE) {
        ...
        if ((descriptor[i].tags && descriptor[i].tags.length > 0) || _defaultStylesObject.text !== '') {
          ...
          _style += `<style:style style:name="${_uniqueStyleID}" style:family="text"><style:text-properties ${_styleContent}${_defaultStylesObject.text !== '' ? ' ' + _defaultStylesObject.text : '' }/></style:style>`;
   ...
   */
   getTemplateDefaultStyles : function (paragraphStyles, options, xmlContent) {
    ...
    console.log(_defaultTemplateStylesId, _defaultTemplateStyles)
  ...
  */
  addHtmlDefaultStylesDatabase : function (options, styleID, styleObject) {
    if (options && styleID && !options.htmlStylesDatabase.has(styleID)) {
