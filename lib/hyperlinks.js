...
const BOOKMARK_DOCX_AUTO_UPDATE_MARKER = '{c.now:neutralForArrayFilter:cumCount}';
...
const hyperlinks = {
  ...
  replaceBookmarkAndTableOfContentDocx : function (str, options) {
    const _fieldRefMap = new Map();
    return str
      // here we use [\s\S]+? instead of [^>]+? because bookmarkStart can contains Carbone tags with array filters (">1" for example)
      .replace(/<w:bookmarkStart [\s\S]+?\/>/g, (bookmarkStartTag) => {
        return bookmarkStartTag
          .replace(/w:id="(\d+?)"/  , (str, id)  => {
            return `w:id="${BOOKMARK_DOCX_AUTO_UPDATE_MARKER}${id.padStart(3, '0')}"`;
          })
          .replace(/w:name="([^"]+?)"/, (str, ref) => {
            // if w:name already contains a Carbone tag (yes, it can happens in MS Word), do not add new Carbone tag. It should work like this
            if (/\{[^{]+?\}/.test(ref) === true) {
              return str;
            }
            const _newRef = `${ref}_${BOOKMARK_DOCX_AUTO_UPDATE_MARKER}`;
            _fieldRefMap.set(ref, _newRef);
            return `w:name="${_newRef}"`;
          });
      })
      // here I use [^>]+? because bookmarkEnd contains only ids and cannot contain Carbone tags with array filters ">"
      ...
      .replace(/<w:instrText[\s\S]+?<\/w:instrText>/g, (automaticField) => {
        _fieldRefMap.forEach((val, key) => {
          automaticField = automaticField.replaceAll(key, val);
        });
        // http://officeopenxml.com/WPfieldInstructions.php
        if (/NUMCHARS|NUMPAGES|NUMWORDS|PAGEREF/.test(automaticField) === true) {
          // force hard refresh if the document contains some dynamic field
          options.hardRefresh = options.hardRefreshUser ?? true;
        }
      ...
      .replace(/<w:hyperlink ([^>]+?)>[\s\S]+?<\/w:hyperlink>/g, (insideHyperlink, hyperlinkAttributes) => {
        ...
        if (_isTableOfContentHyperlink === false) {
          return insideHyperlink.replace(hyperlinkAttributes, hyperlinkAttributes.replace(/w:anchor="([^"]+?)"/, (str, ref)  => {
            const _replacedRef = _fieldRefMap.get(ref);
            return  _replacedRef ? `w:anchor="${_replacedRef}"` : str;
          }));
        }
        options.hardRefresh = options.hardRefreshUser ?? true;
    ...
    for (let i = 0, n = template.files.length; i < n; i++) {
      ...
      if (/_rels\/document.xml.rels/.test(_file.name)) {
        _file.data = _file.data.replace(_regexRels, function (xmlHyperlink, markerStart, marker) {
          if (parser.isCarboneMarker(marker?.replace('%5B', '[')) === true) {
  ...
  validateURL : function (link, defaultUrlOnError = '') {
    ...
    if (_qmarkIndex > 0) {
      ...
      /**
       * If missing a slash, the character is added to fix the URL.
       * - MS Word supports URLs without slash
       * - LibreOffice does support URLs without slash now. However, older versions didn't support it and it was printing a absolute local path "C://..".
      */
      if (_hasSlash === false && _qmarkIndex > 0) {
        link = link.substring(0, _qmarkIndex + 1) + '/' + link.substring(_qmarkIndex + 1);
      }
      else
      if (_hasSlash === false) {
        return defaultUrlOnError;
  ...
  URL_ON_ERROR : 'https://carbone.io/hyperlink-validation'
};
