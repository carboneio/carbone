...
const hyperlinks = {
  ...
  insertHyperlinksLO : function (template) {
    ...
    if (_contentFileId > -1 && !!template.files[_contentFileId] === true) {
      template.files[_contentFileId].data = template.files[_contentFileId].data.replace(/xlink:href="[^<>"]*(%7b|%7d|%257b|%257d|%7B|%7D|%257B|%257D|{|})*[^<>"]*"/g, function (attributeXlinkHref) {
         ...
         */
        return hyperlinks.decodeMarker(attributeXlinkHref).replace(/[^="{}]*{/g, '{').replace(/}([^<>{}"]*)/g, '}');
      });
    }
    return template;
  },
  ...
  validateURL : function (link) {
    ...
    console.log(link);
    /** Check the domain + extension OR the IPv4 + port with a slash at the end */
    if (/((([a-z\d]([a-z\d-]*[a-z\d])*)\.)+[a-z]{2,}|((\d{1,3}\.){3}\d{1,3}))(\:\d+)?\//.test(link) === false) {
      link = hyperlinks.URL_ON_ERROR;
    }
    try {
      url = new URL(link);
    } catch (_) {
