...
const hyperlinks = {
  ...
  insertHyperlinksLO : function (template) {
    ...
    if (_contentFileId > -1 && !!template.files[_contentFileId] === true) {
      template.files[_contentFileId].data = template.files[_contentFileId].data.replace(/<(text|draw):a[^]*?xlink:href="([^<>"]*(%7b|%7d|%257b|%257d|%7B|%7D|%257B|%257D|{|})*?[^<>"]*?)"/g, function (xml, xmlTagType, attributeXlinkHref) {
        /**
    ...
    for (let i = 0, n = template.files.length; i < n; i++) {
      ...
      if (/_rels\/sheet+\d*.xml.rels/.test(_file.name)) {
        _file.data = _file.data.replace(/Target="((http[s]?:\/\/)?[dc]\.[^"]*)"/g, function (xmlHyperlink) {
          return xmlHyperlink.replace(/Target="(http[s]?:\/\/)?/g, 'Target="{').replace(/\/?"$/g, ':validateURL}"');
        });
      }
    }
  },
  ...
  validateURL : function (link, fileType = '') {
    ...
    if (_qmarkIndex > 0) {
      let _hasSlash = false;
      while (_qmarkIndex > 0 && link[_qmarkIndex] !== '.' ) {
        if (link[_qmarkIndex] === '/') {
          _hasSlash = true;
        }
        _qmarkIndex--;
   ...
   */
  addLinkDatabase: function (options, hyperlink) {
    let _properties = {
      id : options.hyperlinkDatabase.size
    };
    if (!options.hyperlinkDatabase.has(hyperlink)) {
      // If the image doesn't exist, it create a new ID
      options.hyperlinkDatabase.set(hyperlink, _properties);
    } else {
      _properties = options.hyperlinkDatabase.get(hyperlink)
    }
    return _properties;
  },
