...
const image = {
  ...
  preProcessLo : function (template) {
    ...
    for (var i = 0; i < template.files.length; i++) {
      ...
      _file.data = _file.data.replace(_picturesRegex, function (xml, alternativeText) {
        ...
        let _newAlternativeText = alternativeText.replace(/\{([^{]+?)\}/, function (xmlMarker, marker) {
          if (parser.isCarboneMarker(marker) === true) {
            // Get and remove the imageFit formatter in all the cases
        ...
        if (_marker !== '') {
          ...
          if (_imageFit === 'contain') {
            // Markers contain: to scale the replaced content to maintain its aspect ratio while fitting within the element's content box
            xml = xml
              .replace(/svg:width="([^"]*?)cm"/g, (xmlWidth, widthCm) => xmlWidth.replace(xmlWidth, `svg:width="{${_marker}:scaleImageLo(width, ${widthCm})}"`))
              .replace(/svg:height="([^"]*?)cm"/g, (xmlHeight, heightCm) => xmlHeight.replace(xmlHeight, `svg:height="{${_marker}:scaleImageLo(height, ${heightCm})}"`));
          }
        }
        return xml;
      });
  ...
  _getImageSize (imageInfo, unit = 'px') {
    ...
    if (unit === 'emu') { // convert pixel to EMU
      ...
      console.log(imageInfo.newImageWidth, imageInfo.newImageHeight);
    }
    else { // default pixel
