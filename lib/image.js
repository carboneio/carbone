...
const image = {
  ...
  preProcessDocx : function (template) {
    ...
    const _file = template.files.find(file => {
      return file.name.includes('word/document.xml');
    });
    if (!_file) {
      debug('preprocess docx image: the file `document.xml` is not found');
      return;
    }
  ...
  postProcessDocx : function (template, data, options) {
    ...
    let _mediaPath = null;
    const _imageRels = file.getTemplateFile(template, 'word/_rels/document.xml.rels');
    if (_imageRels === null || typeof(_imageRels.data) !== 'string' ) {
    ...
    _imageRels.data = _imageRels.data.replace(/<Relationship[^<>]*?Target="([^<>]*?)\.(jpg|png|jpeg|gif|bmp|xmb|svg|webp|pbm){1}"[^<>]*?\/>/g, function (xml, p1) {
      if (p1) {
        ...
        // Remove the media reference to create a new one with a new ID
        return '';
      }
      return xml;
    });
    ...
    // console.log('folder IMAGES:', _mediaPath);
    let _imageLinks = [];
    ...
    while (_imageItem.done === false) {
      ...
      template.files.push({
        ...
        name   : _imagePath[0] !== '/' ? 'word/' + _imagePath : _imagePath,
        parent : '',
        data   : _image.data
      });
      _imageLinks.push(`<Relationship Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/image" Target="${_imagePath}" Id="rIdCarbone${_image.id}"/>`);
      _imageItem = _imageDatabaseIt.next();
    }
    _imageRels.data = _imageRels.data.replace('</Relationships>', _imageLinks.join('') + '</Relationships>');
    // console.log('FINAL:', _imageRels.data);
    return template;
  },
