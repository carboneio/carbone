...
const image = {
  ...
  preProcessDocx : function (template) {
    ...
    for (let i = 0, j = template.files.length; i < j; i++) {
      ...
      if (_file.name.indexOf('.xml') !== -1 && _file.name.indexOf('.rels') === -1 && _file.name.indexOf('Layout') === -1 && _file.name.indexOf('Master') === -1 &&
        ...
        // If it is a Shape, we must delete the fallback shape otherwise it will create a corrupted document
        if (_file.data.indexOf('<mc:Fallback') !== -1) {
          _file.data = _file.data.replace(/<mc:Fallback>[^]*?<\/mc:Fallback>/g, '');
          console.log("Pass!");
        }
        ...
        _file.data = _file.data.replace(/<w:drawing>([^]*?)<\/w:drawing>/g, function (xmlImage) {
          ...
          if (xmlImage.indexOf('<pic') === -1 && xmlImage.indexOf('<a:blip') === -1) {
            return xmlImage;
          }
  ...
  downloadImage : function (imageLinkOrBase64, data, options, callback) {
    ...
    if (/[^\u0020-\u00ff]/.test(imageLinkOrBase64) === true) {
      ...
      return callback(`Error Carbone: image URL contains unescaped characters: ${imageLinkOrBase64}`);
    }
    // HTTP Request
    rock({method : 'GET', url : imageLinkOrBase64, timeout : 6000, tenantId : options.tenantId, requestId : options.requestId }, function (err, res, body) {
      if (err) {
        return callback(err);
      }
