...
const image = {
  ...
  preProcessDocx : function (template) {
    ...
    _file.data = _file.data.replace(_regexDocxImage, function (xmlImage) {
      ...
      if (_imageMarker) {
        ...
        // Markers to resize the image with the same ratio
  ...
  _getImageSize (imageInfo) {
    ...
    imageInfo.imageWidth = Math.floor(_dimensionsPx.width * 914400 / 96); // convert pixel to EMU
    imageInfo.imageHeight = Math.floor(_dimensionsPx.height * 914400 / 96); // convert pixel to EMU
  },
  _computeImageSize (imageInfo) {
    const _newSize = image._fitDimensions({
      ...
      x : imageInfo.imageSourceWidth,
      y : imageInfo.imageSourceHeight
    });
    imageInfo.imageSourceWidth = _newSize.x;
    imageInfo.imageSourceHeight = _newSize.y;
  },
  /**
