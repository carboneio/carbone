      ...
      _file.data = _file.data.replace(_picturesRegex, function (xml, alternativeText) {
        ...
        if (_marker !== '') {
          ...
          if (_imageFit === 'contain') {
            // Markers contain: to scale the replaced content to maintain its aspect ratio while fitting within the element's content box
            xml = xml
              .replace(/svg:width="([^"]*?)(cm|in)"/g, (xmlWidth, widthCm, unit) => xmlWidth.replace(xmlWidth, `svg:width="{${_marker}:scaleImageLo(width, ${widthCm}, ${unit})}"`))
              .replace(/svg:height="([^"]*?)(cm|in)"/g, (xmlHeight, heightCm, unit) => xmlHeight.replace(xmlHeight, `svg:height="{${_marker}:scaleImageLo(height, ${heightCm}, ${unit})}"`));
          }
        }
        return xml;
      });
