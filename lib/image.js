...
const image = {
  ...
  downloadImage : function (imageLinkOrBase64, data, callback) {
    ...
    if (imageLinkOrBase64 && imageLinkOrBase64.includes('"bcid":') === true) {
      let _barcodeData = {};
      try {
        _barcodeData = JSON.parse(imageLinkOrBase64);
      }
      catch (err) {
      ...
      return bwipjs.toBuffer(_barcodeData, function (err, png) {
        if (err) {
          ...
          return callback(null, {
            ...
            extension : 'png',
            mimetype  : image.getMimeTypeFromExtensionImage('png')
          });
        }
      });
    }
