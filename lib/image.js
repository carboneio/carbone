  ...
  updateImageDatabase : function (data, options, callback) {
    ...
    function walkImage () {
      ...
      image.downloadImage (_imageURLBase64, data, (err, imageInfo) => {
        if (err) {
          debug(err);
          // If something went wrong, the picture is replaced by 'invalid-image.jpg'
          var invalidImage;
          try {
            invalidImage = fs.readFileSync(path.join(__dirname, '/assets/invalid-image.jpg'));
          }
          catch (err) {
            debug("Can't read the file 'invalid-image.jpg': " + err);
          }
          imageInfo = {
            data      : invalidImage,
            extension : 'jpg',
