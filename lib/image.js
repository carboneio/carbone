    ...
    for (let i = 0, j = template.files.length; i < j; i++) {
      ...
      if (_file.name.indexOf('.xml') !== -1 && _file.name.indexOf('.rels') === -1 &&
        ...
        _file.data = _file.data.replace(/<w:drawing>([^]*?)<\/w:drawing>/g, function (xmlImage) {
          ...
          // <w:drawing> can be a chart, a table, a diagram, ...
          // So if it is not an image, leave immediately.
          // https://social.msdn.microsoft.com/Forums/office/en-US/7c1415d9-f28d-4256-83ea-d8ebd6b9035e/why-is-the-uri-required-for-graphicdata-element?forum=oxmlsdk
          if (xmlImage.indexOf('<pic') === -1) {
            return xmlImage;
          }
  ...
  _computeImageSize (imageInfo) {
    ...
    */
    if (!imageInfo?.imageWidth || !imageInfo?.imageHeight ||
      ...
      /** newImageWidth/newImageHeight contains the size of the downloaded image */
      ...
      if (_maxSize === imageInfo.imageWidth && imageInfo.imageWidth > _baseUnitValue) {
        imageInfo.imageWidth = _baseUnitValue;
        imageInfo.imageHeight = _baseUnitValue * imageInfo.newImageHeight / imageInfo.newImageWidth;
      }
      if (_maxSize === imageInfo.imageHeight && imageInfo.imageHeight > _baseUnitValue) {
        imageInfo.imageWidth = _baseUnitValue * imageInfo.newImageWidth / imageInfo.newImageHeight;
        imageInfo.imageHeight = _baseUnitValue;
      }
    }
