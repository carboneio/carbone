...
const image = {
  ...
  _isImageListAnchorTypeBlockLO : function (xmlImage, marker) {
    ...
    if (_anchorType === null) {
      console.error('Carbone warning: the template contains a list of images, it is not supported and may break the report.');
      return false;
    }
    else if (_anchorType[1] !== 'as-char') {
      console.error(`Carbone warning: the template contains a list of floating images, you must change the images anchor-type to "as character" where the marker "${marker}" is bound.`);
      return false;
    }
    return true;
  },
