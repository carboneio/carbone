...
const image = {
  ...
  postProcessXlsx : function (template, data, options) {
    ...
    for (let i = 0, n = template.files.length; i < n; i++) {
      ...
      if (_resultRegex && _resultRegex[0] && _resultRegex[1] && isNaN(_resultRegex[1]) === false) {
        ...
        if (!_mediaPath) {
          debug('the media folder for the images has not been found, default set to media/');
          _mediaPath = 'media/'
          // continue;
        }
        let _imageLinks = [];
    ...
    for (let i = 0, j = template.files.length; i < j; i++) {
      ...
      while (_imageItem.done === false) {
        ...
        if (image.checkIfImageIncludedDocx(_file.name, _image.sheetIds)) {
          ...
          console.log(_imagePathForTemplateFiles)
          ...
          if (file.getTemplateFile(_addImageToZip, _imagePathForTemplateFiles) === null) {
            ...
            console.log("PUSH", _imagePathForTemplateFiles)
          }
          _imageLinks.push(`<Relationship Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/image" Target="${_imagePath}" Id="${image.getImageReference(_image.id)}"/>`);
  ...
  _computeImageSize (imageInfo) {
    if (!imageInfo?.imageWidth || !imageInfo.imageHeight ||
         imageInfo?.imageWidth === -1 || imageInfo?.imageHeight === -1) {
      /*
       * Happenning when the image is coming the HTML marker and the width or height attribute is missing
       * No default image canva (temporary image) is available
       * 17cm is 21cm maximum page width or height of an A4 page MINUS 4cm or padding
       */
      imageInfo.imageWidth = imageInfo.newImageWidth;
      imageInfo.imageHeight = imageInfo.newImageHeight;
      let _maxSize = Math.max(imageInfo.imageWidth, imageInfo.imageHeight);
      if (_maxSize === imageInfo.imageWidth && imageInfo.imageWidth > 17) {
        imageInfo.imageWidth = 17;
        imageInfo.imageHeight = 17 * imageInfo.newImageHeight / imageInfo.newImageWidth;
      }
      if (_maxSize === imageInfo.imageHeight && imageInfo.imageHeight > 17) {
        imageInfo.imageWidth = 17 * imageInfo.newImageWidth / imageInfo.newImageHeight;
        imageInfo.imageHeight = 17;
      }
    }
    /** Fallback if the downloaded ressource is not an image and it is coming from the HTML formatter */
    if (!imageInfo?.imageWidth || !imageInfo?.imageHeight ||
        imageInfo.imageWidth === -1 || imageInfo.imageHeight === -1) {
  ...
  downloadImage : function (imageLinkOrBase64, data, options, callback) {
    if (imageLinkOrBase64?.type === 'echarts@v5') {
