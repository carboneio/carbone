...
/*****************************************************************************************************************/
  ...
  function zip(sourceDir, destFile, callback){
    ...
    if(typeof destFile === 'function'){
      callback = destFile;
      _toStdout = true;
      _zipPath = path.resolve('temp', (new Date()).valueOf() + '.docx');
    }

    /* Following line does not word yet with NodeJS but is the real command for terminal to get content of zipped file in STDOUT */
