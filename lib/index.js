...
var carbone = {
  ...
  /**
   * @description It finds and returns a list of translation markers "{t  }" from a directory which contains templates.
   * @param {String} dirPath absolute path to a directory
   * @param {Function} callback (err, result) result is an Array of String (translation markers)
   */
  getTranslationMarkersFromDir : function  (dirPath, callback) {
    if (typeof callback !== 'function') {
      return 'Error getTranslationMarkersFromDir: the second argument (callback function) is missing.';
    }
    else if (typeof dirPath !== 'string') {
      callback('Error getTranslationMarkersFromDir: the first argument (path of the directory) is not a string.', []);
    }
    else if (path.isAbsolute(dirPath) === false) {
      callback('Error getTranslationMarkersFromDir: the first argument (path of the directory) is not an absolute path.', []);
    }
    else if (fs.existsSync(dirPath) === false) {
      callback('Error getTranslationMarkersFromDir: the path of the directory does not exist.', []);
    }
    else {
      const _files = helper.walkDirSync(dirPath);
      if (_files && _files.length > 0) {
        translator.parseFiles(_files, [], function (err, resp) {
          if (err) {
            callback(err, []);
          }
          else {
        ...
        callback('Warning getTranslationMarkersFromDir: nothing has been found on the directory.', []);
      }
    }
  }
};
