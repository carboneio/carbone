...
function batchProcess (template, data, options, finalCallback, fnToExecute) {
  ...
  let   _counter = 0;
  const _items = data[options.splitReportOn.replace('d.', '')];
  ...
  function executeItem (item, itemCallback) {
    ...
    _initialOptions.renderPrefix       += _counter++;
    _initialOptions.formatters         = options.formatters;
    fnToExecute(_initialTemplate, item, _initialOptions, (err, filePath) => {
  ...
  function allItemExecuted (err) {
    ...
    console.log(' |- End batch for tenantId '+options.tenantId + ' rid: ' + options.requestId);
  ...
  helper.genericQueue(_items, executeItem, allItemExecuted, allItemExecuted, {stopOnError : true}).start();
}
