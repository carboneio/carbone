...
var carbone = {
   ...
   * @param {Object} formatters {toInt: function(d, args, agrs, ...)}
   */
  generateOutputFilename : function (finalReportName, extension) {
    var _randomName = helper.getRandomString();
    if (!finalReportName) {
      finalReportName = 'report';
    }
    var _filename = _randomName + '_' + encodeURIComponent(`${finalReportName}.${extension}`);
    return path.join(params.tempPath, _filename);
  },
   ...
   *                          'isBufferOutput' : (default: true) return a path instead of a buffer
   *                       }
          ...
          walkFiles(template, data, options, 0, function (err, report) {
            ...
            postprocessor.execute(template, data, options, function () {
              ...
              file.buildFile(report, function (err, result) {
                ...
                var _outputFilename = '';
                if (optionsRaw.isBufferOutput === false) {
                //TODO manage the case whete options.convertTo is an object
                  _outputFilename = carbone.generateOutputFilename(report.reportName, options.convertTo || options.extension);
                }
                if (typeof(options.convertTo) === 'string' && options.convertTo !== '') {
                  ...
                  convert(result, options.convertTo, optionsConvert, function (err, result) {
                    callback(err, result, report.reportName, options.convertTo);
                  });
                }
                  ...
                  convert(result, options.convertTo.formatName, optionsConvert, function (err, result) {
                    callback(err, result, report.reportName, options.convertTo.formatName);
                  });
                }
                else {
                  if (options.outputFilePath) {
                    return fs.writeFile(options.outputFilePath, data, function (err) {
                      callback(err, options.outputFilePath, report.reportName);
                    });
                  }
                  callback(null, result, report.reportName, options.extension);
                }
              });
            });
          });
