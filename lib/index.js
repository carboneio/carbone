  ...
  fs.writeFile(_filenameInput, data, function(err){
    ...
    if(params.delegateToServer===true && clientSocket !== null){
      ...
      clientSocket.send(_job, function(err, res){
        if(err || res.error){
          fs.unlink(_filenameInput, function (err){
            if(err){
              console.log('Carbone: Could not delete file '+_filenameInput);
            }
            return callback('Error while converting document');
          });
        }
        else {
          fs.readFile(_filenameOutput, function(errRead, data){
            fs.unlink(_filenameInput); // TODO No callback, very BAD
      ...
      converter.convertFile(_filenameInput, _fileFormat, _formatOptions, function(err, data){
