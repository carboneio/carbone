...
var carbone = {
  ...
  reset : function () {
    ...
    params.maxTemplateSize         = 20 * 1024 * 1024;
    params.maxDownloadFileCount     = 20;
    params.maxDownloadFileSizeTotal = 10 * 1024 * 1024;
    ...
    params.egressUserAgent         = 'carbone-ee';
    params.egressTenantFilter      = [];
    params.egressExcludeFilterMode = true;
    params.egressMaxRetry          = 0;
    params.egressProxyPrimary      = '';
    params.egressProxySecondary    = '';
    params.egressProxyPath         = '{path}';
  },
   ...
   * @param {Boolean} isEnabled   Can be used to disable a formatter. If false, the builder will return an error that the formater exists but is disbaled.
   */
  addFormatters : function (customFormatters, isEnabled = true) {
    for (var f in customFormatters) {
      /* generateImageDocxGlobalId is a private formatter of image, mandatory for most DOCX template, even in Community Edition */
      input.formatters[f] = (isEnabled === true || f === 'generateImageDocxGlobalId') ? customFormatters[f] : false;
    }
  },
  ...
  renderXML : function (xml, data, optionsRaw, callbackRaw) {
    ...
    const _file = {
      ...
      embeddings : [],
      files      : [
        { name : 'inline.xml', parent : '', data : xml, isMarked : true }
      ]
    };
    that.render(_file, data, optionsRaw, callbackRaw);
  },
  ...
  render : function (templatePath, data, optionsRaw, callbackRaw) {
    input.parseOptions(optionsRaw, callbackRaw, function (options, callback) {
      const _renderTime = options.metrics.time('renderTime');
      ...
      file.openTemplate(templatePath, function (err, template) {
        ...
        const _preProcessTime = options.metrics.time('preProcessTime');
        preprocessor.execute(template, options, function (err, template) {
          _preProcessTime.timeEnd();
          if (err) {
            return callback(err, null);
          }
          batchProcess(template, data, options, callback, (template, data, options, callback) => {
            options.metrics.add('batchSize', 1);
            // parse all files of the template
            walkFiles(template, data, options, 0, function (err, report) {
              if (err) {
                return callback(err, null);
              }
              const _postProcessTime = options.metrics.time('postProcessTime');
              postprocessor.execute(template, data, options, function () {
                _postProcessTime.timeEnd();
                ...
                file.buildFile(report, function (err, result) {
                  ...
                  convert (result, report.reportName, options, function (err, bufferOrFile) {
                    ...
                    callback(err, bufferOrFile, report.reportName, options.debugInfo);
                  });
                });
              });
            });
...
function convert (inputFileBuffer, customReportName, options, callback) {
  ...
  fs.writeFile(_intermediateFile, inputFileBuffer, function (err) {
    ...
    const _convertTime = options.metrics.time('convertTime');
    converter.convertFile(_intermediateFile, _convertTo.format, _convertTo.optionsStr, _renderFile, function (errConvert, outputFile) {
      ...
      file.mergePDF(outputFile, options.fileDatabase, outputFile, (err) => {
        if (err) {
          return callback(err, null);
        }
...
function batchProcess (template, data, options, finalCallback, fnToExecute) {
  let _items;
  let _subArrayName = '';

  options.batchSplitBy = options.batchSplitBy.trim();
  if (!options.batchSplitBy) {
    // normal use case -> no batch
    return fnToExecute(template, data, options, finalCallback);
  }
  if (options.batchSplitBy === 'd') {
    _items = data;
  }
  else if (/^d\.[^.[\]]+$/.test(options.batchSplitBy) === true ) {
    _subArrayName = options.batchSplitBy.replace('d.', '');
    _items = data[_subArrayName];
  }
  else {
    return finalCallback('batchSplitBy accepts only "d" or "d.myArrayName" format', null);
  }
  if (!(_items instanceof Array) || _items.length === 0) {
    return finalCallback(`"${options.batchSplitBy}" must be a non empty array`, null);
  }
  if (params.nbReportMaxPerBatch === 0) {
    return finalCallback('Batch processing deactivated. nbReportMaxPerBatch = 0', null);
  }
  if (_items.length > params.nbReportMaxPerBatch) {
    return finalCallback(`Cannot process more than ${params.nbReportMaxPerBatch} documents`, null);
  }
  if (!options.renderPrefix) {
    return finalCallback('Cannot use batch processing in buffer mode', null);
  }

  const _allGeneratedFiles = [];
  // generate a unique random & safe filename
  const _renderPrefix     = (options.renderPrefix || '').replace(/[^0-9a-z-]/gi, '');
  const _randomNamePart   = helper.getRandomString();
  const _customReportName = 'batch';
  const _renderFilename   = _renderPrefix + _randomNamePart + helper.encodeSafeFilename(_customReportName) + '.zip';
  const _renderFile       = path.join(params.renderPath, _renderFilename);

  let   _counter          = _items.length;
  const _cloudLog         =  (options.tenantId ? ` uid=${options.tenantId}-${options.requestId}` : '');

  const TEMPLATE_TYPE_ARRAY = 1;
  const TEMPLATE_TYPE_OBJECT = 0;
  let _templateType = TEMPLATE_TYPE_OBJECT;
  const _firstTagFound = preprocessor.findFirstTag(template);
  if (_firstTagFound.startsWith(options.batchSplitBy + '[') === true) {
    _templateType = TEMPLATE_TYPE_ARRAY;
  }

  console.log('Starting batchSplitBy '+ options.batchSplitBy + _cloudLog);

  function executeItem (item, itemCallback) {
    console.log(' |- batch ' + (_counter--) + _cloudLog);
    let _newData = item;
    if (_templateType === TEMPLATE_TYPE_ARRAY) {
      _newData = _subArrayName === '' ? [item] : { [_subArrayName] : [item] };
    }
    const _initialOptions              = JSON.parse(JSON.stringify(options));
    const _initialTemplate             = JSON.parse(JSON.stringify(template));
    _initialOptions.imageDatabase      = new Map(options.imageDatabase);
    _initialOptions.colorDatabase      = new Map(options.colorDatabase);
    _initialOptions.fileDatabase       = new Map(options.fileDatabase);
    _initialOptions.colorDatabaseNew   = new Map(options.colorDatabaseNew);
    _initialOptions.chartDatabase      = new Map(options.chartDatabase);
    _initialOptions.htmlDatabase       = new Map(options.htmlDatabase);
    _initialOptions.htmlStylesDatabase = new Map(options.htmlStylesDatabase);
    _initialOptions.hyperlinkDatabase  = new Map(options.hyperlinkDatabase);
    _initialOptions.aggDatabase        = new Map(options.aggDatabase);
    _initialOptions.metrics            = options.metrics; // keep same metrics;
    _initialOptions.formatters         = options.formatters;
    fnToExecute(_initialTemplate, _newData, _initialOptions, (err, filePath) => {
      _allGeneratedFiles.push(filePath);
      itemCallback(err);
    });
  }

  function allItemExecuted (err) {
    if (err) {
      file.deleteFiles(_allGeneratedFiles);
      return finalCallback(err, null);
    }
    console.log(' |- End batch '+ _cloudLog + ' renderId= ' + path.basename(_renderFile));
    file.batchZip (_allGeneratedFiles, _renderFile, (err) => {
      file.deleteFiles(_allGeneratedFiles);
      if (err) {
        return finalCallback(err, null);
      }
      finalCallback(null, _renderFile);
    });
  }

  // keep one factory for other job
  const _parallelJob = params.factories > 1 ? params.factories - 1  : params.factories;

  // allItemExecuted Callback called once
  helper.genericQueue(_items, executeItem, allItemExecuted, _parallelJob ).start();
}
...
carbone.addFormatters(require('../formatters/aggregator.js') , params.isEnterpriseEditionMode);
carbone.addFormatters(require('../formatters/barcode.js')    , params.isEnterpriseEditionMode);
carbone.addFormatters(require('../formatters/image.js')      , params.isEnterpriseEditionMode);
carbone.addFormatters(require('../formatters/file.js')       , params.isEnterpriseEditionMode);
carbone.addFormatters(require('../formatters/chart.js')      , params.isEnterpriseEditionMode);
carbone.addFormatters(require('../formatters/color.js')      , params.isEnterpriseEditionMode);
carbone.addFormatters(require('../formatters/html.js')       , params.isEnterpriseEditionMode);
carbone.addFormatters(require('../formatters/hyperlinks.js') , params.isEnterpriseEditionMode);
carbone.addFormatters(require('../formatters/form.js')       , params.isEnterpriseEditionMode);
translator.loadTranslations(params.templatePath);
