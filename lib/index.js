...
var carbone = {
  ...
  buildXML : function(xml, data){
    ...
    _descriptor = extracter.reOrderHierarchy(_descriptor);
    var _builder = builder.getBuilderFunction(_descriptor);
    //console.log('\n\n\n');
    //console.log(JSON.stringify(_builder.toString,null,2));
    //console.log('\n\n\n');
    var _xmlParts = _builder(data, that.formatters);
    var _xmlResult = builder.assembleXmlParts(_xmlParts, 5); //TODO -> adapt the depth of the sort according to the maximum depth in _xmlParts
    return _xmlResult;
  },
    ...
    prepareCarbone(templatePath, function(isZipped){
      if(isZipped){
        ...
        fs.readFile(templatePath, 'utf8', function(err, content){
          var _newContent = that.buildXML(content, data);
