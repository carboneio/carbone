...
var carbone = {
  ...
  renderXML : function (xml, data, optionsRaw, callbackRaw) {
    input.parseOptions(optionsRaw, callbackRaw, function (options, callback) {
      ...
      statistics.startRecording(options, 'preParseXML');
      xml = preprocessor.preParseXML(xml, options);
      statistics.endRecording(options, 'preParseXML');
      return builder.buildXML(xml, data, options, (err, result) => {
        statistics.endRecording(options, 'executionTime');
        return callback(err, result, statistics.formatResult(options.carboneStatistics));
      });
    });
  },
          ...
          walkFiles(template, data, options, 0, function (err, report) {
            ...
            file.buildFile(report, function (err, result) {
              ...
              convert (result, report.reportName, options, function (err, bufferOrFile) {
                statistics.endRecording(options, 'executionTime');
                callback(err, bufferOrFile, report.reportName, statistics.formatResult(options.carboneStatistics));
              });
            });
          });
        });
