...
function batchProcess (template, data, options, finalCallback, fnToExecute) {
  ...
  if (options.splitReportOn?.startsWith('d.') === false ) {
    return finalCallback('splitReportOn accept only "d.myArrayName" format', null);
  }
  const _objectName = options.splitReportOn.replace('d.', '');
  const _items = data[_objectName];
