...
function batchProcess (template, data, options, finalCallback, fnToExecute) {
  if (!options.splitReportOn) {
    return fnToExecute(template, data, options, finalCallback);
  }
  if (options.splitReportOn !== 'd' || /^d.[^\.\[\]]+$/.test(options.splitReportOn) === false ) {
    return finalCallback('splitReportOn accepts only "d" or "d.myArrayName" format', null);
  }
  let _items;
  let _subArrayName = '';
  if (options.splitReportOn === 'd') {
    _items = data;
    if (!(_items instanceof Array)) {
      return finalCallback('Bad splitReportOn, the data (d) must be an array', null);
    }
  }
  else if ( /^d.[^\.\[\]]+$/.test(options.splitReportOn) === true ) {
    _subArrayName = options.splitReportOn.replace('d.', '');
    _items = data[_subArrayName];
    if (!(_items instanceof Array)) {
      return finalCallback(`Bad splitReportOn, "d.${options.splitReportOn}" must be an array`, null);
    }
  }

  const _allGeneratedFiles = [];
  if (_items === undefined ||  _items.length === 0) {
    return finalCallback('Undefined object list', null);
  }
  if (!options.renderPrefix) {
    return finalCallback('Cannot use batch processing in this mode', null);
  }
  ...
  console.log('Starting batch mode for tenantId '+options.tenantId + ' rid: ' + options.requestId);
  function executeItem (item, itemCallback) {
    console.log(' |- batch ' + _counter + ' rid: ' + options.requestId);
    let _newData = item;
    if (options.splitTemplateType === 'array') {
  ...
  function allItemExecuted (err) {
    ...
    console.log(' |- End batch for tenantId '+options.tenantId + ' rid: ' + options.requestId + ' renderId= ' + path.basename(_renderFile));
  ...
  helper.genericQueue(_items, executeItem, allItemExecuted, params.factories ).start();
}
