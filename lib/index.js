...
var carbone = {
  ...
  addFormatters : function(formatters){
    for(var f in formatters){
      carbone.customFormatters[f] = formatters[f];
    }
  },

  buildXML : function(xml, data, complement){
    var that = this;
    var _separateMarkersAndXML = parser.findMarkers(xml);
    if(_separateMarkersAndXML.markers.length===0){
      return xml;
    }
    
    var _dynamicDescriptor = extracter.splitMarkers(_separateMarkersAndXML.markers);
    //console.log('\n\n\n');
    //console.log(JSON.stringify(_dynamicDescriptor,null,2));
    //console.log('\n\n\n');
    var _descriptor = extracter.splitXml(_separateMarkersAndXML.xml, _dynamicDescriptor);
    ...
    var _obj = {
      'd' : data,
      'c' : (typeof complement !== 'function') ? complement : undefined
    };
    var _xmlParts = _builder(_obj, that.formatters);
    //console.log('\n\n\n');
    //console.log(JSON.stringify(_xmlParts,null,2));
    //console.log('\n\n\n');
  ...
  renderPDF : function(templatePath, data, complement, callback){
    if(typeof complement === 'function') {
      callback = complement;
      complement = undefined;
    }
    carbone.render(templatePath, data, complement, function(content){
  ...
  function cacheFormatters(callback){
    ...
    fs.readdir(_formattersPath, function(err, files){
      for (var i = files.length - 1; i >= 0; i--) {
        ...
        for(var f in carbone.customFormatters){
          carbone.formatters[f] = carbone.customFormatters[f];
        }
      }
      return callback();
    });
