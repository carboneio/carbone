...
var carbone = {
   ...
   * @param {Object} formatters {toInt: function(d, args, agrs, ...)}
   */
  generateOutputFilename : function (renderPath, renderPrefix, finalReportName, extension) {
    renderPrefix = renderPrefix || '';
    if (!finalReportName) {
      finalReportName = 'report';
    }
    var _filename = renderPrefix + encodeURIComponent(`${finalReportName}.${extension}`);
    return path.join(renderPath, _filename);
  },

  /**
   * Return true if the template is already cached by Carbone.
   *
   * Carbone.render must be called "just after" in order to use this cache. Otherwise, the cache could
   * be removed in a next NodeJS event loop.
   *
   * @param      {String}    templatePath  The template path
   * @return     {boolean}   True if template in cache, False otherwise.
   */
  isTemplateInCache : function (templatePath) {
    return false;
  },

  delTemplateCache : function (templatePath) {
    return true;
  },
   ...
   *                          'renderPath'   : If defined, it return a path instead of a buffer
  ...
  render : function (templatePath, data, optionsRaw, callbackRaw) {
    // if(templatePath == notInCache) {
    //   
    //   vaChercher(templatePath, donneMoiiUnStream)
    // }
    parseOptions(optionsRaw, callbackRaw, function (options, callback) {
          ...
          walkFiles(template, data, options, 0, function (err, report) {
            ...
            postprocessor.execute(template, data, options, function () {
              ...
              file.buildFile(report, function (err, result) {
                ...
                var _outputFilename = '';
                var _reportName = null;
                if (optionsRaw.renderPath !== null && optionsRaw.renderPath !== undefined && optionsRaw.renderPath.length > 0) {
                  // TODO manage the case whete options.convertTo is an object
                  _outputFilename = carbone.generateOutputFilename(optionsRaw.renderPath, optionsRaw.renderPrefix, report.reportName, options.convertTo || options.extension);
                  _reportName = path.basename(_outputFilename);
                }
                if (typeof(options.convertTo) === 'string' && options.convertTo !== '') {
                  ...
                  convert(result, options.convertTo, optionsConvert, function (err, result) {
                    callback(err, result, _reportName || report.reportName);
                  });
                }
                  ...
                  if (_outputFilename.length > 0) {
                    return fs.writeFile(_outputFilename, result, function (err) {
                      callback(err, _outputFilename, _reportName || report.reportName);
                    });
                  }
...
function parseOptions (options, callbackFn, callback) {
  ...
  parser.findVariables(options.variableStr, function (err, str, variables) {
    var _options = {
      ...
      colorDatabase     : new Map(), // dynamic colors
 ...
 *                                  outputFilename : '' write this file
 *                                }
...
function convert (data, convertTo, options, callback) {
  ...
  var _exportFilters = _fileFormatOutput.indexOf('pdf') !== -1 ? options.formatOptions : {};
    ...
    fs.writeFile(_filePathInput, data, function (err) {
      ...
      converter.convertFile(_filePathInput, _fileFormatOutput, _formatOptions, function (errConvert, data) {
        ...
        if (options.outputFilename) {
          return fs.writeFile(options.outputFilename, data, function (err) {
            callback(err, options.outputFilename);
          });
        }
        callback(errConvert, data);
      }, null, null, _exportFilters);
    });
  }
}
