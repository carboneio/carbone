    ...
    fs.readdir(_formattersPath, function(err, files){
      for (var i = files.length - 1; i >= 0; i--) {
        ...
        for(var f in _formatters){
          /*(function(fn){*/
            carbone.formatters[f] = _formatters[f];/*function(){
              var _fnArguments = arguments;
              var _arguments = [];
              Object.keys(_fnArguments).map(function(key){
                _arguments.push(_fnArguments[key]);
              });
              return fn.apply(_arguments[0], _arguments.slice(1));
            };
          })(_formatters[f]);*/
        }
      }
