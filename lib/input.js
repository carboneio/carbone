...
function parseOptions (userDefinedOptions, callbackFn, callback) {
  ...
  parser.findVariables(userDefinedOptions.variableStr, function (err, str, variables) {
    const _options = {
      ...
      hardRefresh       : userDefinedOptions.hardRefresh || false,       // Can be forced to true if DOCX contains Table Of Content (see lib/hyperlinks.js)
      hardRefreshUser   : userDefinedOptions.hardRefresh,                // Keep user defined hardRefresh to respect his choice, even if the DOCX contains a Table of content
      isDebugActive     : userDefinedOptions.isDebugActive || false,     // show info about the template
      useHighPrecisionArithmetic : userDefinedOptions.useHighPrecisionArithmetic || false,  // use arbitrary-precision decimal arithmetic
      uniqueId          : 1,         // global counter, used to generate a unique id for images for example.
      imageDatabase     : new Map(), // dynamic images
      fileDatabase      : new Map(), // pdf to merge.
      colorDatabase     : new Map(), // dynamic colors created by bindColor
      colorDatabaseNew  : new Map(), // dynamic colors created by :color
      colorStyleAgg     : [],        // temporary variable to store all :color applied on the style style/paragraph/cell
      colorStyleDatabase: {},        // styles that will be customized by :color formatter
      chartDatabase     : new Map(), // dynamic charts
      htmlDatabase      : new Map(), // dynamic html
      htmlStylesDatabase: new Map(), // Map to save default applied style from templates
      hyperlinkDatabase : new Map(), // dynamic hyperlinks
      renderPrefix      : userDefinedOptions.renderPrefix,
      batchSplitBy      : userDefinedOptions.batchSplitBy || '',
      debugInfo         : { markers : [], sample : {} }, // filled and returned after POST /render if isDebugActive === true
      aggDatabase       : new Map(),
      // only used by webserver to monitor egress traffic
      tenantId          : userDefinedOptions.tenantId,
      requestId         : userDefinedOptions.requestId,
      // used by builder
      tempSample        : {},  // used internally to build debugInfo.sample
      stopPropagation   : false,
      isConditionTrue   : null,
      isAndOperator     : null,
      isHidden          : null,
      id                : 0,
      preReleaseFeatureIn : userDefinedOptions.preReleaseFeatureIn || params.preReleaseFeatureIn
    };
    _options.debugInfo.metrics = _options.metrics.get(); // add a reference in debugInfo object
...
function parseConvertTo (options, userDefinedConvertTo) {
  ...
  if (_fileFormatOutput === null) {
    if (_extensionOutput !== options.extension) {
      return `Format "${options.extension}" can't be converted to "${_extensionOutput}".`;
    }
    else if (options.hardRefresh === true) {
      options.hardRefresh = false; // ignore hardRefresh if the file is unknown from LibreOffice (xml to xml)
    }
  }
...
function checkAndSetOptionsForPDF (userDefinedPDFFilters, convertToObj) {
  ...
  if (userDefinedPDFFilters.UseLosslessCompression === undefined) {
    userDefinedPDFFilters.UseLosslessCompression = true;
  }
