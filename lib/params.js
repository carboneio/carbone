...
module.exports  = {
  ...
  studio                  : false,
  studioUser              : '',               // Format: "admin:password". Deactivate studio basic authentication if empty or undefined
  maxDataSize             : 60 * 1024 * 1024, // Default: 60MB for rendering
  maxTemplateSize         : 20 * 1024 * 1024, // Default: 20MB
  /* Maximum volume of files downloaded from external URL (used by appendFile formatters) */
  maxDownloadFileCount    : 20,
  /* Total maximum size limit for all downloaded files combined in bytes (used by appendFile formatters) */
  maxDownloadFileSizeTotal : 10 * 1024 * 1024,
  /* Template path */
  ...
  renderPathRetention     : 50,
  renderFilePrefixLength  : 0,
  /* Time interval to clean the `/template` and `/render` directories in minutes */
  cleanIntervalTime       : 10,
  ...
  /* Max number of report in a batch when batchSplitBy is used. If 0, batch processing is disable and return an error */
  nbReportMaxPerBatch     : 0,
  /* Enable XLSM file support */
  xlsmEnabled             : false,
  // bit field to activate securoty feature.
  securityLevel           : 0,
  /* Activate Enterprise Edition features. Is Forced to false at webserver startup  */
  isEnterpriseEditionMode : true,
  /* Activate Collection of Usage Statistics */
  ...
  // Max repetition factor in loops. Ex. `{d[i+1*qty]`
  maxRepetitionFactor     : 400,
  // Available variables:
  //   - {protocol}  : https / http
