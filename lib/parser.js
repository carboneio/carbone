...
var parser = {
    ...
    while ((_tag = _similarTag.exec(leftSideXml)) !== null){
      if(_tag[1]==='/'){
        ...
        if(_tag.index<indexWhereToStopSearch){
          _lastOpeningTagIndexBeforeStop[_tagCount] = _tag.index;
        }
   ...
   * @return {integer} index position in the xml string
   */
  findClosingTagPosition : function(rightSideXml, openingTagName, indexWhereToStartSearch){
  ...
  findPivot : function(partialXml){
    ...
    if(_highestTags.length<2 || _highestTags[0].type==='<'){
      return null;
    }
    var _firstTag = _highestTags[0];
    var _lastTag = _highestTags[_highestTags.length-1];
    var _pivot = {
      'part1End'  : {'tag': _firstTag.tag, 'pos': _highestTags[1].pos },
  ...
  findRepetitionPosition : function(xml, pivot, roughStartIndex){
    ...
    var _startTagPos = _that.findOpeningTagPosition(_leftSideXml, pivot.part1End.tag, roughStartIndex);
    // Second part
    var _rightSideXml = xml.slice(pivot.part2Start.pos);
    var _endTagPos = _that.findClosingTagPosition(_rightSideXml, pivot.part2Start.tag);
