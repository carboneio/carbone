...
var parser = {
  ...
  findMarkers : function(xml, callback){
    ...
    var _cleanedXml = xml.replace(/(\r\n|\n|\r)/g,' ').replace(/\{([\s\S]+?)\}/g, function(m, text, offset) {
      ...
      var _obj = {
        ...
        'name' : '_root.'+that.cleanMarker(_marker)
      };
   ...
   * @param  {Object}   existingVariables   existing variables
  ...
  findVariables : function(xml, existingVariables, callback){
    ...
    var _cleanedXml = xml.replace(/\{((?:\s*<[^>]*>\s*)*#[\s\S]+?)\}/g, function(m, text, offset) {
      var _marker = parser.extractMarker(text);
      var _xmlOnly = parser.cleanXml(text);
      var _variableStr = _marker.replace(/[\n\t]/g, '').replace(/&lt;/g, '<').replace(/&gt;/g, '>');
      //find pattern #myVar($a,$b) menu[$a+$b]
      var _pattern = /#\s*([\S]+?)\s*(?:\(([\s\S]+?)\))?\s+([\s\S]+?)$/g.exec(_variableStr);
      if(_pattern instanceof Array){
        ...
        if(_paramStr !== undefined){
          ...
          for (var p = 0; p < _params.length; p++) {
            ...
            _code = _code.replace(new RegExp('\\'+_param,'g'), '_$'+p+'_');
          };
        }
        var _regex = new RegExp('\\$'+_variable+'(?:\\(([\\s\\S]+?)\\))?', 'g');
        existingVariables[_variable] = {
          'code' : _code,
          'regex' : _regex
        };
        return _xmlOnly;
  ...
  /**
   * Detect used variables and update all markers accordingly
   * @param {Array} markers : all markers of the document
   * @param {Object} variables : all declared variables of the document
   */
  preprocessMarkers : function(markers, variables, callback){
    if(variables !== undefined && variables !== {} ){
      ...
      for (var i = 0; i < markers.length; i++) {
        ...
        //is the marker contain a variable?
        if(_markerName.indexOf('$') !== -1){
          //check if it matches with one previously declared variable
          for(var _varName in variables){
            var _variable = variables[_varName];
            var _pattern = [];
