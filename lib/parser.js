...
var parser = {
  ...
  /**
   * Assign loop IDs for rowNumber formatter
   *
   * @param   {Array}  markers  Array of markers
   * @return  {Array}           Array of markers with loop ID in rowNumber parameters
   */
  assignLoopId : function (markers) {
    var _loopWithRowNumberRegex = /.*?:rowNumber(\((.*?)\))?/g;
    var match;

    for (var _key in markers) {
      var _marker = markers[_key];

      // If the marker has a rowNumber formatter
      if (match = _loopWithRowNumberRegex.exec(_marker.name)) {
        ...
        _marker.name = _marker.name.replace(/rowNumber(\(.*?\))?/, 'rowNumber' + _parameters);
      }
    }
  },
