...
var parser = {
  ...
  findMarkers : function (xml, callback) {
    ...
    xml = xml.replace(/(\r\n|\n|\r)/g,' ');
    var exec = that.findMarkersInsideTag(xml);
    var _allMarkers = exec.newMarkers;
    xml = exec.newXml;
    // "?:" avoiding capturing
    var _cleanedXml = xml.replace(/\{([^{]+?)\}/g, function (m, text, offset) {
      // clean the marker
  ...
  /**
   * It search all markers inside xml tag and replace it with an empty string it add all the markers found in a list
   * @param  {String} xml [Xml to parse]
   * @return {Object}     [Object with the new xml string and a list with all markers found]
   */
  findMarkersInsideTag : function (xml) {
    var that = this;
    var _allMarkers = [];
    ...
    // loop on all xml tag which contain a marker
    while (_oneTag = _reInsideTag.exec(xml)) {
      var _offsetToAdd = _oneTag.index;
      var _initialLength = _oneTag[0].length;
      // Replace all carbone marker in tag by an empty string
      _oneTag[0] = _oneTag[0].replace(/\{([^{]+?)\}/g, function (m, text, offset) {
        // If ther is several times the same marker in the tag, we increase the index to get the right position
        var _index = 1;
        ...
        _insidePreviousMarkerLength = that.countCharNotInXml(_oneTag.input, 0, that.getPosition(xml, m, _index));
        ...
        var _obj = {
          pos  : offset - _insidePreviousMarkerLength + _offsetToAdd,
          name : '_root.'+_cleanedMarker
        };
        _allMarkers.push(_obj);
        _allMarkersLength += _cleanedMarker.length + 2;
        return '';
      });
      // we do not replace in xml now, else some markers are missed
      _toReplace.push({
        newStr: _oneTag[0],
        index: _oneTag.index,
    ...
    // When we find all markers we replace the new generated string in xml
    var _offset = 0;
    for (var _i = 0 ; _i < _toReplace.length ; _i++) {
      var _current = _toReplace[_i];
      xml = [xml.slice(0, _current.index - _offset), _current.newStr, xml.slice(_current.index + _current.initialLength - _offset)].join('');
      _offset += _current.initialLength - _current.newStr.length;
    }

    return {
      newXml: xml,
      newMarkers: _allMarkers
    }
  },
