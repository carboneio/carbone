...
var parser = {
  ...
  findMarkers : function (xml, callback) {
    ...
    _cleanedXml = _cleanedXml.replace(/(\{[^{]+?\})/g, function (m, markerWithXml) {
      ...
      if (/</.test(markerWithXml) === true) {
        ...
        if (that.isCarboneMarker(_markerOnly) === false) {
          return m;
        }
  ...
  findSafeConditionalBlockPosition : function (xml, startSearchIndex, endSearchIndex) {
    ...
    var _firstValidTagPos = startSearchIndex;
    var _lastValidTagPos = startSearchIndex;
    ...
    // edge case: when the first XML tag detected is already the last XML tag between startSearchIndex and endSearchIndex
    if (_tagRegExp.lastIndex >= endSearchIndex) {
      // if the last closing XML tag is a self-closing tag, include it
      if ( _tag[2] === '/' ) {
        return [startSearchIndex, _tagRegExp.lastIndex];
      }
      return [startSearchIndex, _tag.index];
    }
    while (_tag !== null) {
      // closing tag
      if (_tag[1] === '/') {
        ...
        if (_beginPos !== undefined) {
          _lastValidTagPos = _tagRegExp.lastIndex;
          if ( _openingTagPos.length < _minimalXmlDepth ) {
            _isCurrentPartValid = true;
            _minimalXmlDepth = _openingTagPos.length;
            _firstValidTagPos = _beginPos;
          }
        }
