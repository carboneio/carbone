     ...
     //capture words inside () in order to translate them with the correct lang.json.
      ...
      if(_pattern instanceof Array){
        /*--------------------------------------------------------------*\
              ...
              the expression. Else we write _pattern[1]
        \*--------------------------------------------------------------*/
        ...
        //_sentenceToTranslate=parser.replaceEncodedMathematicalCharactersByNoEncodedCharacters(_sentenceToTranslate);
        _sentenceToTranslate= parser.replaceEncodedOperatorCharactersByNoEncodedCharacters(_sentenceToTranslate);


        var finalStr=_sentenceToTranslate;
        if(objLang!==undefined && objLang[_sentenceToTranslate] !== undefined){
          finalStr = objLang[_sentenceToTranslate];
        }

        finalStr = parser.replaceNoEncodedOperatorCharactersByEncodedCharacters(finalStr);
        //finalStr = parser.replaceNoEncodedSpecialCharactersByEncodedCharacters(finalStr);
        //finalStr = parser.replaceNoEncodedMathematicalCharactersByEncodedCharacters(finalStr);

        
        return finalStr + _xmlOnly;
