...
var parser = {
  ...
  /**
   * Parse the xml, select the {t()} marker and translate them by using the corresponding traduction in params.objLang
   * @param  {String}   xml
  ...
  translate : function(xml, objLang, callback){
    if(typeof(xml) !== 'string' || objLang === undefined){
    ...
    var _cleanedXml = xml.replace(/\{\s*((?:\s*<[^>]*>\s*)*t[\s\S]+?)\}/g, function(m, text, offset) {
      ...
      if(_pattern instanceof Array && _pattern.length > 1){
        ...
        //if the translation exists, translaye it
        if(objLang[_strToTranslate] !== undefined){
