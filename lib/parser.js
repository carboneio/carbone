...
var parser = {
  ...
  /**
   * Parse the xml, select the {t()} marker and translate them by using the corresponding traduction in the lang file
   * @param  {String}   xml
   * @param  {Object}   objLang {
  ...
  translate : function(xml, objLang, callback){
    ...
    var _cleanedXml = xml.replace(/\{\s*((?:\s*<[^>]*>\s*)*t[\s\S]+?)\}/g, function(m, text, offset) {
      ...
      if(_pattern instanceof Array && _pattern.length > 1){
        ...
        //if the translation exists and different of null, translaye it. 
        if(objLang !== undefined && objLang[_strToTranslate] !== undefined && objLang[_strToTranslate]!==""){
          _translatedStr = objLang[_strToTranslate];
        }
