...
var parser = {
  ...
  findPivot_v5 : function (xmlFlattened, positionOfFirstIthTag = -1, positionOfFirstIthPlusOneTag) {
    ...
    if (_part1End === null) {
      ...
      _part1EndPos    = (_nextNode.index > _part1End.lastIndex ? _nextNode.index : _part1End.lastIndex); // include non-XML part which is just after end of part 1
      _part2StartPos  = _part2Start?.index ?? _part1EndPos;
    }
    if (_part2Start !== null) {
    ...
    const _repetitionSection = {
      startEven : _part1StartPos,
      endEven   : _part2StartPos,
      startOdd  : _part2StartPos,
      endOdd    : _part2EndPos
    };
  ...
  flattenXML_v5 : function (xml) {
    ...
    while (_tag !== null) {
      ...
      if (_tagText[1] === '/') {
        ...
        if (_correspondingOpenPos === undefined) {
          _correspondingOpenPos = { id : _depth }; // TEMP FOR TEST
          // throw new Error('XML not valid');
        }
        _correspondingOpenPos.match = _xmlTag.id;
        _xmlTag.match = _correspondingOpenPos.id;
      ...
      // is not a self-closing tag, so it is an opening tag
        ...
        _xmlTag.depth = _depth++;
        _xmlTag.match = Number.MAX_SAFE_INTEGER; // only for test
        _openingTagPos.push(_xmlTag);
      }
      _array.push(_xmlTag);
      _tag = _xmlTagRegExp.exec(xml);
