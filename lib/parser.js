...
var parser = {
  ...
  isCarboneMarker : function (oneMarker) {
    const _isCarboneMarkerRegexp = /^\{?\s*(?:[cdt]\s*[.[:(])|^\{?\s*[#$]|^\{?\s*bindColor|^\{?\s*bindChart/;

    return _isCarboneMarkerRegexp.test(oneMarker) && parser.isPrintableChar(oneMarker);
  },

  /**
   * Determines whether the specified text is printable character.
   *
   * It filters binary file (unknown type)
   *
   * @param      {String}   text    The text
   * @return     {Boolean}  True if the specified text is printable character, False otherwise.
   */
  isPrintableChar : function (text) {
  ...
  translate : function (xml, options, callback) {
    ...
    // capture {t  } markers. The part "(?:<[^>]*>\s*)*" is used to eleminate xml tags
    var _cleanedXml = xml.replace(/\{\s*((?:<[^>]*>\s*)*t[\s\S]+?)\}/g, function (m, text) {
  ...
  findVariables : function (xml, existingVariables, callback) {
    ...
    let _error = null;
    // capture {#  } markers.
    var _cleanedXml = xml.replace(/\{([^{]+?)\}/g, function (m, text) {
      if (parser.isPrintableChar(text) === false || /^\s*#/.test(text) === false) {
        return m;
      }
      ...
      if (_pattern instanceof Array) {
        ...
        try {
          ...
          _error = 'Error: impossible to parse variable '+ _variable;
        }
        return _xmlOnly;
      }
      else {
    ...
    process.nextTick(function () {
      callback(_error, _cleanedXml, existingVariables);
    });
  },
  ...
  preprocessMarkers : function (markers, variables, callback) {
    ...
    parser.replaceOldCountByNewCount(markers);
    callback(null, markers);
  },

  /**
   * Replace old :count formatter by new one
   *
   * @param   {Array}  markers  Array of markers
   * @return  {Array}           Array of markers with :count replaced by :cumCount
   */
  replaceOldCountByNewCount : function (markers) {
    for (var i = 0; i < markers.length; i++) {
      var _marker = markers[i];
      var _indexOfCount = _marker.name.indexOf(':count');
      if (_indexOfCount !== -1) {
        var _lastChar = _marker.name[_indexOfCount+6];
        if (_lastChar === undefined || _lastChar === '(' || _lastChar === ':') {
          _marker.name = _marker.name
            .replace(':count', ':cumCount')
            .replace(/\(\)$/,'')
            .replace(/:cumCount\((\d+)\)/, (str, nb) => {
              return ':cumCount:add('+(parseInt(nb, 10) + -1)+')';
            });
        }
      }
    }
   ...
   * Finds a pivot v 5.
   *
   * @param   {Array}            xmlFlattened                  The xml flattened
   * @param   {number}           [positionOfFirstIthTag=-1]    The position of first ith tag
   * @param   {(number|string)}  positionOfFirstIthPlusOneTag  The position of first ith plus one tag
   */
  findPivot_v5 : function (xmlFlattened, positionOfFirstIthTag = -1, positionOfFirstIthPlusOneTag) {
    let _xmlNode = xmlFlattened[0] ?? {}; // if no XML, just
    if (!positionOfFirstIthPlusOneTag) {
      positionOfFirstIthPlusOneTag = xmlFlattened[xmlFlattened.length-1].lastIndex ;
    }
    let _part1End = null;
    let _part2Start = null;

    let _indexOfFirstTag = 0;
    let _indexOfLastTag = 0;
    //  | <aa>
    for (_indexOfFirstTag = 0              ; _indexOfFirstTag < xmlFlattened.length && xmlFlattened[_indexOfFirstTag].index < positionOfFirstIthTag; _indexOfFirstTag++);
    //  | <aa>
    for (_indexOfLastTag = _indexOfFirstTag; _indexOfLastTag  < xmlFlattened.length && xmlFlattened[_indexOfLastTag].index < positionOfFirstIthPlusOneTag; _indexOfLastTag++);
    _indexOfLastTag--;

    let _lowestMatch = _indexOfFirstTag;
    for (let i = _indexOfFirstTag; i <= _indexOfLastTag; i++) {
      _xmlNode = xmlFlattened[i];
      // if the node is opening before the first node of the i-th row
      if (_xmlNode.match < _lowestMatch) {
        _lowestMatch = _xmlNode.match;
        _part1End = _xmlNode;
      }
      if (_xmlNode.match > _indexOfLastTag) {
        _part2Start = _xmlNode;
        break;
      }
    }
    let _part1StartPos = positionOfFirstIthTag; // flat structure
    let _part1EndPos  = 0;
    let _part2StartPos  = 0;
    let _part2EndPos  = positionOfFirstIthPlusOneTag; // TODO should position of last [i+1] tag FLAT structure

    if (_part1End === null) {
      if (_part2Start === null) {
        // flat structure (only text, self-closing XML tags)
        _part1EndPos = _part2StartPos = positionOfFirstIthPlusOneTag;
      }
      else {
        // only opening tag between first [i] carbone tag and first [i+1] carbone tag
        _part1EndPos = _part2StartPos = _part2Start.index;
      }
    }
    else {
      _part1StartPos  = xmlFlattened[_part1End.match].index;
      const _nextNode = xmlFlattened[_part1End.id + 1] ?? {};
      _part1EndPos    = (_nextNode.index > _part1End.lastIndex ? _nextNode.index : _part1End.lastIndex); // include non-XML part
      _part2StartPos  = _part2Start?.index ?? positionOfFirstIthPlusOneTag;
    }
    if (_part2Start !== null) {
      _part2EndPos = xmlFlattened[_part2Start.match].lastIndex;
    }

    const _repetitionSection = {
      part1Start : _part1StartPos,
      part1End   : _part2StartPos, // _part1EndPos, not used but I keep for code for the moment
      part2Start : _part2StartPos,
      part2End   : _part2EndPos
    };
    return _repetitionSection;
  },
   ...
   * @return {Array}
   */
  flattenXML_v5 : function (xml) {
    const _array = [];
    if (typeof (xml) !== 'string') {
      return _array;
    }
    // avoid capture with () to reduce memory allocation
    // simple regex, ReDOS safe
    const _xmlTagRegExp = /<[^>]+>/g;
    let _tag = _xmlTagRegExp.exec(xml);
    let _depth = 0;
    const _openingTagPos = [];
    if (_tag === null) {
      _array.push({
        id             : 0,
        index          : 0,
    ...
    while (_tag !== null) {
      const _tagText  = _tag[0];
      const _xmlTag = {
        id             : _array.length,
        index          : _tag.index,
        lastIndex      : _xmlTagRegExp.lastIndex,
        depth          : _depth,  // not used
        match          : _array.length, // self-closing tag
        matchWithIndex : _tag.index
      };
      // closing tag
      if (_tagText[1] === '/') {
        _xmlTag.depth = _depth--;  // not used
        let _correspondingOpenPos = _openingTagPos.pop();
        if (_correspondingOpenPos === undefined) {
          throw new Error('The XML of your document is not valid, please contact the support');
        }
        _correspondingOpenPos.match = _xmlTag.id;
        _correspondingOpenPos.matchWithIndex = _xmlTag.index;
        _xmlTag.match = _correspondingOpenPos.id;
        _xmlTag.matchWithIndex = _correspondingOpenPos.index;
      }
      // is not a self-closing tag, so it is an opening tag
      else if (_tagText[_tagText.length - 2] !== '/') {
        _xmlTag.depth = _depth++;  // not used
        _openingTagPos.push(_xmlTag);
      }
      _array.push(_xmlTag);
   ...
   *
   * This version uses the new flattenXML which does not keep non-XML node
   *
   * Safe means "does not break XML if the XML block between beginning and end is removed"
   *
   * @param  {String}  xmlFlattened     flattened XML returned by the new flattenXML_v5
   ...
   */
  findSafeConditionalBlockPosition_v5 : function (xmlFlattened, beginTextPos, endTextPos) {
    beginTextPos = Math.trunc(beginTextPos);
    endTextPos   = Math.trunc(endTextPos);

    let _validCandidates = [];
    let _newCandidate    = null;
    let _lastCandidate   = [];

    // if there is not XML, xmlFlattened contains exactly one node
    if (xmlFlattened.length === 1) {
      return [[beginTextPos, endTextPos]];
    }

    let _firstTagAfterIfBegin = xmlFlattened[0];
    let _firstTagBeforeIfEnd = xmlFlattened[0];

    // "|<aa>"   OR  "| <aa>"
    for (let i = 0                         ; i < xmlFlattened.length && xmlFlattened[i].index < beginTextPos; i++) { _firstTagAfterIfBegin = xmlFlattened[i+1]; }
    if (_firstTagAfterIfBegin === undefined) {
      return [[beginTextPos, endTextPos]];
    }
    // "</aa> |"  OR   "</aa>|"
    for (let i = _firstTagAfterIfBegin.id ; i < xmlFlattened.length && xmlFlattened[i].lastIndex <= endTextPos;  i++) { _firstTagBeforeIfEnd    = xmlFlattened[i]; }

    let _tag                = _firstTagAfterIfBegin;
    let _lowestTagCandidate = _firstTagAfterIfBegin;
    let _prevTag            = xmlFlattened[_tag.id-1] ?? {};
    let _indexOfLastTag     = _prevTag.lastIndex;
    // if the tag before is on edge: "<aa>|"  OR  "<a|a>"
    if (_prevTag.lastIndex >= beginTextPos) {
      // go back if it is a opening tag, it's corresponding closing tag is still inside the condition, and it's an adjacent tag
      while ( _prevTag !== undefined && _prevTag.match > _prevTag.id && _prevTag.match <= _firstTagBeforeIfEnd.id && _prevTag.lastIndex === _indexOfLastTag ) {
        _indexOfLastTag = _prevTag.index;
        _lowestTagCandidate = _prevTag;
        _prevTag = xmlFlattened[_prevTag.id-1];
      }
    }

    _tag                     = _firstTagBeforeIfEnd;
    let _highestTagCandidate = _firstTagBeforeIfEnd;
    let _nextTag             = xmlFlattened[_tag.id+1] ?? {};
    let _lastIndexOfLastTag  = _nextTag.index;
    // </a|a> OR  |</aa>
    if (_nextTag.index <= endTextPos) {
      // go next if it is a closing tag, it's corresponding opening tag is still inside the condition, and it's an adjacent tag
      while ( _nextTag !== undefined && _nextTag.match < _nextTag.id  && _nextTag.match >= _firstTagAfterIfBegin.id && _nextTag.index  === _lastIndexOfLastTag ) {
        _lastIndexOfLastTag = _nextTag.lastIndex;
        _highestTagCandidate = _nextTag;
        _nextTag = xmlFlattened[_nextTag.id+1];
      }
    }

    let _currentVisitedNode = _lowestTagCandidate.id;
    let _lastPos = beginTextPos;
    const _dummyObject     = {};  // constant, empty object to avoid instanciation in the loop below
    while (_currentVisitedNode < xmlFlattened.length) {
      if (_tag.index >= endTextPos) {
        break;
      }
      _tag = xmlFlattened[_currentVisitedNode];
      const _matchingTag = _tag.match >= _tag.id && _tag.match !== Number.MAX_SAFE_INTEGER ? xmlFlattened[_tag.match] : _dummyObject; /* opening tag or self-closing */

      const _maxLastIndex = Math.max(_lastPos, beginTextPos);
      const _minLastIndex = Math.min(_tag.index, endTextPos);
      // detect and keep non-XML section  <a>| sdsd |</b>
      if (_maxLastIndex < _minLastIndex) {
        _newCandidate = [_maxLastIndex, _minLastIndex];
      }

      _currentVisitedNode++;
      // if this XML part is included in the if-block
      if (_matchingTag.id <= _highestTagCandidate.id) {
        _newCandidate = _newCandidate === null ? [_tag.index, _matchingTag.lastIndex] : [_newCandidate[0], _matchingTag.lastIndex];
        // jump directly to the next valid XML part
        _currentVisitedNode = _matchingTag.id + 1;
        _lastPos = _matchingTag.lastIndex;
      }
      else {
        _lastPos = _tag.lastIndex;
      }
    ...
    // edge case when there is valid candidates found at all
    if (_validCandidates.length === 0) {
      return [[endTextPos, endTextPos]];
    }
  ...
  /**
   * Adds a fake XML node for each conditional block.
   *
   * @param   {Array}  conditionalBlocks  The conditional blocks [[1, 10], [15, 20]]
   * @param   {Array}  xmlFlattened       The xml flattened
   * @return  {Array}                     The xml flattened with conditions inside, sorted
   */
  addFakeNodeForEachConditionalBlock (conditionalBlocks, xmlFlattened) {
    const _len = xmlFlattened.length;

    if (conditionalBlocks.length === 0) {
      return xmlFlattened;
    }

    // sort to speed up duplicated removal
    conditionalBlocks.sort((a, b) => {
      return b[0] - a[0];
    });

    const _distinctConditionalBlocks = [];
    let _prevNode = [-1, -1];

    // remove duplicated conditional blocks (useless). It is enough to only have one couple of XML node to change the behavior of loops
    for (let i = 0 ; i < conditionalBlocks.length; i++) {
      const _node = conditionalBlocks[i];
      if ( !(_node[0] === _node[1] && _prevNode[0] === _node[0] && _prevNode[1] === _node[1] )) {
        _distinctConditionalBlocks.push(_node);
      }
      _prevNode = _node;
    }

    // create fake XML ndoe for each IF begin/end
    for (let i = 0 ; i < _distinctConditionalBlocks.length; i++) {
      const _conditionBlock = _distinctConditionalBlocks[i];
      const _openId  = 2 * i + _len;
      const _closeId = 2 * i + 1 + _len;
      xmlFlattened.push({ id : _openId , index : _conditionBlock[0], lastIndex : _conditionBlock[0], depth : 0, match : _closeId, matchWithIndex : _conditionBlock[1]});  // depth not used
      xmlFlattened.push({ id : _closeId, index : _conditionBlock[1], lastIndex : _conditionBlock[1], depth : 0, match : _openId , matchWithIndex : _conditionBlock[0]});  // depth not used
    }

    // sort again the XML
    const _newXMLFlattened = xmlFlattened.slice(0);
    _newXMLFlattened.sort(sortXMLTree);

    // fix id / match
    for (let i = 0 ; i < _newXMLFlattened.length; i++) {
      const _node = _newXMLFlattened[i];
      _node.id = i;
    }
    for (let i = 0 ; i < _newXMLFlattened.length; i++) {
      const _node = _newXMLFlattened[i];
      _node.match = xmlFlattened[_node.match].id;
    }

    return _newXMLFlattened;
  },
  ...
  parseMathematicalExpression : function (mathExpr, safeVariableInjectionFn) {
    ...
    for (let i = _variables.length - 1; i > -1; i--) {
      ...
      // If the variable does not start by "." or "d." or "c."" or a number, it means that we have cut a variable.
      // It happens only if the variable contains a dash
      if (/^[0-9.]|^(?:d\.)|^(?:c\.)/.test(_trimVariable) === false) {
  ...
  /**
   * Removes and get xml declaration.
   *
   * @param      {string}  xml     The xml
   * @return     {Object}  { xml, declaration }
   */
  removeAndGetXMLDeclaration : function (xml) {
    if (typeof (xml)!== 'string') {
      return { xml , declaration : '' };
    }
    let _endOfDeclaration = xml.lastIndexOf('?>') + 2;
    let _declaration = xml.slice(0, _endOfDeclaration);
    let _xmlWithoutDeclaration = xml.slice(_endOfDeclaration);
    return { xml : _xmlWithoutDeclaration, declaration : _declaration};
  },
   ...
   * @return {string}  the merker withotu formatters
   */
  removeFormatters : function (marker) {
    if (typeof (marker) !== 'string') {
      return marker;
    }
    const _trimMarker = marker.trim();
    // replace : inside arrays to avoid bugs
    const _markerWithoutDirectArrayAccess    = _trimMarker.replace(/\[[^[]+?\]/g, (m) => m.replace(/:/g, '|'));
    const _firstDoublePointAfterClosingArray = _markerWithoutDirectArrayAccess.indexOf(':');
    return _trimMarker.slice(0, _firstDoublePointAfterClosingArray) + '}';
  },

  /**
   * Removes markers in XML
   *
   * @param  {string}  xml       xml with markers
   * @param  {string}  replaceBy replace marker by this value
   * @return {string}  xml       without markers
   */
  removeMarkers : function (xml, replaceBy = '0') {
    return xml?.replace(/\{([^{]+?)\}/g, function (m, marker) {
      if (parser.isCarboneMarker(marker) === false) {
        return m;
      }
      return replaceBy;
    });
...
function sortXMLTree (a, b) {
  ...
  const _isAClosingTag = a.id > a.match;
  const _isBClosingTag = b.id > b.match;
  ...
  if (_isAClosingTag === false && _isBClosingTag === false) {
    return b.matchWithIndex - a.matchWithIndex;
  }
  if (_isAClosingTag === true && _isBClosingTag === true) {
    return b.matchWithIndex - a.matchWithIndex;
  }
