...
var parser = {
  ...
  isCarboneMarker : function (oneMarker) {
    const _isCarboneMarkerRegexp = /^\{?\s*(?:[cdt]\s*[.[:(])|^\{?\s*[#$]|^\{?\s*bindColor|^\{?\s*bind/;
  ...
  removeFormatters : function (marker) {
    ...
    const _markerWithoutDirectArrayAccess    = marker.replace(/\[[^[]+?\]/g, (m) => m.replace(/:/g, '|'));
    const _firstDoublePointAfterClosingArray = _markerWithoutDirectArrayAccess.indexOf(':');
    return marker.slice(0, _firstDoublePointAfterClosingArray) + '}';
  },
