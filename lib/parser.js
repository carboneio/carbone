...
var parser = {
    ...
    this._getPotentialPicturesFromMarkers(_markers, function (potentialPictures) {
      if (potentialPictures.length === 0) {
        ...
        for (var i = 0; i < potentialPictures.length; i++) {
          ...
          _potentialPicture = _potentialPicture.markers;
  ...
  _getPotentialPicturesFromMarkers : function (markers, cb) {
    ...
    var _indexMarkers = 0;
    var _dfCount = -1;
    var _loopStartRgx = /\[\s*?i\s*?(?:\s*?=\s*?1\s*?)?\s*?\]/;
    var _loopEndRgx = /\[\s*?i\s*?\+\s*?.\s*?\]/;

    while (_indexMarkers < markers.length) {
      var _marker = markers[_indexMarkers];
      var _picture = _pictures[_indexPictures];

      if (_marker.type === 'dfOpen') {
        if (_dfCount <= 0) {
          _dfCount = 1;
          _picture.meta.starts = _marker.pos;
        } else {
          _dfCount++;
        }
        ...
        if (_marker.name) {
          if (_marker.name.match(_loopStartRgx)) {
            _pictures[_indexPictures].loops.start.push(_marker);
          } else if (_marker.name.match(_loopEndRgx)) {
            _pictures[_indexPictures].loops.end.push(_marker);
          } else {
            _pictures[_indexPictures].markers.push(_marker);
          }
        } else {
          _pictures[_indexPictures].markers.push(_marker);
        }
      ...
      if (_dfCount === 0) {
        if (_picture.loops.start.length > _picture.loops.end.length) {
          while (_picture.loops.start.length > _picture.loops.end.length) {
            var toMove = JSON.parse(JSON.stringify(_picture.loops.start[_picture.loops.start.length - 1]));

            _picture.loops.start.splice(_picture.loops.start.length - 1, 1);
          }
        }
        _picture.meta.ends = _marker.pos;
        for (var i = 0; i < _picture.loops.start.length; i++) {
          _picture.loops.start[i].pos = _picture.meta.starts - 1;
        }
        for (var j = 0; j < _picture.loops.end.length; j++) {
          _picture.loops.end[j].pos = _picture.meta.ends + 14;
        }
        //loopsMarkers = loopsMarkers.concat(_picture.loops.start).concat(_picture.loops.end);
        _indexPictures++;
        _pictures.push({
          meta  : {
            starts : -1,
      ...
      _indexMarkers++;
      if (_indexMarkers === markers.length) {
        return cb(_pictures, loopsMarkers);
      }
    }
  },
