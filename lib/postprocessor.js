...
var postprocessor = {
   ...
   */
	execute : function (template, data, options, callback) {
		...
		for (var i = -1; i < template.embeddings.length; i++) {
			...
			switch (_fileType) {
				case 'docx':
					this.embedDocxPictures(template);
					break;
				case 'odt':
					this.embedOdtPictures(template);
					break;
				default:
					break;
			}
   ...
   */
	embedDocxPictures : function (template, callback) {
		...
		var _relsFile = dynpics.getTemplate(template, 'word/_rels/document.xml.rels');
		...
		this.replaceDocxRels(template, _relsFile.data,  function (newTemplate, newRels) {
			// We save the returned values and return the new template
			template = newTemplate;
			_relsFile.data = newRels;
			return callback(null, template);
		});
	},
	/**
	...
	replaceDocxRels : function (template, xml, callback) {
		...
		var _relsPicRegex = /<Relationship\s*?Id="\S*?"\s*?Type="http:\/\/schemas\.openxmlformats\.org\/officeDocument[^><]*?image".*?Target="(http.*?\.([a-z]*?))".*?>/ig;
		var _regexMatch = _relsPicRegex.exec(xml);
		var _picId = 0;
		...
		while (_regexMatch) {
			var _extension = _regexMatch[2];
			// Name which will be the one in relations file (target)
			var _newPicName = 'media/CarbonePicture' + _picId + '.' + _extension;
			// Href of the picture
			var _href = _regexMatch[1];

			this.addDocxContentType(template, _extension, function (newContentType) {
				 dynpics.getTemplate(template, '[Content_Types].xml').data = newContentType;
			});
			// Get picture ready to push in template files
			this.getPictureFile(_href, 'word/' + _newPicName, function (picture) {
				// Replace relation target
				xml = xml.replace(_href + '" TargetMode="External"', _newPicName + '"');
			...
			_regexMatch = _relsPicRegex.exec(xml);
		}
	},
	/**
	...
	embedOdtPictures : function (template, callback) {
		...
		this.getOdtPictures(_contentFile.data, function (pictures) {
			...
			_that.generateOdtEmbeds(pictures, function (rels) {
				// Replace them
				_that.replaceOdtEmbeds(template, _contentFile.data, rels, function (newContent, newTemplate) {
