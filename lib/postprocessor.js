...
const postprocessor = {
  ...
  execute : function (template, data, options, callback) {
    ...
    for (let j = 0; j < template.files.length; j++) {
      ...
      if (_file.name !== '___CARBONE___') {
        _newArray.push(template.files[j]);
      }
    }
    template.files = _newArray;
    ...
    for (var i = -1; i < template.embeddings.length; i++) {
      ...
      switch (_fileType) {
        ...
        case 'odp':
          ...
          color.postProcessLOColorFormatter(template, data, options);
        case 'odg':
        ...
        case 'ods':
          ...
          break;
        case 'html':
          html.removeCarboneTags(template, 'html');
          break;
        case 'docx':
  ...
  update : function (data, options, callback) {
    // manage images
    file.fetchFileDatabase(options, (err) => {
      if (err) {
        return callback(err);
      }
