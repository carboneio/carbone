...
var postprocessor = {
  ...
  execute : function (template, data, options, callback) {
    ...
    for (var i = -1; i < template.embeddings.length; i++) {
      ...
      var _fileType = options.extension || path.extname(_mainOrEmbeddedTemplate).toLowerCase().slice(1);
      if (!_fileType) {
        return callback(new Error('The file type is not defined on the filename extension or on the option object.'));
      }
      switch (_fileType) {
        case 'docx':
  ...
  embedDocxDocumentsPictures : function (template, documents, data, options, callback) {
    ...
    for (let i = 0; i < _count; i++) {
      ...
      this.embedDocxDocumentPictures(template, _document, data, options, () => {
        if (--_count === 0) {
          // We're done
          return callback(null, template);
        }
  ...
  replaceDocxRels : function (template, relsFile, data, options, document, callback) {
    ...
    for (var i = 0; i < _picturesMatches.length; i++) {
      ...
      this.embedDocxPictureRel(template, data, options, _relation, i, function (err, newRelations, picture) {
        ...
        // We don't check err because error can occured bu it's not a problem, it's because
        // we try to replace image that must not be replace. So nothing is replace (this is why there is an error)
        // but the image that we must replace will be replace in the next loop
   ...
   * @param  {Object}   template  Carbone report
   * @param  {Object}   data      Carbone data
   * @param  {Object}   options   Carbone options
   * @param  {Function} callback
   */
  embedOdtPictures : function (template, data, options, callback) {
    ...
    this.embedFileOdtPictures(template, 'content.xml', data, options, function (err, template) {
      _that.embedFileOdtPictures(template, 'styles.xml', data, options, callback);
    });
  },
  /**
   * Handle dynamic pictures for a given report file.
   * @param  {Object}   template  Carbone report
   * @param  {String}   filename  Report filename to process
   * @param  {Object}   data      Carbone data
   * @param  {Object}   options   Carbone options
   * @param  {Function} callback
   */
  embedFileOdtPictures : function (template, filename, data, options, callback) {
    var _that        = this;
    var _contentFile = dynpics.getTemplate(template, filename);
    var _manifest    = dynpics.getTemplate(template, 'META-INF/manifest.xml');

    if (!_contentFile || !_contentFile.data) {
      return callback(null, template);
    }
    ...
    this.getOdtPictures(_contentFile.data, function (pictures) {
      ...
      _that.generateOdtEmbeds(pictures, function (rels) {
        // Replace them
        _that.replaceOdtEmbeds(template, _contentFile.data, rels, data, options, function (err, newContent, template) {
  ...
  updateOdtManifest : function (template, manifest, callback) {
    ...
    for (var i = 0; i < template.files.length; i++) {
      ...
      _match = _file.name.match(_isPicture);
      if (_match) {
