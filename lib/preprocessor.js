  ...
  handleHideFormatter : function (template, format) {
    ...
    const findPosXmlElement = (file, index, xmlFormat, options) => {
      ...
      while (startTagPos >= 0) {
        ...
        if (childEndTagPosFound.includes(_endRowPos) === false && _endRowPos < hidePos) {
      ...
      while (endTagPos < file.data.length) {
        ...
        /** Check if it is the correct row, instead of a child-table within the cell */
        if (childStartTagPosFound.includes(_beginRowPos) === false && _beginRowPos > hidePos) {
          ...
          nbrToHide += 1;
          /**
           * The hidePos is moved to change the reference hide marker pos to the next row
           * `hidePos` is used to verify if the row start/end positions are correct
           */
          hidePos = endTagPos;
     ...
     */
    const _hideFormatterRegex = /({[^{}]+?):hide\(([^),]*),?([^)}]*)?\)}/g;
    const _newMarkerPosList = [];
    const _allHideMarkers = [];
    let _totalTextRemoved = 0;

    // find all hide markers and remove them in the same time
    _file.data = _file.data.replace(_hideFormatterRegex, (markerFull, markerWithoutHide, typeWithQuote, typeOptionWithQuote, hideIndex) => {
      ...
      if (_xmlFormat?.[_type] === undefined) {
        ...
        throw Error('Unknown parameter in formatter hide() of "'+markerFull+'". Do you mean ":hide('+_alternativeType+')"?');
      }
      if (_type === 'row') {
        _options.nbrElementsToHide = parseInt(_typeOption) || 1;
      }
      _allHideMarkers.push({ index : (hideIndex - _totalTextRemoved), withoutHide : markerWithoutHide, format : _xmlFormat[_type], options : _options });
      _totalTextRemoved += markerFull.length;
      return '';
    });

    for (let i = 0; i < _allHideMarkers.length; i++) {
      const _marker = _allHideMarkers[i];
      ...
      if (_foundEndPos && _foundEndPos.xml.indexOf(_marker.withoutHide) !== -1) {
        continue;
      }
      /** if a position is already saved for the hideEnd, the marker is moved before the existing marker */
      if (_foundEndPos) {
        _foundEndPos.xml = `<carbone>${_marker.withoutHide}:hideEnd}</carbone>` + _foundEndPos.xml;
      }
      else {
        /** Create a new end position [{pos: X, xml: '' }] */
        _newMarkerPosList.push({ pos : _tagPos.end, xml : `<carbone>${_marker.withoutHide}:hideEnd}</carbone>` });
      }
      ...
      if (_foundStartPos) {
        _foundStartPos.xml += `<carbone>${_marker.withoutHide}:hideBegin}</carbone>`;
      }
      else {
        _newMarkerPosList.push({ pos : _tagPos.start, xml : `<carbone>${_marker.withoutHide}:hideBegin}</carbone>` });
      }
    }
