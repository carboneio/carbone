...
var preprocessor = {
  ...
  execute : function (template, options, callback) {
    ...
    for (let i = 0, len = template.files.length; i < len; i++) {
      ...
      template.files[i].data = preprocessor.preParseDataMarker(template.files[i].data, _onlySetMarkers);
    }
  ...
  handleDropFormatter : function (template, format) {
    ...
    const findPosXmlElement = (file, index, xmlFormat, options) => {
      ...
      while (endTagPos < file.data.length) {
        ...
        if (childStartTagPosFound.includes(_beginRowPos) === false && _beginRowPos > dropPos) {
          ...
          nbrToHide += 1;
          /**
           * The dropPos is moved to change the reference hide marker pos to the next row
     ...
     */
    const _dropFormatterRegex = /({[^{}]+?):drop\(([^),]*),?([^)}]*)?\)}/g;
    ...
    _file.data = _file.data.replace(_dropFormatterRegex, (markerFull, markerWithoutDrop, typeWithQuote, typeOptionWithQuote, dropIndex) => {
      ...
      if (_xmlFormat?.[_type] === undefined) {
        ...
        throw Error('Unknown parameter in formatter drop() of "'+markerFull+'". Do you mean ":drop('+_alternativeType+')"?');
      }
      if (_type === 'row' || _type === 'p') {
        _options.nbrElementsToHide = parseInt(_typeOption) || 1;
      }
      _allDropMarkers.push({ index : (dropIndex - _totalTextRemoved), withoutDrop : markerWithoutDrop, format : _xmlFormat[_type], options : _options });
    ...
    for (let i = 0; i < _allDropMarkers.length; i++) {
      ...
      if (_foundEndPos) {
        _foundEndPos.xml = `<carbone>${_marker.withoutDrop}:hideEnd}</carbone>` + _foundEndPos.xml;
      }
      else {
        /** Create a new end position [{pos: X, xml: '' }] */
        _newMarkerPosList.push({ pos : _tagPos.end, xml : `<carbone>${_marker.withoutDrop}:hideEnd}</carbone>` });
      }
      ...
      if (_foundStartPos) {
        _foundStartPos.xml += `<carbone>${_marker.withoutDrop}:hideBegin}</carbone>`;
      }
      else {
        _newMarkerPosList.push({ pos : _tagPos.start, xml : `<carbone>${_marker.withoutDrop}:hideBegin}</carbone>` });
      }
    }
  ...
  /**
   * Loop through XML files to extract carbone markers which modify data object
   *
   * @param {Object} template
   */
  preParseDataMarker : function (xml, onlySetMarkers) {
