...
var preprocessor = {
  ...
  handleHideRowFormatter : function (template, format) {
    ...
    if (!_file || !_xmlFormat || _file.data.indexOf(':hideRow') === -1) {
      return;
    }

    /**
     * Find all hideRows retrieve the marker
     */
    ...
    for (let i = 0; i < _hideRowList.length; i++) {
      const _hideRowRegex = new RegExp(`${helper.regexEscape(_hideRowList[i])}:hideRow[^}]*}`, 'g');
      const _hideRowIndex = _file.data.search(_hideRowRegex);
      const _row = extractTableRow(_file, _hideRowIndex, _xmlFormat);
      if (_row.start < 0 || _row.end < 0) {
        continue;
      }
      _file.data = `${_file.data.substring(0, _row.start)}<carbone>${_hideRowList[i]}:hideBegin}</carbone>${_row.xml.replace(_hideRowRegex, '')}<carbone>${_hideRowList[i]}:hideEnd}</carbone>${_file.data.substring(_row.end)}`;
    }
  },
  /**
