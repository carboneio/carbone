      ...
      try {
        switch (_fileType) {
          ...
          case 'docx':
            ...
            hyperlinks.preProcesstHyperlinksDocx(template, options);
  ...
  generateUniqueIds : function (template) {
    for (var i = 0; i < template.files.length; i++) {
      ...
      if (_file.name.indexOf('.xml') !== -1 && _file.name.indexOf('.rels') === -1 &&
        ...
        _file.data = _file.data.replace(/<wp:docPr ([^>]*)id="\d+"/g, '<wp:docPr $1id="{c.now:private:generateImageDocxGlobalId}"');
      }
    }
    return template;
  },
