...
var preprocessor = {
      ...
      try {
        switch (_fileType) {
          ...
          case 'xlsx':
            ...
            break;
          case 'docx':
            preprocessor.handleHideFormatter(template, 'docx')
            ...
            break;
          case 'odt':
            preprocessor.handleHideFormatter(template, 'odt')
  ...
  handleHideFormatter : function (template, format) {
    let _file = null;
    let _xmlFormat = null;
    ...
    while ((res = _hideRowPattern.exec(_file.data)) !== null) {
      ...
      if (_marker && res?.[2]) {
        _hideRowList.push({
          marker   : _marker,
          formatter: res?.[0],
          type     : (helper.removeQuote(res?.[2])).trim()
        })
      }
    }
    ...
    const extractTableRow = (file, index, xmlFormat) => {
      ...
      */
      ...
      while (rowEndPos < file.data.length) {
        ...
        let _rowStartPosVerifyIndex = rowEndPos;
        while (_rowStartPosVerifyIndex !== -1) {
          _rowStartPosVerifyIndex = file.data.lastIndexOf(xmlFormat.start, _rowStartPosVerifyIndex)  // E.G. Searching '<table:table-row>'
          /** check if it is the correct tag name */
          if (_rowStartPosVerifyIndex !== -1 && file.data[_rowStartPosVerifyIndex + xmlFormat.start.length] !== '>' && file.data[_rowStartPosVerifyIndex + xmlFormat.start.length] !== ' ') {
            _rowStartPosVerifyIndex -= 1;
            continue;
          }
          break;
        }

        const _beginRowPos = _rowStartPosVerifyIndex;
        /** Check if it is the correct row, instead of a child-table within the cell */
    ...
    for (let i = 0; i < _hideRowList.length; i++) {
      if (!_hideRowList[i].marker || !_hideRowList[i]?.type || !_xmlFormat?.[_hideRowList[i]?.type]) {
        continue;
      }
      const _hideRowRegex = new RegExp(`${helper.regexEscape(_hideRowList[i].marker + _hideRowList[i]?.formatter)}[^}]*}`, 'g');
