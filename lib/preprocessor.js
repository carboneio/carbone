...
var preprocessor = {
  ...
  handleHideFormatter : function (template, format) {
    ...
    while ((res = _hideFormatterRegex.exec(_file.data)) !== null) {
      let _marker = '';
    ...
    const extractXmlElement = (file, index, xmlFormat) => {
      const hidePos = index;
    ...
    /**
     * todo:
     * - [ ] get positions to insert hideBegin/hideEnd without transforming the XML
     * - [ ] Create a list of position [{pos: X, marker: '' }]
     * - [ ] Insert all markers at the correct place
     */
    const _newMarkerPosList = [];
    for (let i = 0; i < _hideList.length; i++) {
      if (!_hideList[i].index, !_hideList[i].marker || !_hideList[i]?.type || !_xmlFormat?.[_hideList[i]?.type]) {
        continue;
      }
      const _tagPos = extractXmlElement(_file, _hideList[i].index, _xmlFormat?.[_hideList[i]?.type]);
      if (_tagPos.start < 0 || _tagPos.end < 0) {
        continue;
      }
      const _foundEndPos = _newMarkerPosList.find(el => el.pos === _tagPos.end);
      /** Remove duplicated */
      if (_foundEndPos && _foundEndPos.xml.indexOf(_hideList[i].marker) !== -1) {
        continue;
      }
      if (_foundEndPos) {
        _foundEndPos.xml = `<carbone>${_hideList[i].marker}:hideEnd}</carbone>` + _foundEndPos.xml;
      } else {
        _newMarkerPosList.push({ pos : _tagPos.end, xml : `<carbone>${_hideList[i].marker}:hideEnd}</carbone>` });
      }
      const _foundStartPos = _newMarkerPosList.find(el => el.pos === _tagPos.start);
      if (_foundStartPos) {
        _foundStartPos.xml += `<carbone>${_hideList[i].marker}:hideBegin}</carbone>`;
      } else {
        _newMarkerPosList.push({ pos : _tagPos.start, xml : `<carbone>${_hideList[i].marker}:hideBegin}</carbone>` });
      }
    }
    _newMarkerPosList.sort((a, b) => b.pos - a.pos);
    console.log(_newMarkerPosList);
    ...
    _file.data = _file.data.replace(/{d.[^{]*:hide(?!Begin)(?!End)\(?[^(}]*}/g, '');
