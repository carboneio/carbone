...
var preprocessor = {
  ...
  handleDropFormatter : function (template, format) {
    ...
    if (format === 'odt' || format === 'odp') {
      ...
      if (format === 'odp') {
        ...
        // in ODP, we must remove the whole frame, otherwise, it shows an SVG fallback
        _xmlFormat.table = {
          start : '<draw:frame',
          end   : '</draw:frame>'
        };
  ...
  preParseXML : function (xml, options) {
    ...
    const _indexOfOptionTagArithmetic = xml?.indexOf('{o.useHighPrecisionArithmetic=true}');
    if (_indexOfOptionTagArithmetic !== -1) {
      options.useHighPrecisionArithmetic = true;
    }
    return xml;
  },
  /**
