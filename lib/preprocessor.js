    ...
    const extractTableRow = (file, index) => {
      ...
      */
      ...
      while (rowStartPos >= 0) {
        ...
        let _endRowPos = file.data.indexOf('</table:table-row>', rowStartPos);
      ...
      while (rowEndPos < file.data.length) {
        ...
        let _beginRowPos = file.data.lastIndexOf('<table:table-row>', rowEndPos)
      ...
      return {
        xml: ((rowStartPos === -1 || rowEndPos === -1) ? file.data : file.data.substring(rowStartPos, rowEndPos)).replace(/{[^{]*:hideRow[^}]*}/g, ''),
        start: rowStartPos,
        end: rowEndPos
      }
    }
    ...
    */
    ...
    for (let i = 0; i < _hideRowPos.length; i++) {
      ...
      if (!_foundRow) {
        ...
        _foundRow = extractTableRow(_file, _hideRowPos[i].index);
