...
var preprocessor = {
      ...
      try {
        switch (_fileType) {
          ...
          case 'xlsx':
            ...
            break;
          case 'docx':
            preprocessor.handleHideFormatters(template, 'docx')
            ...
            break;
          case 'odt':
            preprocessor.handleHideFormatters(template, 'odt')
  ...
  handleHideFormatters : function (template, format) {
    let _file = null;
    let _xmlFormat = null;
    ...
    /**
     * Find all hide expressions
     */

    const _hideRowPattern = /:(hideRow|hideImage|hideP)(\((-?[\d]*)\))?/g
    const _hideRowList = [];
    ...
    const extractTableRow = (file, index, xmlFormat) => {
      ...
      */
      ...
      while (rowStartPos >= 0) {
        ...
        if (rowStartPos !== -1 && file.data[rowStartPos + xmlFormat.start.length] !== '>' && file.data[rowStartPos + xmlFormat.start.length] !== ' ') {
          rowStartPos -= 1;
          continue;
        }
        /** Check if it is the correct row, instead of a child-table within the cell */
        const _endRowPos = file.data.indexOf(xmlFormat.end, rowStartPos); // E.G. Searching '</table:table-row>'
      ...
      console.log("Row start pos:", childEndRowPosFound, rowStartPos);

      let rowEndPos = hideRowPos;

      while (rowEndPos < file.data.length) {
        rowEndPos = file.data.indexOf(xmlFormat.end, rowEndPos) // E.G. Searching '</table:table-row>'

        let _tmpRowEndPos = rowEndPos;
        while (_tmpRowEndPos !== -1) {
          _tmpRowEndPos = file.data.lastIndexOf(xmlFormat.start, _tmpRowEndPos)  // E.G. Searching '<table:table-row>'
          if (_tmpRowEndPos !== -1 && file.data[_tmpRowEndPos + xmlFormat.start.length] !== '>' && file.data[_tmpRowEndPos + xmlFormat.start.length] !== ' ') {
            console.log('SKIP:',_tmpRowEndPos , file.data.substring(_tmpRowEndPos, _tmpRowEndPos + 5))
            _tmpRowEndPos -= 1;
            continue;
          }
          break;
        ...
        const _beginRowPos = _tmpRowEndPos;
        console.log("Found:", _beginRowPos);
        /** Check if it is the correct row, instead of a child-table within the cell */
      ...
      // console.log("Row end pos:", childBeginRowPosFound, rowEndPos, xmlFormat);

      return {
        xml: ((rowStartPos < 0 || rowEndPos < 0) ? file.data : file.data.substring(rowStartPos, rowEndPos)),
        start: rowStartPos,
        end: rowEndPos
      }
    }

    // console.log(_hideRowList);

    for (let i = 0; i < _hideRowList.length; i++) {
      if (!_hideRowList[i].marker || !_hideRowList[i]?.type) {
        continue;
      }
      const _hideRowRegex = new RegExp(`${helper.regexEscape(_hideRowList[i].marker)}:${_hideRowList[i]?.type}[^}]*}`, 'g');
      const _hideRowIndex = _file.data.search(_hideRowRegex);
      const _row = extractTableRow(_file, _hideRowIndex, _xmlFormat?.[_hideRowList[i]?.type]);
      console.log(_row,_hideRowIndex, _hideRowRegex)
      if (_row.start < 0 || _row.end < 0) {
        continue;
      }
      _file.data = `${_file.data.substring(0, _row.start)}<carbone>${_hideRowList[i].marker}:hideBegin}</carbone>${_row.xml.replace(_hideRowRegex, '')}<carbone>${_hideRowList[i].marker}:hideEnd}</carbone>${_file.data.substring(_row.end)}`;
    }
