...
var preprocessor = {
  ...
  handleHideFormatter : function (template, format) {
    ...
    while ((res = _hideFormatterRegex.exec(_file.data)) !== null) {
      ...
      const _hideIndex = res?.index;
      const _type = (helper.removeQuote(res?.[2] || '')).trim();

      if (!_marker || !_hideIndex || !_type || !_xmlFormat?.[_type]) {
        continue;
      }
      /** get positions of the element to insert hideBegin/hideEnd without transforming the XML */
      const _tagPos = findPosXmlElement(_file, _hideIndex, _xmlFormat?.[_type]);
      ...
      if (_foundEndPos && _foundEndPos.xml.indexOf(_marker) !== -1) {
        continue;
      }
      /** if a position is already saved for the hideEnd, the marker is moved before the existing marker */
      if (_foundEndPos) {
        _foundEndPos.xml = `<carbone>${_marker}:hideEnd}</carbone>` + _foundEndPos.xml;
      }
      else {
        /** Create a new end position [{pos: X, xml: '' }] */
        _newMarkerPosList.push({ pos : _tagPos.end, xml : `<carbone>${_marker}:hideEnd}</carbone>` });
      }
      ...
      if (_foundStartPos) {
        _foundStartPos.xml += `<carbone>${_marker}:hideBegin}</carbone>`;
      }
      else {
        _newMarkerPosList.push({ pos : _tagPos.start, xml : `<carbone>${_marker}:hideBegin}</carbone>` });
      }
    }
    ...
    /** Remove all makers that include a ":hide" formatter  */
