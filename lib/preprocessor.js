...
var preprocessor = {
  ...
  handleHideFormatter : function (template, format) {
    ...
    const findPosXmlElement = (file, index, xmlFormat) => {
      const hidePos = index;
    ...
    _file.data = _file.data.replace(_hideFormatterRegex, (markerFull, markerWithoutHide, typeWithQuote, hideIndex) => {
      ...
      _allHideMarkers.push({ index : (hideIndex - _totalTextRemoved), withoutHide : markerWithoutHide, format : _xmlFormat[_type] });
    ...
    for (let i = 0; i < _allHideMarkers.length; i++) {
      ...
      const _tagPos = findPosXmlElement(_file, _marker.index, _marker.format);
