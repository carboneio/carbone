...
var preprocessor = {
  ...
  execute : function (template, options, callback) {
    ...
    if (_onlySetMarkers.length > 0) {
      for (let i = _onlySetMarkers.length - 1; i >= 0; i--) {
        template.files.unshift({
          name     : '___CARBONE___',
          data     : _onlySetMarkers[i],
          isMarked : true
        });
      }
    }
      ...
      try {
        switch (_fileType) {
          case 'html':
            ...
            if (params.isEnterpriseEditionMode === true) {
  ...
  /**
   * Finds a first tag for batch processing
   *
   * @param      {Object}   template  The template
   * @return     {String}   the first tag found which starts by {d
   */
  findFirstTag : function (template) {
    const _tagRegex = /{(d[^{}]+?)}/g;
    for (let i = 0, len = template.files.length; i < len; i++) {
      const _xml = template.files[i].data;
      if (typeof(_xml) === 'string') {
        let _firstTag = _tagRegex.exec(_xml);
        if (_firstTag?.length > 1 && parser.isCarboneMarker(_firstTag[0]) === true) {
          return _firstTag[1];
        }
      }
    }
    ...
    var _inlinedXml = xml.replace(/(<(\w)[^>]*t="s"[^>]*>)(.*?)(<\/\2>)/g, function (m, openTag, tagName, content, closeTag) {
      ...
      if (_tab instanceof Array && _tab.length > 0) {
        ...
        if (_contentAsNumber instanceof Array && _tab.length > 0) {
          ...
          const _withTypeFormatter = _contentAsNumber[0].replace(/:formatN(?:\(\s*\d*\s*\))?/, ':toExcelType');
          const _withValueFormatter = _contentAsNumber[0].replace(/:formatN(?:\(\s*\d*\s*\))?/, ':toExcelValue');
          _newXml = openTag.replace('t="s"', 't="'+_withTypeFormatter+'"');
          _newXml += _withValueFormatter;
        }
        else {
  ...
  convertNumberMarkersIntoNumericFormat : function (template) {
    ...
    if (_contentFileId > -1 && !!template.files[_contentFileId] === true) {
      template.files[_contentFileId].data = template.files[_contentFileId].data.replace(/<table:table-cell[^<]*>\s*<text:p>[^<]*formatN[^<]*<\/text:p>\s*<\/table:table-cell>/g, function (xml) {
        ...
        return xml;
      }).replace(/<style:table-row-properties[^>]+\/>/g, function (rowStyle) {
        if (rowStyle.indexOf(' style:use-optimal-row-height="true"') !== -1) {
          // force LibreOffice to recompute row height to avoid this bug: https://github.com/carboneio/carbone-ee/issues/748
          // all the discussion about this issue is here: https://bugs.documentfoundation.org/show_bug.cgi?id=62268
          return rowStyle.replace(/ style:row-height="[^"]+"/g, '');
        }
        return rowStyle;
      });
    }
