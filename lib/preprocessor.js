...
// var colorOld = require('./color.old');
...
var preprocessor = {
  ...
  execute : function (template, data, options, callback) {
    if (!(callback instanceof Function)) {
      // fast fix to test everything before publishing
      callback = data;
    }
    ...
    for (var i = -1; i < template.embeddings.length; i++) {
      ...
      switch (_fileType) {
        case 'xlsx':
          ...
          break;
        case 'docx':
          // colorOld.replaceColorMarkersDocx(template);
          ...
          break;
        case 'odt':
          color.preProcessOdt(template, options);
          // colorOld.replaceColorMarkersOdt(template);
