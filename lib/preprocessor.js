...
var preprocessor = {
  ...
  execute : function (template, callback) {
    ...
    for (var i = -1; i < template.embeddings.length; i++) {
      ...
      // Clean XML tags inside Carbone markers
      preprocessor.removeXMLInsideMarkersLoop(template);
      switch (_fileType) {
        case 'xlsx':
  ...
  /**
   * @description loop through files to Remove XML tags inside markers
   * @param {Object} template
   */
  removeXMLInsideMarkersLoop : function (template) {
    for (let i = 0, j = template.files.length; i < j; i++) {
      template.files[i].data = parser.removeXMLInsideMarkers(template.files[i].data);
    }
  },
  /**
