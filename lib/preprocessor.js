...
var preprocessor = {
  ...
  handleHideFormatter : function (template, format) {
    ...
    const extractXmlElement = (file, index, xmlFormat) => {
      ...
      return {
        xml: ((startTagPos < 0 || endTagPos < 0) ? file.data : file.data.substring(startTagPos, endTagPos)),
        start: startTagPos,
        end: endTagPos
      }
    }
    ...
    /**
     * Loop to:
     * 1. Search every hide markers
     * 2. Extract the XML element to hide
     * 3. Inject hideBegin/hideEnd around the XML element
     * 4. Start again on step 1
     */
    for (let i = 0; i < _hideList.length; i++) {
      if (!_hideList[i].marker || !_hideList[i]?.type || !_xmlFormat?.[_hideList[i]?.type]) {
        continue;
      }
      const _hideRegex = new RegExp(`${helper.regexEscape(_hideList[i].marker + _hideList[i]?.formatter)}[^}]*}`, 'g');
      const _hideIndex = _file.data.search(_hideRegex);
      const _row = extractXmlElement(_file, _hideIndex, _xmlFormat?.[_hideList[i]?.type]);

      if (_row.start < 0 || _row.end < 0) {
        continue;
      }
      _file.data = `${_file.data.substring(0, _row.start)}<carbone>${_hideList[i].marker}:hideBegin}</carbone>${_row.xml.replace(_hideRegex, '')}<carbone>${_hideList[i].marker}:hideEnd}</carbone>${_file.data.substring(_row.end)}`;
    }
  },
  /**
