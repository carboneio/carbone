...
var preprocessor = {
    ...
    for (var i = -1; i < template.embeddings.length; i++) {
      ...
      switch (_fileType) {
        case 'xlsx':
          ...
          break;
        case 'ods':
          preprocessor.convertNumberMarkersIntoNumericFormat(template, function (err, template) {
   ...
   * @return
   */
  convertNumberMarkersIntoNumericFormat : function (template, callback) {
    const _contentFileId = template.files.findIndex(x => x.name === 'content.xml');
    if (_contentFileId > -1 && !!template.files[_contentFileId] === true) {
      template.files[_contentFileId].data = template.files[_contentFileId].data.replace(/<table:table-cell[\w\s\d:=\-"]* office:value-type="string"[\w\s\d:=\-"]*>[\n\s]*<text:p>[\n\s]*{\s?[d|c].[\w\d]*:formatN\([\w\d]*\)\s?}[\n\s]*<\/text:p>[\n\s]*<\/table:table-cell>/g, function (xml) {
        xml = xml.replace(/:formatN\(.*\)/, '');
        const _marker = /{.*}/.exec(xml)[0];
    ...
    return callback("[ODS File] - content.xml doesn't exit", template);
  }
};
