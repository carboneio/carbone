...
var preprocessor = {
  ...
  execute : function (template, callback) {
    ...
    for (var i = -1; i < template.embeddings.length; i++) {
      ...
      // If the current template is a String == is a embedded document
      if (_mainOrEmbeddedTemplate.constructor === String) {
        // Set the template extension to the document extension
        template.extension = path.extname(_mainOrEmbeddedTemplate).toLowerCase().slice(1);
      }
      // Determine file extension
      var _fileType = template.extension || path.extname(_mainOrEmbeddedTemplate.filename).toLowerCase().slice(1);
      switch (_fileType) {
        case 'xlsx':
