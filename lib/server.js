...
function startServer () {
  ...
  serverSocket.on('message', function (req) {
    ...
    if (_message.input === undefined) {
      // backward compatibility with v0.10
      converter.convertFile(_message.inputFile, _message.format, _message.formatOptions, function (err) {
        req.send({error : err});
      }, true, _message.outputFile);
    }
    else {
      fs.writeFile(_message.inputFile, new Buffer(_message.input, 'base64'), function (err) {
        if (err) {
          return req.send({
            error  : err,
            output : null
        ...
        converter.convertFile(_message.inputFile, _message.format, _message.formatOptions, function (err, result) {
          req.send({
            error  : err,
            output : (err === null ) ? result.toString('base64') : null
          });
        }, false, null, _message.exportFilters);
      });
    }
  });
