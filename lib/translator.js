 ...
 */
  handleParams: function (args) {
    ...
    {
      translator.generateLang(_dirpath,_lang,function(err,newObjLang,bodyMsg,keyTypeCountArray){
        if(!err){
 ...
 */
  generateLang: function (dir,lang,callback){
      ...
      if(!fs.existsSync(_templatePath)){
        ...
        translator.reportsOldAndNewKeys(_files,_oldObjLang,_oldKeys,_newKeys,_oldObjLangKeyNotFound,0,function(err,newObjLang,keyTypeCountArray){
           //create and fill the file
          fs.writeFileSync(_templatePath, JSON.stringify(newObjLang, null, 2));
          _bodyMsg += '\nNews translate keys found :'+keyTypeCountArray[0];
          callback(err,newObjLang,_bodyMsg,keyTypeCountArray);
        });
      }else{
        _bodyMsg = 'The lang file : '+lang+'.json has been found';
        _langFileData = fs.readFileSync(_templatePath);
        _oldObjLang = JSON.parse(_langFileData);
        ...
        translator.reportsOldAndNewKeys(_files,_oldObjLang,_oldKeys,_newKeys,_oldObjLangKeyNotFound,0,function(err,newObjLang,keyTypeCountArray){
          ...
          fs.writeFileSync(_templatePath, JSON.stringify(newObjLang, null, 2));
          _bodyMsg += '\nNews translate keys found : '+keyTypeCountArray[0];
          _bodyMsg += '\nTranslate keys found keep or updated : '+keyTypeCountArray[1];
          _bodyMsg += '\nTranslate keys not found : '+keyTypeCountArray[2];
          _bodyMsg += '\nOlds translate keys not found : '+keyTypeCountArray[3];
          callback(err,newObjLang,_bodyMsg,keyTypeCountArray);
        });
      }
  },
  ...
  reportsOldAndNewKeys: function(files,oldObjLang,oldKeys,newKeys,oldObjLangKeyNotFound,index,callback){
    if(index < files.length ) {
      ...
      translator.updateALangFile(oldKeys,newKeys,oldObjLangKeyNotFound,function(err,newObjLang,keyTypeCountArray){
        callback(err,newObjLang,keyTypeCountArray);
      });
    }
  },
  ...
  updateALangFile: function(oldKeys,newKeys,oldObjLangKeyNotFound,callback){
    ...
    if(oldObjLangKeyNotFound)
    {
      ...
      var _oldObjLangKeyNotFound = Object.keys(oldObjLangKeyNotFound);

       //Firstly, stock the old not found keys
      _oldObjLangKeyNotFound.forEach(function(keyNotFound) {
        if(keyNotFound.substring(0,3)!=='<!>')
        {
          _oldKeysRemovedToSorted['<!>'+keyNotFound] = oldObjLangKeyNotFound[keyNotFound]
    ...
    var _countNewKeys= 0;
    //write news keys
    _newKeys.forEach(function(newKey) {
      _newObjLang[newKey] = '';
      _countNewKeys++;
    });
    ...
    var _countOldKeys= 0;
    //write the old keys
    _oldKeys.forEach(function(oldKey) {
      _newObjLang[oldKey] = oldKeys[oldKey];
      _countOldKeys++;
    });

    _keyTypeCountArray.push(_countNewKeys);
    _keyTypeCountArray.push(_countOldKeys);
    _keyTypeCountArray.push(_countNewOldObjectLangKeyNotFound);
    _keyTypeCountArray.push(_countOldObjectLangKeyNotFound);

    callback(null,_newObjLang,_keyTypeCountArray);
  }
}
