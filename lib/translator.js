...
var usage = [
  '\n/********************************************************************************\\',
                              ' Translator usage:',
  '\n -l [lang] : find all t() markers and insert them in a new file [lang].json.',
  '\n create this file if not exist. If the file exist, delete unused translator keys ',
  '\n and keep existant translator markers and add the new ones.',
  '\n\\********************************************************************************/'
].join('\n');
  ...
  handleParams: function (args) {
    ...
    switch(_argument){
      ...
      case "-t":
        if(_lang)
        {
          console.log('Carbone: generate the lang file : '+_lang+'.json in progress');
          var carboneParamsFilePath = path.resolve(__dirname+'/../../../middlewares', "carbone-params.js");
          var template_path = params.templatePath;
          fs.readFile(carboneParamsFilePath, function(err, content){
            if(!err){
              template_path = __dirname+'/../../../middlewares';
            }else{

            }
            translator.generateLang(template_path,_lang);
          });
        }else{
      ...
      if(data !== undefined )
      {
        ...
        for (var j = 0; j < _files.length; j++) {
          ...
          file.openTemplate(_filePath, function(err, template){
            ...
            //Firsly, write news keys
            var newObjLang = {};
            newObjectLangKeys.forEach(function(newKey) {
              newObjLang[newKey] = '';
            });

            //Secondly, write the old keys
