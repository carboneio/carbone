...
var translator = {
  ...
  handleParams : function (args) {
    var _translatorCommandError = false;
    var _lang = null;
    var _dirpath = null;
    var _errorMessage = 'Error : Invalid argument : write generateLang --help or -h for assistance ';
      ...
      switch (_argument) {
        ...
        case '-p':
          ...
          if (!fs.existsSync(_dirpath)) {
            _translatorCommandError = true;
            _errorMessage = 'Error : Invalid  directory path argument : This directory path does not exist';
          }
          break;
        case '--help':
        case '-h':
    ...
    if (_translatorCommandError === true) {
      console.log(CARBONE_HEADER + _errorMessage + CARBONE_FOOTER);
      process.exit();
    }
    if (_lang !== null && _dirpath !== null) {
      translator.generateLang(_dirpath, _lang, function (err, newObjLang, bodyMsg) {
        if (!err) {
  ...
  generateLang : function (dir, lang, callback) {
    ...
    if (!fs.existsSync(dir)) {
      _errorMessage = 'docs folder does not exist';
      return callback(_errorMessage, null, null);
    }

    if (!fs.existsSync(_dirLangPath)) {
      fs.mkdirSync(_dirLangPath, parseInt('0766',8), function (err) {
        if (err) {
          _errorMessage = "error! Can't create the lang directory!";
          console.log(_errorMessage + CARBONE_FOOTER);
          callback(_errorMessage, null, null);
        }
      });
    }
    ...
    if (index < files.length) {
      ...
      translator.reportTranslatorMarker(_filePath, oldObjLang, oldKeys, newKeys, oldObjLangKeyNotFound,
        function (err, oldObjLangKeys, newObjLangKeys, oldObjLangKeyNotFound) {
          var _oldObjLangKeyNotFound = oldObjLangKeyNotFound;
          var _oldKeys = oldObjLangKeys;
    ...
    file.openTemplate(filePath, function (err, template) {
      for (var k = 0; k < template.files.length; k++) {
        ...
        if (_file.isMarked === true) {
          ...
          xml.replace(/\{\s*((?:\s*<[^>]*>\s*)*t[\s\S]+?)\}/g, function (m, text) {
  ...
  updateALangFile : function (oldKeys, newKeys, oldObjLangKeyNotFound, callback) {
    ...
    if (oldObjLangKeyNotFound) {
      ...
      _oldKeysRemoved.sort(translationSort);
