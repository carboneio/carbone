...
function handleParams(args) {
  ...
  switch(_argument){
    ...
    case "-t":
      if(_lang)
      {
        console.log('Carbone: generate the lang file : '+_lang+'.json');
        generateLang(params.templatePath,_lang);
      }else{
...
/*****************************************************************************************************************/
   ...
   */
function generateLang(dir,lang,callback){
    var _res = {};

    var _dirPath = params.templatePath+'/lang';
    var _templatePath =  path.resolve(_dirPath+'/', lang+'.json');
    if(!fs.existsSync(_dirPath)){
     fs.mkdirSync(_dirPath, 0766, function(err){
       if(err){ 
         console.log(err);
