...
function addTemplate (req, res, next) {
  ...
  try {
    ...
    res.send({ success : false, error : 'Try uploading your file using form data' }, 400);
    return next();
  }
  ...
  bb.on('file', (fieldname, file, filename) => {
    ...
    if (fieldname === 'template') {
      computeHash(hashStream, payload, (err, hashId) => {
        if (err) {
          res.send({ success : false, error : 'Error when uploading file' }, 500);
          return next();
        }
        ...
        writeTemplateLocally(fileWriteStream, hashId, (err) => {
          if (err) {
            ...
            res.send({ success : false, error : err.message || err.toString() }, 500);
            return next();
          }
  ...
  bb.on('field', (fieldname, val) => {
    if (fieldname !== 'payload') {
      res.send({ success : false, error : 'Request Error: "' + fieldname + '" is an unknown field, do you mean payload?' }, 400);
      return next();
    }
...
function writeTemplateLocally (readStream, hashId, callback) {
  ...
  writeStream.on('error', (err) => {
    console.log('Add template error: ' + err.toString());
