...
MIGbMBAGByqGSM49AgEGBSuBBAAjA4GGAAQA1xJ3Vidk7Bu1gpI/2GmV4KnMmQrI\n\
FFYvowFpRb2KpHgZghEkcYSUFWbDRgTIld4+RIPl6wOCfcZJe1/gWM3CpfMB940S\n\
F36jzoLy/9sNXXokdfbcCC/7UNvp3xJmC4cwXh9dR8SDkgR6ts+zCUc3et0IMg77\n\
pJtZ+w46qxb+XclADbo=\n\
-----END PUBLIC KEY-----\n\
';
...
/**
 * Check if user is using binary and check license file is valid
 * @param {Function} callback
 */
function checkLicense (callback) {
  if (package.carboneBuildDate === undefined) {
    return callback(null);
  }

  fs.readFile(path.join(params.workDir, 'license.txt'), 'utf8', (err, license) => {
    if (err) {
      return callback(new Error('Missing license file'));
    }

    kittenJwt.verify(license, PUBLIC_KEY, (err, payload) => {
      if (err) {
        return callback(err);
      }
      ...
      if (package.carboneBuildDate > expirationDate) {
        console.log('Your license is out of date, please update is on https://account.carbone.io or use an older version of carbone ee');
        process.exit(1);
      }

      if (new Date().getTime() > expirationDate) {
        console.log('WARNING: Your license is out of date, please update it on https://account.carbone.io');
      }
...
function startServer () {
  ...
  service.start(params.port, params.bind).then(() => {
...
function initWorkingDirectory () {
  ...
  try { fs.mkdirSync(params.templatePath, { recursive: true }); } catch (e) {}; // eslint-disable-line
  try { fs.mkdirSync(params.renderPath, { recursive: true });   } catch (e) {}; // eslint-disable-line
  try { fs.mkdirSync(params.assetPath, { recursive: true });    } catch (e) {}; // eslint-disable-line
  try { fs.mkdirSync(params.configPath, { recursive: true });   } catch (e) {}; // eslint-disable-line
  try { fs.mkdirSync(params.pluginPath, { recursive: true });   } catch (e) {}; // eslint-disable-line
