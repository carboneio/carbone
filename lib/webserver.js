...
/**
 * If the template is ephemeral (base64), readTemplate plugin is never called
 * 
 * And the ephemeral template is stored temporarily as a file.
 * It would be better to avoid this but it is simpler for now
 *
 */
function readTemplateWrapper (req, res, templateId, callback) {
  if (isEphemeralTemplate(req) === false) {
...
/**
 * Determines whether the render request is done with an ephemeral template.
 *
 * @param   {Object}   req     The request
 * @return  {boolean}  True if the specified request is ephemeral template, False otherwise.
 */
function isEphemeralTemplate (req) {
  return req.params?.templateId === 'template' && req.body?.template?.length > 0;
}
...
function renderReport (req, res, templateAbsolutePath, data, renderOptions) {
  try {
    ...
    carbone.render(templateAbsolutePath, data, renderOptions, (err, reportAbsolutePath, reportName, debugInfo) => {
      ...
      if (isEphemeralTemplate(req) === true) {
        fs.unlink(templateAbsolutePath, () => {
          console.log('Warning: Ephemeral template cannot be deleted: '+ templateAbsolutePath);
        });
      }
