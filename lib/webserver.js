...
function handleParams (args, callback = () => {}) {
  ...
  parseConfig((err) => {
    ...
    console.log('License:');
    console.log('- Trial period valid until 6th October 2021');
...
function renderReport (req, res, input, data, renderOptions) {
  try {
    carbone.render(input, data, renderOptions, (err, data, reportName, extension) => {
      if (err) {
      ...
      let _ext = extension;
      if (typeof(renderOptions.convertTo) === 'object' && renderOptions.convertTo.formatName) {
        _ext = renderOptions.convertTo.formatName;
      }
      else if (renderOptions.convertTo) {
        _ext = renderOptions.convertTo;
      }

      reportName = encodeURIComponent(`${reportName}.${_ext}`);

      if (storageAddon && storageAddon.onRenderEnd !== undefined) {
        return storageAddon.onRenderEnd(req, res, reportName, data, (err, newReportName) => {
          if (err) {
            return res.send({
              success : false,
              error   : 'Error while rendering template ' + err.toString()
          ...
          return writeRenderedFile(req, res, newReportName, data);
        });
      }

      return writeRenderedFile(req, res, reportName, data);
    });
  }
  catch (e) {
