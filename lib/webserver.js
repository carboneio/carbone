...
function startServer () {
  ...
  service.start(params.port).then(() => {
    ...
    webhookRenderJobInterval = setInterval(runWebhookRenderJob, WEBHOOK_RENDER_JOB_INTERVAL_TIME);
  });
}
 ...
 *
 */
function runWebhookRenderJob (newQueueSize) {
  ...
  fs.readdir(params.queuePath, (err, files) => {
    if (err) {
      console.log('Webhook Render queue - cannot retreive queue: ', err.toString());
      return runWebhookRenderJob(0);
    }

    if (files && files.length === 0) {
      console.log('Webhook Render queue - empty');
      return runWebhookRenderJob(0);
    }
    ...
    fs.readFile(_queueItemPath, 'utf8', (err, content) => {
      if (err) {
        console.log(`Webhook Render queue - cannot read queue item ${_queueItemName}: ${err.toString()}`);
        return runWebhookRenderJob(_queueSize - 1);
      }
      fs.unlink(_queueItemPath, (err) => {
        if (err) {
          console.log(`Webhook Render queue - cannot delete queue item ${_queueItemName}: ${err.toString()}`);
          return runWebhookRenderJob(_queueSize - 1);
        }
        let _queueItemData = {};
        try {
          ...
          console.log(`Webhook Render queue -  JSON not valid ${_queueItemName}: ${err.toString()}`);
          return runWebhookRenderJob(_queueSize - 1);
        }
        const _res = {
          send : (response) => {
            try {
              get.concat({
                ...
                if (err) {
                  console.log(`Webhook Render queue - Send render ID failled ${_queueItemName}: ${err.toString()} ${data}`);
                }
                else {
                  console.log('Webhook Render queue - Send render ID success');
                }
                return runWebhookRenderJob(_queueSize - 1);
              });
            }
              ...
              console.log(`Webhook Render queue - Send render ID failled ${_queueItemName}: ${err.toString()}`);
              return runWebhookRenderJob(_queueSize - 1);
            }
          }
        };
