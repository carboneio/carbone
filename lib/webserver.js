...
function setDefaultStoragePlugins () {
  ...
  storageAddon = {
    ...
    writeTemplate  : (req, res, templatePath, hashId, next) => { return next(); }, // eslint-disable-line
    ...
    // TODO inverse  reportName, reportPath to be like carbone
...
function generateReport (req, res, next) {
  ...
  storageAddon.beforeRender(req, res, _renderOptions.data, _renderOptions, (err) => {
    if (err) {
      res.send(returnMessage({ errorCode : 'w114' }), 400);
      return next();
    }
    storageAddon.readTemplate(req, _templateId, (err, templateAbsolutePath) => {
      if (err) {
        res.send({ success : false, error : err.toString() }, 400);
        return next();
      }
      return renderReport(req, res, templateAbsolutePath, _renderOptions.data, _renderOptions, next);
    });
  });
}
 ...
 */
function renderReport (req, res, templateAbsolutePath, data, renderOptions, next) {
  try {
    ...
    carbone.render(templateAbsolutePath, data, renderOptions, (err, reportAbsolutePath, reportName) => {
      ...
      storageAddon.onRenderEnd(req, res, reportName, reportAbsolutePath, _statistics, (err) => {
        if (err) {
          res.send(returnMessage({ errorCode : 'w101', args : [err.toString()] }), 500);
          return next();
        }
        res.send(returnMessage(null, null, { renderId : _renderId }));
        return next();
      });
    });
  }
...
function getRenderedReport (req, res, next) {
  ...
  storageAddon.readRender(req, res, _renderId, (err, renderAbsolutePath) => {
    if (err) {
      ...
      res.send(returnMessage({ errorCode : 'w102' }));
      return next();
    }
...
function addTemplate (req, res, next) {
  ...
  form.parse(req, (error, fields, files) => {
    ...
    computeHash(files.template.path, payload, (err, hashId) => {
      ...
      storageAddon.writeTemplate(req, res, _templatePathTemp, hashId, (err) => {
        if (err) {
          console.log('write template error: ', err);
          res.send(returnMessage({ errorCode : 'w109' }), 400);
          return next();
        }
        console.log(`file uploaded : ${_templatePathTemp} => ${hashId}`);
        fs.rename(_templatePathTemp, path.join(params.templatePath, hashId), (err) => {
          if (err) {
