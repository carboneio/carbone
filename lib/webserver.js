...
function renderWebhookQueue (newQueueSize) {
  ...
  if (newQueueSize <= 0) {
    /** Finished rendering reset the queue. The method will be run next time interval */
    webhookQueueRendering = false;
    return;
  }

  if (webhookQueueRendering === true && !newQueueSize) {
    /** A `renderWbhookQueue` is already processing and this instance should be stopped, the method has been fired because of the time interval */
    return;
  }
        ...
        const _res = {
          send : (response) => {
            try {
              ...
              console.log(`render wekhook queue - Send render ID failled ${_queueItemName}: ${err.toString()}`);
              return renderWebhookQueue(_queueSize - 1);
            }
          }
        };
