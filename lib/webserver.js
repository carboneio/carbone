...
let webhookQueueRendering = false;
...
const WEBHOOK_INTERVAL_TIME = 1 * 20 * 1000;
const SERVER_TIMEOUT = 62000;
...
const errorCode = {
  ...
  w117 : () => 'Error while preparing the render webhook'
};
...
function startServer () {
  ...
  service.start(params.port).then(() => {
    ...
    webhookInterval = setInterval(renderWebhookQueue, WEBHOOK_INTERVAL_TIME);
  });
}
...
function initWorkingDirectory () {
  ...
  try { fs.mkdirSync(params.queuePath  , { recursive: true, mode: 0o750 }); } catch (e) {console.log(e)}; // eslint-disable-line
...
function generateReport (req, res) {
  ...
  storageAddon.beforeRender(req, res, _renderOptions.data, _renderOptions, (err) => {
    ...
    storageAddon.readTemplate(req, res, _templateId, (err, templateAbsolutePath) => {
      ...
      const _webhookSuccessUrl = req.headers['carbone-webhook-success-url'];
      if (_webhookSuccessUrl) {
        /** TODO ? verify the URL format */
        const _callbackUrl = req.headers['carbone-webhook-success-url'];
        const _queueItemName = `${Date.now()}-${(req.jwtPayload !== undefined) ? req.jwtPayload.iss : ''}.json`;
        const _queueItemData = JSON.stringify({
          /** renderReport arguments */
          successUrl           : _webhookSuccessUrl,
          templateAbsolutePath : templateAbsolutePath,
          body                 : _renderOptions,
          /** req used by renderReport */
          req                  : {
            headers    : req.headers,
            jwtPayload : req.jwtPayload,
            url        : `${req.url} to ${_callbackUrl}`,
        ...
        return fs.writeFile(_queuePath, _queueItemData, function (err) {
          if (err) {
            console.log('Error when saving new deferred render on queue: ', err.toString());
            return res.send(returnMessage({ errorCode : 'w117' }), 500);
          }
          return res.send(returnMessage(null, null, { data : {}, message : 'Document generation in progress and a render ID will be send to your callback URL.' }));
        });
      }
...
function addTemplate (req, res) {
  ...
  form.parse(req, (error, fields, files) => {
    ...
    return writeTemplateAndEnd(req, res, files.template.hash, files.template.path);
  });
}
 ...
 *
 */
function renderWebhookQueue (newQueueSize) {
  console.log("Calling renderWebhookQueue", newQueueSize, webhookQueueRendering);
  if (newQueueSize <= 0) {
    console.log("Finished rendering, reset the queue!");
    webhookQueueRendering = false;
    return;
  }

  if (webhookQueueRendering === true && !newQueueSize) {
    console.log("Already processing");
    return;
  }

  webhookQueueRendering = true;

  fs.readdir(params.queuePath, (err, files) => {
    if (err) {
      console.log('render webhook queue - cannot retreive queue: ', err.toString());
      return renderWebhookQueue(0);
    }

    if (files && files.length === 0) {
      console.log('render webhook queue empty');
      return renderWebhookQueue(0);
    }
    ...
    fs.readFile(_queueItemPath, 'utf8', (err, content) => {
      if (err) {
        console.log(`render wekhook queue - cannot read queue item ${_queueItemName}: ${err.toString()}`);
        return renderWebhookQueue(_queueSize - 1);
      }
      fs.unlink(_queueItemPath, (err) => {
        if (err) {
          console.log(`render wekhook queue - cannot delete queue item ${_queueItemName}: ${err.toString()}`);
          return renderWebhookQueue(_queueSize - 1);
        }
        console.log();
        let _queueItemData = {};
        try {
          ...
          console.log(`render wekhook queue -  JSON not valid ${_queueItemName}: ${err.toString()}`);
          return renderWebhookQueue(_queueSize - 1);
        }
        const _res = {
          send : (response) => {
            get.concat({
              ...
              if (err) {
                console.log(`render wekhook queue - Send render ID failled ${_queueItemName}: ${err.toString()} ${data}`);
              }
              else {
                console.log('render wekhook queue - Send render ID success');
              }
              return renderWebhookQueue(_queueSize - 1);
            });
          }
