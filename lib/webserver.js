...
function handleParams (args) {
  ...
  console.log('License:');
  console.log('- Trial period valid until 31th August 2020');
...
function generateReport (req, res) {
  ...
  try {
    carbone.render(_resolvedInput, _data, _renderOptions, (err, data, reportName, extension) => {
      ...
      const _ext = _renderOptions.convertTo || extension;
...
function getRenderedReport (req, res) {
  console.log('REQUEST ' + req.method + ' ' + req.url);
  const _renderId       = encodeURIComponent(req.params.renderId);
  const _finalFilename  = path.join(params.renderPath, _renderId);

  res.setHeader('Content-Disposition', `filename="${decodeURIComponent(path.basename(_finalFilename))}"`);
...
function addTemplate (req, res) {
  ...
  form.parse(req, function ( err, fields, body) {
    ...
    computeHash(body.template.path, body.payload, (err, hashId) => {
      if (err) {
        return res.send({
          success : false,
