...
function startServer () {
  ...
  service.use((req, res, next) => {
    ...
    req.requestUID = helper.getUID(); // for egress controller
    return next();
  });
...
function generateReport (req, res) {
  ...
  // for egress controller
  _renderOptions.issuer = req.jwtPayload?.iss;
  _renderOptions.ruid = req.requestUID;
          ...
          send : (response) => {
            try {
              rock({
                ...
                issuer  : _queueItemData?.req?.jwtPayload?.iss, // for egress controller
                headers : {
