...
const MAX_DATA_SIZE = 60 * 1024 * 1024; // for rendering
let studioCache = '';
let studioIconCache = '';
...
function startServer () {
  ...
  service.post('/render/:templateId', bodyParser.json({limit : MAX_DATA_SIZE}), generateReport);
...
function addTemplate (req, res) {
  ...
  if ( _isJSONType === true ) {
    ...
    const _hashId = crypto.createHash('sha256').update(_template, 'base64').digest('hex');
