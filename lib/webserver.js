...
function generateReport (req, res) {
  ...
  req.headers['carbone-template-id'] = _templateId;
...
function renderReport (req, res, templateAbsolutePath, data, renderOptions) {
  try {
    ...
    carbone.render(templateAbsolutePath, data, renderOptions, (err, reportAbsolutePath, reportName, debugInfo) => {
      ...
      let _statistics = {
        renderId  : _renderId,
        templateId: req.headers['carbone-template-id'],
...
function verifyTemplateType (req, res, templatePath, callback) {
  carbone.getFileExtension(templatePath, (err, extension) => {
    ...
    req.headers['carbone-template-mimetype'] = Object.prototype.hasOwnProperty.call(mimetype, extension) === true ? mimetype[extension].mime : '';
    return callback();
  });
}
...
function isRenderRunning () {
  ...
  if (nbCurrentRender === 0) {
    console.log('SOFT-STOP: No more active renders. Exit in 10 seconds');
