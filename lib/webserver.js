...
const errorCode = {
  ...
  w117 : () => `With a test key, you can only convert in the following format (${params.testConvertAvailable.join(', ')})`
};
...
function checkServiceStatus (req, res, next) {
  const softStopUnavailableRoute = [
    { url : 'POST /render', check : 'startsWith' },
    { url : 'POST /template', check : 'startsWith' },
    { url : 'GET /template', check : 'startsWith' },
    { url : 'DELETE /template', check : 'startsWith' },
    { url : 'GET /', check : 'equal' }
  ];
  ...
  if (STOP) {
    ...
    for (let i = 0; i < softStopUnavailableRoute.length; i++) {
      let url = `${req.method} ${req.url}`;
      if (softStopUnavailableRoute[i].check === 'startsWith' && url.startsWith(softStopUnavailableRoute[i].url)) {
        return res.send(503);
      }
      else if (softStopUnavailableRoute[i].check === 'equal' && url === softStopUnavailableRoute[i].url) {
        return res.send(503);
      }
    }
  }
...
function startServer () {
  ...
  service.use(checkServiceStatus);
...
function _setStopForTest (which, status) {
  if (which === 'STOP') {
    STOP = status;
  }
...
function addTemplate (req, res) {
  ...
  form.parse(req, (error, fields, files) => {
    ...
    storageAddon.writeTemplate(req, res, _hashId, _templatePathTemp, (err, filename) => {
      if (err) {
        console.log('write template error: ', err);
        return res.send(returnMessage({ errorCode : 'w109' }), 400);
      }
      console.log(`Add template : ${_templatePathTemp} => ${filename}`);
      fs.rename(_templatePathTemp, path.join(params.templatePath, filename), (err) => {
        if (err) {
...
process.on('SIGTERM', () => {
  ...
  setInterval(() => {
    server.getConnections((err, count) => {
      if (count === 0) {
        console.log('No more active connections left, exit...');
  ...
  setTimeout(() => {
    console.log('End of timeout, exit');
    process.exit(0);
  }, 1000 * 30);
});

process.on('SIGUSR1', () => {
  console.log('RECEIVE SIGUSR 1');
  SOFT_STOP = true;

  setInterval(() => {
    server.getConnections((err, count) => {
      if (_renderCounter === 0 && count === 0) {
        console.log('No more active connections and download left, exit...');
        return process.exit(0);
      }
    });
  }, 50);
