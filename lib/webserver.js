...
// set JWT cache size
kittenJwt.set({serverCacheSize : 1000, serverGetPublicKeyErrorCacheExpiration : 60 * 1000 });
let authAddon = null;
...
let nbCurrentRender = 0;
let SOFT_STOP = false;
let checkIntervalStop = null;

const NB_MAX_BATCH_PER_TENANT = 1;
const queueJobByTenant = new Map();

const errorCode = {
  w100 : () => 'Template not found', // 404
  w101 : (error) => `Error while rendering template ${error}`, // 500
  w102 : () => 'Rendered file does not exist', // Not used anymore
  w103 : () => 'Cannot find template extension, does it exists?', // Not used anymore
  w104 : () => 'File not found', // 404
  w105 : () => 'Cannot delete template', // 400
  w106 : () => 'Content-Type header should be multipart/form-data (preferred) or application/json (base64 mode)', // 400
  w107 : () => 'Try uploading your file using form data', // 400
  w108 : () => 'Cannot calculate hash id', // Not used anymore
  w109 : () => 'Cannot store template', // 400
  w110 : () => '"Content-Type" header is not "application/json"', // 400
  w111 : () => 'Missing "data" property in body', // 422
  w112 : () => '"template" field is empty', // 422
  w113 : () => 'Service cannot render template', // Not used anymore
  w114 : () => 'Cannot execute beforeRender', // 500
  w115 : () => 'Invalid or undefined TemplateId or RenderId in the URL', // 400
  w116 : () => 'Cannot retrieve template', // 400
  w117 : () => 'Error while setting up the webhook', // 500
  w118 : () => 'Template format not supported, it must be an XML-based document: DOCX, XLSX, PPTX, ODT, ODS, ODP, ODG, XHTML, IDML, HTML or an XML file', // 415
  w119 : () => `Template too large, the file size limit is ${params.maxTemplateSize/1024/1024} MB`, // 413
  w120 : () => 'Unauthorized, please provide a correct API key on the "Authorization" header. The API key is available on your Carbone account: https://account.carbone.io',
  w121 : () => `Content too large, the JSON size limit is ${params.maxDataSize/1024/1024} MB`, // 413
  w122 : () => `Maximum batch processing per tenant reached (${NB_MAX_BATCH_PER_TENANT})`, // 429
};
...
function handleParams (args, callback = () => {}) {
  ...
  while (args.length > 0) {
    ...
    switch (_argument) {
      ...
      case '-S':
        ...
        break;
      case '--securityLevel':
        _cliParams.securityLevel = args.shift();
        break;
      case '--lang':
      case '-l':
      ...
      case '--license':
      case '-l':
        _cliParams.license = args.shift();
        break;
      case '--maxDataSize':
      case '-mds':
        _cliParams.maxDataSize = args.shift();
        break;
      case '--cleanIntervalTime':
      case '-ci':
        _cliParams.cleanIntervalTime = args.shift();
        break;
      case '--xlsmEnabled':
      case '-xe':
        _cliParams.xlsmEnabled = true;
        break;
      case '--help':
      case '-h':
        console.log(usage);
  ...
  console.log(helper.assignObjectIfAttributesExist(params, _configParams)); // lowest priority
  ...
  params.isEnterpriseEditionMode = false; // deactivate Enterprise feature if this parameter is defined in configuration file

  if (params.xlsmEnabled === true) {
    format.spreadsheet.xlsm = { extension : 'xlsm' , desc : 'Microsoft Excel 2007/2010 XML', format : 'Calc MS Excel 2007 VBA XML' };
  }

  helper.findCheckLicense(params, LICENSE_PUBLIC_KEY, (isValid, licenseErrorMessage, license) => {
    if (!isValid && process.pkg /* do not check in development */) {
      params.isEnterpriseEditionMode = false;
    }
    else {
      params.isEnterpriseEditionMode = true;
    }
    console.log(helper.printVersion(license, licenseErrorMessage, params.isEnterpriseEditionMode));
    // Now, all params are set, load carbone to start with a custom renderPath
    carbone = require('./index');

    // rock-req must be configured before loading plugins. Rock.defaults inheritance with global.rockReqConf
    egress.setEgressConfig(params);
    egress.collectStatistics(license?.iss); // called after rock-req init
    // Load all plugins
    ...
    formatters  = loadPlugin('formatters.js');
    ...
    if (params.studioUser.length > 2) {
      console.log('Basic authentication is enabled. Use it only in a secure environment.');
    }
    ...
    converter.init(function () {
      console.log('LibreOffice converter is ready');
      return callback();
    });
  });
}
...
function buildStudio () {
  ...
  carbone.renderXML(_html, { params }, (err, dynamicHtml) => {
    studioCache = dynamicHtml
...
function startServer () {
  service = restana({
    ...
    errorHandler (err, req, res) {
      if (req.url === '/template' && /request entity too large/.test(err) === true) {
        return res.send(returnMessage({ errorCode : 'w119' }), 413);
      }
      if (/No JSON Web Token detected in Authorization/.test(err) === true) {
        /** If a Carbone.io server, a custom message is returned */
        return res.send(params?.egressProxyPrimary?.length > 0 ? returnMessage({ errorCode : 'w120' }) : { success : false, error : err.toString() }, 401);
      }
      if (/PayloadTooLargeError/.test(err) === true) {
        return res.send(returnMessage({ errorCode : 'w121' }), 413);
      }
  ...
  service.use((req, res, next) => {
    ...
    req.requestId = helper.getUID(); // for egress controller
    return next();
  });
  ...
  if (params.studio === false) {
    // Defined before authentication middleware to always returns OK 200 when the studio is off (same page as /status)
    service.get('/', getStatus);
  }
  // Two authentication methods:
  // - JWT in general
  // - Basic Authentication if params.studioUser.length > 2
  if (params.authentication === true) {
    ...
    service.use((req, res, next) => {
      // exceptions
      if (  req.skipAuthentication === true // demo template, studio request -> TODO remove and simplify (studio should send JWT and  include demo in carbone)
        || (req.method === 'GET' && req.url === '/status')
        || (req.method === 'GET' && req.url.startsWith('/render'))) {
        return next();
      }
      // Accept basic auth if activated
      if (params.studioUser.length > 2) {
        let _auth = req.headers.authorization || req.headers.Authorization || '';
        if ( _auth.indexOf(helper.encodeSafeFilename(params.studioUser)) === -1 ) {
      ...
      // JWT for everything else
      return _verifyTokenFn(req, res, next);
    });
  }

  if (params.studio === true) {
    console.log('Studio is on');
    buildStudio();
    service.get('/', (req, res) => {
      res.setHeader('Content-Type', 'text/html');
      res.send(studioCache);
    });
  ...
  service.get('/templates', getTemplates);
  service.post('/template', bodyParser.json({limit : params.maxTemplateSize}), addTemplate);
  ...
  service.start(params.port).then(() => {
    ...
    cleanInterval  = setInterval(cleanDirectories, parseInt(params.cleanIntervalTime, 10) * 60 * 1000);
  });
}
...
function getStatus (req, res) {
  // If a gracefull exit is received, /status returns 503,
  // but internally everything works for remaining renders. The watchdog must be alive for example!
  const _communicatedStatus = (SOFT_STOP === true) ? 503 : status.code;
  return res.send({
    success : (_communicatedStatus === 200) ? true : false,
    code    : _communicatedStatus,
...
function initWorkingDirectory () {
  ...
  if (fs.existsSync(path.join(params.configPath, AUTH_KEY_NAME + '.pub')) === false && isWindows === false) {
...
function generateReport (req, res) {
  ...
  const _templateId    = validateTemplateId(req.params.templateId);
  ...
  if (_renderOptions.data === undefined) {
    return res.send(returnMessage({ errorCode : 'w111' }), 422);
  }
  ...
  // only used for egress controller
  _renderOptions.tenantId  = req.jwtPayload?.iss;
  _renderOptions.requestId = req.requestId;

  storageAddon.beforeRender(req, res, _renderOptions.data, _renderOptions, (err) => {
    if (err) {
      return res.send(returnMessage({ errorCode : 'w114' }), 500);
    }
    readTemplateWrapper(req, res, _templateId, (err, templateAbsolutePath) => {
      if (err) {
...
/**
 * If the template is volatile (base64), readTemplate plugin is never called
 *
 * And the volatile template is stored temporarily as a file.
 * It would be better to avoid this but it is simpler for now
 *
 */
function readTemplateWrapper (req, res, templateId, callback) {
  if (isVolatileTemplate(req) === false) {
    return storageAddon.readTemplate(req, res, templateId, callback);
  }
  const _templatePathTemp = path.join(params.templatePath, helper.getUID());
  if (req.body.template.length > params.maxTemplateSize) {
    return res.send(returnMessage({ errorCode : 'w119' }), 413);
  }
  fs.writeFile(_templatePathTemp, req.body.template, 'base64', (err) => {
    if (err) {
      return callback(err, _templatePathTemp);
    }
    carbone.getFileExtension(_templatePathTemp, (err) => {
      if (err) {
        fs.unlink(_templatePathTemp, () => {});
        return res.send(returnMessage({ errorCode : 'w118' }), 415); // 415: Unsupported Media Type
      }
      callback(null, _templatePathTemp);
    });
  });
}
...
/**
 * Determines whether the render request is done with an volatile template.
 *
 * @param   {Object}   req     The request
 * @return  {boolean}  True if the specified request is volatile template, False otherwise.
 */
function isVolatileTemplate (req) {
  return req.params?.templateId === 'template' && req.body?.template?.length > 0 && req?.jwtPayload?.iss !== 'demo';
}
...
function newWebhookItemToRender (req, res, templateAbsolutePath, _renderOptions) {
  ...
  if (_renderOptions.batchSplitBy) {
    const _lastJob = queueJobByTenant.get(_renderOptions.tenantId);
    if (_lastJob && _lastJob > (Date.now() - (params.converterFactoryTimeout * 2))) {
      return res.send(returnMessage({ errorCode : 'w122' }), 429);
    }
    queueJobByTenant.set(_renderOptions.tenantId, Date.now());
  }
...
function renderReport (req, res, templateAbsolutePath, data, renderOptions) {
  try {
    ...
    carbone.render(templateAbsolutePath, data, renderOptions, (err, reportAbsolutePath, reportName, debugInfo) => {
      ...
      if (isVolatileTemplate(req) === true) {
        fs.unlink(templateAbsolutePath, (err) => {
          if (err) {
            console.log('Warning: Volatile template cannot be deleted: '+ templateAbsolutePath);
          }
        });
      }

      if (renderOptions.batchSplitBy) {
        queueJobByTenant.delete(renderOptions.tenantId);
      }
      let _renderId = '';
      ...
      storageAddon.afterRender(req, res, err, reportAbsolutePath, reportName, _statistics, (err) => {
        ...
        if (renderOptions.isDebugActive === true) {
...
function getRenderedReport (req, res) {
  ...
  storageAddon.readRender(req, res, _renderId, (err, renderAbsolutePath) => {
    if (err) {
      ...
      return res.send(returnMessage({ errorCode : 'w104' }), 404);
    }
    sendFile(req, res, renderAbsolutePath, _filename, (err) => {
      if (err) {
        console.log('Send rendered report error: ' + err);
      }
...
/**
 * Sends a file stored on disk
 *
 * WARNING: if the req.method = HEAD, the callback is NOT called.
 * res.send is called to send only headers.
 *
 * @param  {Object}    req       The request
 * @param  {Object}    res       The resource
 ...
 */
function sendFile (req, res, filePath, filename, callback) {
  ...
  let _contentDisposition = `filename="${encodeURIComponent(filename)}"`;
  if (req.query?.download === 'true') {
    _contentDisposition = 'attachment; ' + _contentDisposition;
  }
  res.setHeader('Content-Disposition', _contentDisposition);
  // avoid deleting the file if it is a HEAD request
  if (req.method === 'HEAD') {
  ...
  // Why? 99% of the time, the file exists. So we do a lazy check. If node cannot
  // read it, it must returns a 404 error. But the NodeJS pipeline can destroy the stream before
  // sending the 404 reponse. To avoid a crash (ERR_HTTP_HEADERS_SENT), we initiate the stream only
  // if we can open the file. And we make sure the 404 error is sent only if 'open' event has not been triggered (pipeline started)
  let _hasFile = false;
  _file.on('open', () => {
    _hasFile = true;
  ...
  _file.on('error', (e) => {
    if (_hasFile === false) {
      _hasFile = true; // if error is fired twice. We should avoid to use once('error') according to our experience with rock-req
      res.send(returnMessage({ errorCode : 'w104' }), 404); // send 404 only if header not already sent (ERR_HTTP_HEADERS_SENT)
      callback(e);
    }
  });
}
...
function getTemplate (req, res) {
  ...
  storageAddon.readTemplate(req, res, _templateId, (err, templateAbsolutePath) => {
    ...
    carbone.getFileExtension(templateAbsolutePath, (err, extension) => {
      ...
      sendFile(req, res, templateAbsolutePath, _filename, (err) => {
        if (err) {
 ...
 * For now: used only for Make
 */
function getTemplates(req, res) {
  return res.send(returnMessage(null, null, []));
}
...
function deleteTemplate (req, res) {
  ...
  storageAddon.deleteTemplate(req, res, _templateId, (err, templateAbsolutePath) => {
    if (err) {
      ...
      console.log('[delete template] plugin storage error: ' + err.toString());
      return res.send(returnMessage({ errorCode : 'w105' }), 400);
    }
    fs.unlink(templateAbsolutePath, (err) => {
      if (err) {
        console.log('[delete template] template already deleted in local storage');
      }
...
function addTemplate (req, res) {
  ...
  if ( _isJSONType === true ) {
    if (typeof(req.body.template) !== 'string' || req.body.template.length === 0) {
      return res.send(returnMessage({ errorCode : 'w112' }), 422);
    }
    const _templatePathTemp = path.join(params.templatePath, helper.getUID());
    ...
    return fs.writeFile(_templatePathTemp, _template, 'base64', (err) => {
      ...
      try {
        req.headers['carbone-template-size'] = Buffer.byteLength(_template, 'base64');
      } catch(err) {
        req.headers['carbone-template-size'] = 0;
      }
      return writeTemplateAndEnd(req, res, _hashId, _templatePathTemp);
    });
  }

  // else if content-type = multipart/form-data
  const form = formidable({
    maxFileSize   : params.maxTemplateSize,
  ...
  form.parse(req, (error, fields, files) => {
    ...
    req.headers['carbone-template-size'] = files.template.size;
...
function writeTemplateAndEnd (req, res, hashId, templatePathTemp) {
  verifyTemplateType(req, res, templatePathTemp, function () {
...
function verifyTemplateType (req, res, templatePath, callback) {
  carbone.getFileExtension(templatePath, (err, extension) => {
    if (err) {
      console.log('[verify template type] error: ' + err.toString());
      return res.send(returnMessage({ errorCode : 'w118' }), 415); // 415: Unsupported Media Type
    }
    req.headers['carbone-template-extension'] = extension;
    req.headers['carbone-template-mimetype'] = mimetype?.[extension]?.mime ?? '';
    return callback();
  });
}
          ...
          send : (response) => {
            try {
              rock({
                ...
                tenantId : _queueItemData?.req?.jwtPayload?.iss, // for egress controller
                headers  : {
                  'Content-Type' : 'application/json',
                  'user-agent'   : 'Carbone' // AWS load balancer requires a user-agent
                }
              }, function (err, res, data) {
                /** TODO: remonter sur Carbone Account si ça a fonctionné pour debugger pour l'utilisateur */
                if (err) {
                  console.log(`Webhook Render Job ${jobId} - Error Status ${res?.statusCode} POST ${_queueItemData.successUrl}. "${_queueItemName}": ${err.toString()} ${data}`);
                }
                else {
                  console.log(`Webhook Render Job ${jobId} - Status ${res?.statusCode} POST ${_queueItemData.successUrl}`);
                }
                return runWebhookRenderJob(jobId, _queueSize - 1);
              });
            }
...
/**
 * Validate or sanitize template Id according to parameters
 *
 * @param  {string}  templateId  The template identifier
 * @return {string}              templateId or null if templateId is not valid
 */
function validateTemplateId (templateId) {
  if (params.securityLevel & helper.SECURITY_API_ID_TEMPLATE_64_HEX_ONLY) {
    if (!(params.securityLevel & helper.SECURITY_API_DISABLE_DEMO_TEMPLATE) && templateId === 'template') {
      return templateId;
    }
    templateId = typeof templateId === 'string' ? templateId.replace(':', '') : templateId;
    return /^[a-f0-9]{64}$/.test(templateId) === true ? templateId : null;
  }
  return sanitizeValidateId(templateId);
}
...
function gracefullyExit () {
  ...
  SOFT_STOP = true;
  console.log('SOFT-STOP: stopping gracefully. GET /status returns 503');
  checkIntervalStop = setInterval(isRenderRunning, 3500);
  exitTimeout(5 * 60);
}

function isRenderRunning () {
  console.log(`SOFT-STOP: ${nbCurrentRender} renders left`);
  if (nbCurrentRender === 0) {
    console.log('SOFT-STOP: No more active renders. Exit in 15 seconds');
    clearInterval(checkIntervalStop);
    exitTimeout(12);
  }
}

function exitTimeout (timeoutSecond) {
  setTimeout(exit, timeoutSecond * 1000);
}
