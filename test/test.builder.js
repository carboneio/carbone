...
describe('builder', function () {
  ...
  describe('replaceColor', function () {
    it('should replace a color in xml with hexa format', function (done) {
      var str = '<xml><style:text-properties fo:color="#ff0000"/>Test des couleurs{bindColor(FF0000, RRGGBB)=d.color}</xml>';
      var data = {
        color: "0000FF"
      }
      parser.findMarkers(str, function (err, cleanedXml, markers) {
        builder.replaceColor(cleanedXml, markers, data, {}, function (err, xmlWithNewColor, newMarkers) {
          helper.assert(err, null);
          helper.assert(xmlWithNewColor, '<xml><style:text-properties fo:color="#0000FF"/>Test des couleurs</xml>')
          helper.assert(newMarkers, []);
          done();
        });
      })
    });
    ...
    it('should replace a color in xml with object format', function (done) {
      var str = '<xml><style:text-properties fo:color="#ff0000"/>Test des couleurs{bindColor(FF0000, OBJECT)=d.color}</xml>';
      var data = {
        color: {
          red: 128,
          green: 128,
      ...
      parser.findMarkers(str, function (err, cleanedXml, markers) {
        builder.replaceColor(cleanedXml, markers, data, {}, function (err, xmlWithNewColor, newMarkers) {
          helper.assert(err, null);
          helper.assert(xmlWithNewColor, '<xml><style:text-properties fo:color="#808080"/>Test des couleurs</xml>')
          helper.assert(newMarkers, []);
          done();
        });
      })
    });
    it('should replace a color one time (color loop)', function (done) {
      var str = '<xml><style:text-properties fo:color="#Ff0000"/><style:text-properties fo:color="#00ff00"/><style:text-properties fo:color="#0000FF"/>' +
                'Test des couleurs{bindColor(0000Ff, RRGGBB)=d.color}{bindColor(00ff00, RRGGBB)=d.color2}{bindColor(FF0000, RRGGBB)=d.color3}</xml>';
      var data = {
        color: "00Ff00",
        color2: "ff0000",
        color3: "0000FF"
      ...
      parser.findMarkers(str, function (err, cleanedXml, markers) {
        builder.replaceColor(cleanedXml, markers, data, {}, function (err, xmlWithNewColor, newMarkers) {
          helper.assert(err, null);
          helper.assert(xmlWithNewColor, '<xml><style:text-properties fo:color="#0000FF"/><style:text-properties fo:color="#FF0000"/><style:text-properties fo:color="#00FF00"/>Test des couleurs</xml>')
          helper.assert(newMarkers, []);
          done();
        });
      })
    });
    it('should replace a color one time (color loop) with other data', function (done) {
      var str = '<xml><style:text-properties fo:color="#Ff0000"/><style:text-properties fo:color="#00ff00"/>{d.tata}<style:text-properties fo:color="#0000FF"/>{d.titi}' +
                'Test des couleurs{bindColor(0000Ff, RRGGBB)=d.color}{d.toto}{bindColor(00ff00, RRGGBB)=d.color2}{bindColor(FF0000, RRGGBB)=d.color3} {d.tete}</xml>';
      var data = {
        color: "00Ff00",
        color2: "ff0000",
        color3: "0000FF",
      ...
      builder.buildXML(str, data, {}, function (err, newXml) {
        helper.assert(err, null);
        helper.assert(newXml, '<xml><style:text-properties fo:color="#0000FF"/><style:text-properties fo:color="#FF0000"/>tata<style:text-properties fo:color="#00FF00"/>titiTest des couleurstoto tete</xml>')
        done();
      });
    });
    it('should not replace a color in text and not in XML', function (done) {
      var str = '<xml><style:text-properties fo:color="#ff0000"/>Test des couleurs{bindColor(FF0000, #RRGGBB)=d.color} Voici un couleur #FF0000</xml>';
      var data = {
        color: "#0000FF"
      }
      parser.findMarkers(str, function (err, cleanedXml, markers) {
        builder.replaceColor(cleanedXml, markers, data, {}, function (err, xmlWithNewColor, newMarkers) {
          helper.assert(err, null);
          helper.assert(xmlWithNewColor, '<xml><style:text-properties fo:color="#0000FF"/>Test des couleurs Voici un couleur #FF0000</xml>')
          helper.assert(newMarkers, []);
          done();
        });
      })
    });
    it('should return an error: object of color not complete', function (done) {
      var str = '<xml><style:text-properties fo:color="#ff0000"/>Test des couleurs{bindColor(FF0000, object)=d.color}</xml>';
      var data = {
        color: {
          red: 120,
      ...
      parser.findMarkers(str, function (err, cleanedXml, markers) {
        builder.replaceColor(cleanedXml, markers, data, {}, function (err, xmlWithNewColor, newMarkers) {
          helper.assert(err, 'd.color is not an object with properties red, green and blue.');
          done();
        });
      })
    });
    it('should return an error: unknow format', function (done) {
      var str = '<xml><style:text-properties fo:color="#ff0000"/>Test des couleurs{bindColor(FF0000, unknow)=d.color}</xml>';
      var data = {
        color: "FFFFFF"
      }
      parser.findMarkers(str, function (err, cleanedXml, markers) {
        builder.replaceColor(cleanedXml, markers, data, {}, function (err, xmlWithNewColor, newMarkers) {
          helper.assert(err, 'unknow is not a correct format (object, RRGGBB, #RRGGBB)');
          done();
        });
      })
    });
    it('should return an error: hexa incorrect', function (done) {
      var str = '<xml><style:text-properties fo:color="#ff0000"/>Test des couleurs{bindColor(FF000, RRGGBB)=d.color}</xml>';
      var data = {
        color: "FFFFFF"
      }
      parser.findMarkers(str, function (err, cleanedXml, markers) {
        builder.replaceColor(cleanedXml, markers, data, {}, function (err, xmlWithNewColor, newMarkers) {
          helper.assert(err, 'The hexadecimal color in "_root.bindColor(FF000,RRGGBB)=d.color" has not a correct format (Exemple: FFFFFF)');
          done();
        });
      })
    });
    it('should return an error: no variable specified', function (done) {
      var str = '<xml><style:text-properties fo:color="#ff0000"/>Test des couleurs{bindColor(FF0000, RRGGBB)=}</xml>';
      var data = {
        color: "FFFFFF"
      }
      parser.findMarkers(str, function (err, cleanedXml, markers) {
        builder.replaceColor(cleanedXml, markers, data, {}, function (err, xmlWithNewColor, newMarkers) {
          helper.assert(err, '_root.bindColor(FF0000,RRGGBB)= : you must specify a variable to replace.');
          done();
        });
      })
    });
    it('should return an error: incorrect hexa in data', function (done) {
      var str = '<xml><style:text-properties fo:color="#ff0000"/>Test des couleurs{bindColor(FF0000, RRGGBB)=d.color}</xml>';
      var data = {
        color: "FFFFF"
      }
      parser.findMarkers(str, function (err, cleanedXml, markers) {
        builder.replaceColor(cleanedXml, markers, data, {}, function (err, xmlWithNewColor, newMarkers) {
          helper.assert(err, 'd.color is not a hexadecimal format (RRGGBB)');
          done();
        });
      })
    });
    it('should return an error: incorrect hexa in data', function (done) {
      var str = '<xml><style:text-properties fo:color="#ff0000"/>Test des couleurs{bindColor(FF0000, RRGGBB)=d.color} {d.toto}</xml>';
      var data = {
        color: "",
        toto: "toto"
      }
